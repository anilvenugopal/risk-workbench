...
.PHONY: help \
        ...
        wsl-user-setup \
        irp-pypi irp-local irp-status

COMPOSE     = docker compose -f infra/docker-compose.yml --env-file infra/.env
BOX         = $(COMPOSE) exec linux-box

# Path to the optional local editable irp-integration checkout.
...
# PyPI 0.8.0 is the tracked source. irp-local changes only the developer's
# installed package; `uv sync --frozen` restores the tracked PyPI package.
irp-pypi:   ## Restore the pinned PyPI irp-integration package
	uv sync --frozen --reinstall-package irp-integration
	@$(MAKE) --no-print-directory irp-status

irp-local:   ## Install your local editable irp-integration checkout in .venv
	@test -d "$(IRP_LOCAL_PATH)" || { \
	    ...
	    exit 1; }
	uv sync --frozen
	uv pip install --reinstall-package irp-integration -e "$(IRP_LOCAL_PATH)"
	@$(MAKE) --no-print-directory irp-status
...
irp-status:   ## Show which irp-integration source/version is currently active
	@uv run --no-sync python -c "import irp_integration as m; print('  version:', getattr(m, '__version__', '?')); print('  path:   ', m.__file__)" 2>/dev/null \
	    || echo "  (run a sync target first: make irp-pypi | irp-local)"
