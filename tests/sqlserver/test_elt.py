...
class TestEnrichValidation:
    def test_empty_dataframe_returns_zero_without_querying_db(
            self, enrich_single_key_target):
        df = pd.DataFrame({"elt_data_key": [], "risk_score": []})
        assert enrich(df, enrich_single_key_target, key_fields="elt_data_key",
                      connection="WORKBENCH") == 0

    def test_missing_key_in_dataframe_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"risk_score": [0.5]})
        with pytest.raises(SQLServerConfigurationError, match="not present in DataFrame"):
            enrich(df, enrich_single_key_target, key_fields="elt_data_key", connection="WORKBENCH")

    def test_nonexistent_target_table_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"elt_data_key": [1], "risk_score": [0.5]})
        with pytest.raises(SQLServerQueryError, match="does not exist"):
            enrich(df, "no_such_table", key_fields="elt_data_key", connection="WORKBENCH")

    def test_key_not_in_target_table_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"ghost_key": [1], "risk_score": [0.5]})
        with pytest.raises(SQLServerConfigurationError, match="not found in target table"):
            enrich(df, enrich_single_key_target, key_fields="ghost_key", connection="WORKBENCH")

    def test_no_matching_enrichment_columns_raises(self, enrich_single_key_target):
    ...
    def test_invalid_schema_name_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"elt_data_key": [1], "risk_score": [0.5]})
        with pytest.raises(SQLServerQueryError, match="Invalid SQL"):
            enrich(df, enrich_single_key_target, key_fields="elt_data_key", schema="bad schema",
                   connection="WORKBENCH")


class TestEnrichColumnMappingValidation:
    """column_mapping resolution happens before the staging/UPDATE statements:
    a mapped key or enrichment column that doesn't exist on the target table
    raises the normal validation errors, naming the *target* column."""

    def test_mapped_enrichment_column_not_on_target_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"elt_data_key": [1], "src_score": [0.5]})
        with pytest.raises(SQLServerConfigurationError, match="No matching enrichment columns"):
            enrich(df, enrich_single_key_target, key_fields="elt_data_key",
                   column_mapping={"src_score": "no_such_column"}, connection="WORKBENCH")

    def test_mapped_key_not_on_target_raises(self, enrich_single_key_target):
        df = pd.DataFrame({"src_id": [1], "risk_score": [0.5]})
        with pytest.raises(SQLServerConfigurationError, match=r"\['no_such_key'\]"):
            enrich(df, enrich_single_key_target, key_fields="src_id",
                   column_mapping={"src_id": "no_such_key"}, connection="WORKBENCH")

    def test_mapped_key_resolves_against_dataframe_name(self, enrich_single_key_target):
        ...
        with pytest.raises(SQLServerConfigurationError, match="not present in DataFrame"):
            enrich(df, enrich_single_key_target, key_fields="src_id",
                   column_mapping={"src_id": "elt_data_key"}, connection="WORKBENCH")

    def test_two_dataframe_columns_mapped_to_same_target_raises(self, enrich_single_key_target):
