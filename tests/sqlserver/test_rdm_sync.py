...
#
# Route behavior uses monkeypatched services. The service tests above cover SQL;
# these assertions mirror the EDM Sync route tests one-for-one.
...
def test_body_poll_populated_mid_sync_returns_204_no_swap(monkeypatch):
    ...
    grp = analysis_service.BrokerAnalysisGroup(
        ...
        analyses=[analysis_service.BrokerAnalysis(
            id="a1", irp_id="5521", name="AEP", rdm_id="rdm-1",
            rdm_name="R")])
...
def test_backfill_skips_stored_results_even_without_the_dedup_row(
    ...
    rdm_service.sync_detail(rdm_id=rdm_id, actor_id=workbench_db.user_a)
    entity_jobs.run_pending()
