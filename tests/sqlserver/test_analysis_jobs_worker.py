...
from tests.sqlserver.analysis_rows import (
    seed_currency,
    seed_edm,
    seed_portfolio,
    seed_suite,
...
from tests.fakes.fake_irp import ep_elements, stats_rows
...
def test_finalize_resolves_by_job_payload_analysis_id(workbench_db, fake_irp):
    seed_currency()
...
def test_finalize_without_analysis_id_fails_the_rwb_job(workbench_db, fake_irp):
    seed_currency()
    edm_id = seed_edm("EDM One")
    portfolio_id = seed_portfolio(edm_id)
    template_id = seed_template()
...
def test_finalize_metadata_failure_fails_the_rwb_job(workbench_db, fake_irp):
    seed_currency()
    edm_id = seed_edm("EDM One")
    portfolio_id = seed_portfolio(edm_id)
    template_id = seed_template()
...
def test_retrieval_stores_the_extract_and_reports_row_counts(workbench_db, fake_irp):
...
def test_retrieval_skips_when_results_already_stored(workbench_db, fake_irp):
    analysis_id = _seed_finished_analysis(
...
def test_retrieval_fails_without_rm_id(workbench_db, fake_irp):
...
def test_retrieval_failure_leaves_extract_null_and_run_finished(workbench_db, fake_irp):
    analysis_id = _seed_finished_analysis()
...
def test_retrieval_rereads_metadata_when_the_pointer_is_missing(workbench_db, fake_irp):
...
def test_retrieval_fails_when_no_pointer_anywhere(workbench_db, fake_irp):
...
def test_broker_retrieval_uses_the_stored_exposure_pointer(workbench_db, fake_irp):
...
def test_broker_retrieval_fails_when_no_pointer_anywhere(workbench_db, fake_irp):
...
def test_claim_snapshots_the_plan_item_and_a_resumed_claim_keeps_it(
    ...
    claimed = analysis_jobs._claim_analysis(
        ...
        item=item, execution_id=execution_id, actor_id=workbench_db.user_a)
