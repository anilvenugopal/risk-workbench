...
def test_claim_racing_cancel_resolves_to_one_winner(workbench_db):
...
def test_resubmit_via_ensure_pending_resets_same_row(workbench_db):
...
def test_list_rwb_jobs_for_monitoring_returns_all_types_and_fields(workbench_db):
...
def test_list_rwb_jobs_for_monitoring_orders_by_type_then_status_then_recency(workbench_db):
    from app.services.rwb_job_service import list_rwb_jobs_for_monitoring
...
def test_resubmit_rwb_job_by_id_matches_ensure_pending_contract(workbench_db):
...
def test_resubmit_rwb_job_unknown_id_returns_none(workbench_db):
    from app.services.rwb_job_service import resubmit_rwb_job

    assert resubmit_rwb_job(rwb_job_id=str(uuid.uuid4())) is None


def test_resubmit_rwb_job_non_terminal_row_returns_none(workbench_db):
...
def test_resubmit_rwb_job_rejects_succeeded_and_cancelled_rows(workbench_db):
    from app.services.rwb_job_service import resubmit_rwb_job
