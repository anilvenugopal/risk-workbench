...
def test_finished_enqueues_finalize_analysis(workbench_db, fake_irp):
...
def test_finished_without_analysis_id_passes_none(workbench_db, fake_irp):
    a = _submitted_analysis(workbench_db, fake_irp)
...
def test_retry_skips_a_soft_deleted_analysis(workbench_db, fake_irp):
...
def test_delete_before_retry_claim_prevents_submission(workbench_db, fake_irp):
    row = _submission_failed_row(workbench_db, fake_irp)
    _age_completed_at(row["job_id"], seconds_ago=10_000_000)
    fake_irp.raise_on_submit_analysis_for.discard("CRE_Portfolio A_Template A")
...
def test_delete_after_retry_claim_is_rejected(workbench_db, fake_irp):
    row = _submission_failed_row(workbench_db, fake_irp)
...
def _stranded_row(workbench_db, fake_irp) -> dict:
...
def test_reclaim_returns_a_stale_retrying_row_to_submission_failed(
        workbench_db, fake_irp):
    row = _stranded_row(workbench_db, fake_irp)
...
def test_reclaim_leaves_a_retry_still_in_flight_alone(workbench_db, fake_irp):
    row = _stranded_row(workbench_db, fake_irp)
...
def test_reclaimed_row_is_retried_once_its_backoff_elapses(workbench_db, fake_irp):
    row = _stranded_row(workbench_db, fake_irp)
    _age_updated_at(row["job_id"],
                    seconds_ago=settings.irp_submission_retry_stale_secs + 5)
