"""Package-card data + the read-only gate (US5, T043).

``package_job_counts`` scopes all/active/failed to a package's members;
``get_package_cards`` exposes member chips + source paths with no rolled-up
package status (FR-018), and create/sync/delete are blocked on a
COMPLETED/CANCELLED submission (SC-011).
"""
