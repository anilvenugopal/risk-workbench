...
def test_template_name_joined_and_survives_template_soft_delete(workbench_db):
    edm = _edm()
...
def test_inserted_at_and_irp_id_populated(workbench_db):
    edm = _edm()
...
def test_rm_url_needs_irp_app_analysis_id_and_a_configured_rm_ui(workbench_db, monkeypatch):
...
def test_finished_reads_ready_and_not_live_once_results_stored(workbench_db):
    edm = _edm()
...
def test_more_than_2100_analyses_use_each_newest_job(workbench_db):
    edm = _edm()
    now = _utcnow()
...
def test_group_key_truth_table(workbench_db):
    edm = _edm()
...
def test_is_deletable_truth_table(workbench_db):
    edm = _edm()
    seeded = _one_of_each_state(edm)
    rows = {a.id: a for a in analysis_service.list_executed_analyses(edm_id=edm)}
...
def test_submission_retrying_is_in_progress_and_not_deletable(workbench_db):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="pending")
...
def test_delete_cascades_to_rm_then_soft_deletes(workbench_db, fake_irp):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="ready", irp_id="9001")
    _job(analysis_id=analysis, status="FINISHED")

    outcome = analysis_service.delete_executed_analyses(
        edm_id=edm, analysis_ids=[analysis], actor_id=workbench_db.user_a)
...
def test_delete_without_irp_id_is_local_only(workbench_db, fake_irp):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="pending")
...
def test_delete_rm_failure_keeps_the_row_and_continues(workbench_db, fake_irp):
    edm = _edm()
...
def test_delete_rejects_a_non_terminal_row(workbench_db, fake_irp):
    edm = _edm()
...
def test_delete_rejects_a_row_of_another_edm(workbench_db, fake_irp):
    ...
    with pytest.raises(ValueError):
        analysis_service.delete_executed_analyses(
            edm_id=edm, analysis_ids=[foreign], actor_id=workbench_db.user_a)
    assert _deleted_at(foreign) is None


def test_delete_rejects_finished_but_unbackfilled_row(workbench_db, fake_irp):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="pending")
    _job(analysis_id=analysis, status="FINISHED")
...
def test_delete_keeps_earlier_rows_when_the_poller_claims_a_later_one(
    ...
    outcome = analysis_service.delete_executed_analyses(
        edm_id=edm, analysis_ids=[first, raced], actor_id=workbench_db.user_a)
...
def test_results_ready_carries_all_five_perspectives_in_kind_order(workbench_db):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="ready", irp_id="9001",
...
def test_condensed_filter_keeps_six_points_largest_first(workbench_db):
    edm = _edm()
...
def test_pending_results_keep_a_ready_row_live(workbench_db):
    edm = _edm()
    analysis = _executed(edm_id=edm, status_code="ready", irp_id="9001")
    _job(analysis_id=analysis, status="FINISHED")
...
def test_currency_reads_from_settings_metadata_or_blank(workbench_db):
    edm = _edm()
...
def test_submitted_settings_parsed_for_display(workbench_db):
    edm = _edm()
...
def test_submission_read_spans_every_edm_with_edm_name(workbench_db):
    submission = _submission(workbench_db.user_a)
...
def test_submission_read_derives_origin_from_rdm_id(workbench_db):
    submission = _submission(workbench_db.user_a)
    edm = _edm()
...
def test_submission_read_carries_results_state_and_job_status(workbench_db):
    submission = _submission(workbench_db.user_a)
    edm = _edm()
    _attach_edm(submission, edm)
    analysis = _executed(edm_id=edm, status_code="ready", irp_id="9001",
...
def test_framework_no_longer_competes_with_analysis_mode(workbench_db):
    edm = _edm()
...
def test_results_columns_follow_ids_order_and_count_missing(workbench_db):
    edm = _edm()
...
def test_results_columns_broker_row_and_failed_retrieval_join(workbench_db):
    edm = _edm()
...
def test_results_column_run_currency_own_row_reads_submitted_snapshot(workbench_db):
    edm = _edm()
    analysis = _executed(
...
def test_results_column_run_currency_broker_row_reads_settings_metadata(workbench_db):
    edm = _edm()
    rdm = _mk("irp_rdm", name="R", status="ready")
...
def test_results_column_run_currency_missing_reads_none(workbench_db):
    edm = _edm()
    rdm = _mk("irp_rdm", name="R", status="ready")
...
def test_results_column_engine_reads_the_extract_snapshot_never_metadata(workbench_db):
    edm = _edm()
...
def _comparable_fixture(workbench_db):
...
def test_comparable_analyses_submission_scope_in_table_order(workbench_db):
    fx = _comparable_fixture(workbench_db)
...
def test_comparable_analyses_contextual_scope_narrows_own_rows(workbench_db):
    fx = _comparable_fixture(workbench_db)
...
def test_comparable_analyses_plain_edm_scope_has_no_broker_rows(workbench_db):
    fx = _comparable_fixture(workbench_db)
...
def test_broker_row_currency_reads_the_live_nested_currency_object(workbench_db):
...
def test_comparable_analyses_gone_scope_reads_none(workbench_db):
    fx = _comparable_fixture(workbench_db)
