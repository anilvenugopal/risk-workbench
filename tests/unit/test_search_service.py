...
Runs on the SQLite unit tier (``iteration2_db``) — every provider goes through
the ordinary ``db.execute`` bound-parameter path, so no provider needs its own
fixture beyond a handful of direct-insert rows.
"""
...
class TestGlobalSearchFloor:
    def test_blank_term_returns_no_groups(self, iteration2_db):
        assert search_service.global_search("", user_roles=["analyst"]) == []

    def test_below_min_term_returns_no_groups(self, iteration2_db):
        assert search_service.global_search("a", user_roles=["analyst"]) == []


class TestPagesProvider:
    def test_matches_by_label(self, iteration2_db):
    ...
    def test_role_gated_node_hidden_from_a_role_without_access(
            self, iteration2_db, monkeypatch):
...
class TestSubmissionsProvider:
    ...
    def test_no_match_omits_group(self, iteration2_db):
...
class TestEdmRdmProvider:
    def test_edm_matches_by_name(self, iteration2_db):
    ...
    def test_rdm_matches_by_name(self, iteration2_db):
...
class TestTemplatesProvider:
    def test_matches_by_name(self, iteration2_db):
        _insert_template(iteration2_db.engine, name="US Wind DLM")
    ...
    def test_soft_deleted_excluded(self, iteration2_db):
        tid = _insert_template(iteration2_db.engine, name="Retired Template")
...
class TestUsersProvider:
    def test_matches_by_display_name(self, iteration2_db):
        _insert_user(iteration2_db.engine, display_name="Priya Sharma",
    ...
    def test_matches_by_email(self, iteration2_db):
        _insert_user(iteration2_db.engine, display_name="Priya Sharma",
                    email="priya.sharma@example.com")
    ...
    def test_inactive_user_excluded(self, iteration2_db):
        uid = _insert_user(iteration2_db.engine, display_name="Departed Analyst",
...
class TestTypeFilter:
    ...
    def test_unknown_type_returns_no_groups(self, iteration2_db):
        _insert_edm(iteration2_db.engine, name="Coastal HO 2026")
        groups = search_service.global_search(
...
class TestGroupCap:
    def test_caps_at_group_limit(self, iteration2_db):
        for i in range(search_service.GROUP_LIMIT + 3):
            _insert_edm(iteration2_db.engine, name=f"Matching EDM {i}")
