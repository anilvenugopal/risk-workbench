"""Unit tests for ``analysis_service.list_comparison_pairs`` (spec 013 T-01/T-06).
...
def test_pairs_string_parses_to_ordered_pairs(iteration2_db):
...
def test_unparseable_or_unresolvable_side_drops_the_pair_whole(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
    a = _own(edm, "A", extract=_extract())
...
def test_deleted_analysis_drops_the_pair_whole(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
    a = _own(edm, "A", extract=_extract())
    b = _own(edm, "B", extract=_extract())
    execute_command(
...
def test_self_pair_is_dropped(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
    a = _own(edm, "A", extract=_extract())
...
def test_unrecorded_currency_on_either_side_drops_the_pair(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
    a = _own(edm, "A", extract=_extract())
...
def test_currency_mismatch_drops_the_pair_naming_both(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
...
def test_only_the_first_five_pairs_survive(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
...
def test_percent_change_per_return_period_and_aal_and_std_dev(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
...
def test_zero_or_missing_base_reads_none_never_inf(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
    base = _own(edm, "A",
...
def test_absent_perspective_on_either_side_yields_no_percent(iteration2_db):
    edm = _mk("irp_edm", name="E", status="ready")
