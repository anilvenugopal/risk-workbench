"""Unit tests for the ``_snapshot_upsert`` race-recovery branch (_common.py).

The UNIQUE-violation branch is reachable only via a concurrent insert between
the pre-check UPDATEs and the INSERT — or via SQL Server's NULLs-are-equal
unique semantics, which SQLite cannot reproduce — so these tests simulate the
