...
1. Open **Templates → Analysis Metadata** (`/templates/metadata`). Before any sync: four tabs
   (Model Profiles, Output Profiles, Event Rate Schemes, Currencies), each with an empty state and
   no last-synced time.
...
2. As admin, create a template: pick a DLM profile (e.g. an `RL25` one) — the Event Rate Scheme
   ...
   save → rejected naming the DLM rule. Pick an HD profile → saves without a scheme.
...
5. Compose a suite from a DLM + an HD template: reorder items, set a per-item portfolio-name
   override, try adding the same template twice (blocked). Save — no mixing error.
...
| SQL Server | `make test-sql` (or `make wsl-test-sql`) | migration of the 8 tables, filtered unique indexes, schema-drift guard |
