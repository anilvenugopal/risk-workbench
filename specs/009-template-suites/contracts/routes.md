...
| `POST /templates/analysis-templates/{id}/duplicate` | admin | Save an identical copy (fields + tags) named `<name> (copy)` — counter on collision, base truncated to fit 200 chars — and redirect to the copy's detail/edit page (P-12/FR-021, T-11). |
| `GET /templates/analysis-templates/scheme-options` | all | HTMX fragment: `<option>` list of live schemes matching `?profile=<name>`'s peril/region, pre-selected when exactly one (T-03). Triggered on profile change; the options render as soon as a profile is chosen, before any filter keystroke (FR-007/O17-9). |
...
| `POST /templates/suites/{id}/duplicate` | admin | Save a copy of the suite with the same membership rows (templates shared, never deep-copied), named the same `(copy)` way, and redirect to the copy's detail/edit page (P-12/FR-021, T-11). |
...
DLM/HD/Accumulation marker + raw software version. Shared context builder with the page route so they cannot drift. Each tab also carries an "Open in Risk Modeler ↗" deep link to that tab's RM settings screen — a fixed tenant-relative path per tab (`riskmodeler/datasources/model-settings/profiles`, `.../output`, `home/reference-data/currencies/currency` for the `currencies` tab, `home/reference-data/currencies/currency-schemes` for the `currency-schemes` tab — split into its own path 2026-08-19, user-corrected), joined to the same tenant-subdomain base the EDM deep links use (`_rm_ui_root`, shared with `edm_service._rm_datasource_url`); hidden when the tenant/base URL is unconfigured. Event Rate Schemes links to Model Composer (`riskmodeler/modelcomposer#event-rate-schemes`, added 2026-08-23 — the schemes live there, not under a `datasources` settings screen). The event-rate tab also renders the `workbench_is_active` value per row (FR-022): an admin sees a toggle checkbox, everyone else the read-only value. |
...
| `POST /templates/metadata/event-rate-schemes/{irp_id}/visibility` | admin | Set `workbench_is_active` on one scheme (FR-022/P-13). Checkbox semantics: `is_active` form field present → active, absent → inactive. HTMX request returns the re-rendered metadata-table fragment (`tab`/`q` carried as query params so the filter survives); non-HTMX PRG to `?tab=event-rate-schemes`. A stale `irp_id` (row removed by a re-sync) is not an error — the re-render simply no longer shows the row. |
...
list_currency_schemes() -> list[CurrencySchemeEntry]  # irp_id, name, code,
                                                      ...
                                                      # no is_default — nothing consumes it
                                                      # (2026-08-19); anchor_currency_code and
