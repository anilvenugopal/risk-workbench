...
**Purpose**: Schema, seed row, nav manifest entries, and router skeleton that every user story builds on.
...
- [X] T050 [US2] [FR-007/O17-9] Event-rate scheme options must populate on model-profile selection: reproduce the blank-until-typed behavior from the 8/20 demo against current code (the T028 fragment may already cover it — the demoed build may predate it); fix the trigger if real; route test asserting the scheme options render on profile change with no filter input. *Investigated 2026-08-20: traced the profile→scheme cascade (native `change` event dispatched with `bubbles:true` from `pick()`, htmx's `hx-vals='js:...'` reading the just-set value, and the `@htmx:after-swap` Alpine listener — confirmed htmx dispatches both the camelCase and kebab-case event names, verified against the vendored `htmx.min.js` and this codebase's other working `@htmx:after-swap` usages) — no defect found in the cascade; the two "Fix dropdowns"/"Fix model profile dropdown" commits earlier the same day (the `@mousedown.prevent` fix) most likely already fixed the demoed symptom. No code change; added regression tests instead (`test_scheme_options_populate_on_profile_change_alone`, `test_edit_form_prefills_scheme_options_for_the_stored_profile`).*

### Design-session-18 amendments (2026-08-23 — note 18; spec P-13/FR-022)

- [X] T051 [US1] [P-13/FR-022] Event-rate-scheme `workbench_is_active` flag: column added to `irp_event_rate_scheme` in `alembic/versions/0001_initial.py` (BIT NOT NULL DEFAULT 1) and `tests/iteration1_mirror.py`; `scheme_options()` filters to active schemes (pairing validation deliberately does not — a template saved against a later-deactivated scheme stays saveable); `set_scheme_visibility()` in `template_service.py` (never stamps `updated_at`, which the metadata page reads as "last synced"); `POST /templates/metadata/event-rate-schemes/{irp_id}/visibility` (admin + CSRF; HTMX returns the re-rendered table fragment with `tab`/`q` carried as query params, non-HTMX PRG); Active column on the event-rate tab of `metadata_table.html` (admin checkbox posting on change, read-only Yes/— otherwise; `metadata_table` fragment route now injects `current_user` so the checkbox survives tab/filter swaps); page/form hint copy updated. Sync worker untouched — its UPDATE lists columns explicitly, guarded by `test_resync_preserves_workbench_scheme_visibility`. *Done 2026-08-23; column-vs-side-table trade-off in research.md R16. 969 unit tests green.*
- [X] T052 [US1] [FR-003] Risk Modeler link on the Event Rate Schemes tab: `rm_path` set to `riskmodeler/modelcomposer#event-rate-schemes` in `_METADATA_TABS` (the schemes live under Model Composer); the tab's negative link assertion in `tests/unit/test_templates_metadata_routes.py` flipped to assert the Model Composer href. *Done 2026-08-23.*

**Checkpoint**: T021–T031 built and unit-green (958 unit tests) under the pre-note-17 design;
T047–T050 amend that build and are themselves done and unit-green (961 unit tests); T051–T052
(note 18) are done and unit-green (969 unit tests) but the schema change needs a DB rebuild and
`make test-sql` before the SQL Server tier is verified. Quickstart US2's live walkthrough against
the dev stack has not been run. **STOP** — approver clicks the running slice before Polish.
...
tests (T031). The workbook import and the seed-script suite rows that were built pre-deferral
are out of scope (spec P-02) and came out via T046.
...
button on both detail pages, and the O17-9 investigation (no defect found in the cascade
route or its hx-get trigger; regression tests added). The remaining work:
...
- UI previews (T015, T025) are approved informally (show → 👍) before the routes and templates are built
