...
**Prerequisites**: plan.md, spec.md, research.md (R1–R14), data-model.md, contracts/routes.md, contracts/transfer-workbook.md, quickstart.md
...
**Purpose**: Track the irp-integration work this feature consumes. The T-06 classification/validation utility **landed 2026-08-18** in the `0.6.0rc1` TestPyPI pre-release; the accumulation-profile read is **tabled** (old T001/T002 — moved to *Deferred: accumulation* below); the currency-scheme and scheme-vintage reads (T-07) are **in the sibling working copy awaiting a release**.
...
- [ ] T045 (external, in progress) Currency-scheme + scheme-vintage reads in `../../IRP/irp-integration` (T-07, R13): `search_currency_schemes`, `search_currency_scheme_vintages`, and `get_latest_currency_scheme_vintage` exist in the sibling working copy (uncommitted on top of `0.6.0rc1`) — the remaining external work is finishing and **releasing** them (TestPyPI, `make irp-testpypi` pin bump). The release pins the `irp_currency_scheme`/`irp_currency_scheme_vintage` columns (data-model.md) and `CurrencySchemeEntry`/`CurrencySchemeVintageEntry` fields (contracts/routes.md). O15-2 is already resolved (P-07 as amended): templates store currency + scheme + vintage. Gates T046–T048

**Checkpoint**: the T-06 validation utility is importable locally. (Accumulation reads deferred; `irp_model_profile.is_accumulation` stays in the schema, defaulting 0, so resuming accumulation is additive — no migration churn. Currency schemes: US1 shipped currencies only — which stay (P-07 as amended) — and the scheme/vintage additions happen in *Rework: currency schemes* once T045's release lands.)
...
**Goal**: Admin builds analysis templates (cached pick lists, DLM-requires-scheme, R8 defaults, tags) and composes unordered suites (no duplicates); four starter suites seed on rebuild; everything global, mutations admin-gated, unresolved references flagged.

**Independent Test**: As admin: create a DLM template (save blocked without a scheme), an HD template (saves without one), a mixed suite; deleting a referenced template is blocked naming the suite; as non-admin: everything visible, no mutation controls, direct POSTs rejected; after `make db-rebuild` the four starter suites exist and survive a re-seed after an edit (quickstart US2).
...
- [ ] T022 [US2] Suite CRUD + item composition in `app/services/template_service.py`: create/update/soft-delete suites, items hard-rewritten on save as an unordered set (no position, no per-item settings — P-08; display sorts by template name), duplicate template per suite rejected (`UNIQUE(suite_id, template_id)`), no DLM/HD/accumulation mixing restriction — same file as T021, sequential
...
- [ ] T029 [US2] Suite builder routes in `app/routers/templates.py` + templates: `GET /templates/suites/new`, `POST /templates/suites` (create with items), `GET /templates/suites/{id}` (detail, items sorted by template name, empty-state marker), `POST /templates/suites/{id}` (update — items rewritten), `POST /templates/suites/{id}/delete`; item rows partial in `app/templates/partials/suite_item_rows.html`
...
- [ ] T031 [P] [US2] Unit tests for routes + gating in `tests/unit/test_templates_routes.py`: create/edit/delete flows, DLM rejection re-renders with the rule named, duplicate-name form error, delete-guard message, item add/remove round-trip, same-template-twice blocked, non-admin sees no controls and direct POSTs are rejected, unresolved badge renders
- [ ] T032 [US2] Workbook parse + import-apply in `app/services/template_service.py` per `contracts/transfer-workbook.md` (needed by the starter seed): openpyxl parse of `Templates`/`Suites` sheets, whole-file validation collecting `(sheet, row, message)` — missing required value, wrong type, duplicate `Name` / `(Suite Name, Template Name)` in file, DLM without scheme and scheme/profile peril-region mismatch when both resolve (both via the T-06 utility, same rules as T021 save), unknown header or sheet — then all-or-nothing one-transaction apply: match-by-name update (tags replaced) or create; matched suites' item lists replaced wholesale; empty-suite row handling; values absent from cache import fine (FR-019); depends on T003 (T-06 utility)
- [ ] T033 [US2] Author the seed workbook `infra/scripts/starter_suites.xlsx` in the transfer-workbook format (2026-08-18 layout: no `Treaty Name Pattern`, no `Position`, no `Portfolio Name Override`; `Currency Scheme` + `Currency Vintage` + `Currency` columns, P-07 as amended — the draft workbook in the repo predates this and needs re-authoring): US, Canada, US+Canada, Global suites, ~10 templates each with indicative settings built from `RMS Default *` profile names observed in the sandbox (P-02)
...
**Goal**: Export everything — every suite with its items plus suite-less templates — to one `.xlsx` workbook; import applies all-or-nothing with every error reported by sheet/row; round-trip is field-identical.
...
- [ ] T036 [US3] Workbook export builder in `app/services/template_service.py`: `Templates` sheet (full field set, tags semicolon-joined, R8 occupancy label mapping, TRUE/FALSE booleans) + `Suites` sheet (suite name, template name); export always writes everything — every live suite and every live template including those in no suite (FR-016, no per-suite selection); rows normalized by name; empty suite exports as one row with `Template Name` blank
...
- [ ] T039 [P] [US3] Round-trip unit test in `tests/unit/test_transfer_workbook.py`: `export(all)` → import into an empty environment → `export(all)` produces field-identical sheets with row order normalized by name (FR-020, SC-004); re-import of the same file updates in place, no duplicates
- [ ] T040 [P] [US3] Import validation-matrix unit tests in `tests/unit/test_import_validation.py`: each FR-018 error class reported with sheet + row and nothing applied (missing required field, wrong type, in-file duplicate name, in-file duplicate suite membership, DLM without scheme, unknown column, missing sheet, peril-region mismatch); unsynced-cache values import and flag unresolved (FR-019); wholesale replace removes locally present items absent from the file (FR-017)
...
- [ ] T041 [P] Reconcile `docs/DATA_MODEL.md`: §7 deltas (`analysis_template_tag.tag_name` replaces `irp_tag_id`; `treat_construction_occupancy_as_unknown` added; `auto_name_pattern`/`region_label`/`peril_code`/`treaty_name_pattern` dropped (O15-6); `currency_scheme_code` + `currency_vintage` join `currency_code` (O15-2, P-07 as amended); `template_suite_item` loses `position`/`portfolio_name_override` — suites unordered; authorship = `inserted_by`, no separate `created_by`) and §10 cache-table columns per data-model.md, including the new `irp_currency_scheme` and `irp_currency_scheme_vintage` alongside `irp_currency`
...
## Rework: currency schemes (T-07, decided 2026-08-18 — gated on T045)

US1 shipped currencies only (`irp_currency` table, `list_currencies` gateway read, Currencies tab, P-06 16-char truncation) — all of which **stay** (P-07 as amended: analysis submission's currency block is `{code, scheme, vintage, asOfDate}`). Once the irp-integration scheme/vintage reads ship in a release (T045), **add** currency schemes and scheme vintages per spec P-07 / research R13, with the 2026-08-19 P-10/T-09 semantics: on the template the scheme + vintage are a **nullable pair** (blank = "Default", resolved at submit time in Iteration 7 — the submission API never defaults). US2's template builder (T027) needs the scheme/vintage pick lists, so run this rework before or alongside US2's builder work if T045's release lands in time; otherwise build the builder with the required currency pick list only (the blank pair is a valid state anyway) and add the scheme/vintage fields with the rest.

- [ ] T046 Add `irp_currency_scheme` and `irp_currency_scheme_vintage` to `alembic/versions/0001_initial.py` per data-model.md (columns pinned by T045; `irp_currency` unchanged); update the `ITERATION4_SCHEMA` mirror block in `tests/iteration1_mirror.py`, drift-guard lists, and the sqlserver assertions from T008 (extends T004/T007/T008)
- [ ] T047 Extend the gateway + worker: add `CurrencySchemeEntry`/`list_currency_schemes` and `CurrencySchemeVintageEntry`/`list_currency_scheme_vintages` to `app/services/irp_gateway.py` and `tests/unit/fakes/fake_irp.py` (`CurrencyEntry`/`list_currencies` and the P-06 truncation stay); the `sync_irp_metadata` actor gains the two fetches/upserts in the same transaction; update `tests/unit/test_metadata_sync_worker.py` and the `--run-irp` shape test (extends T011–T014, T020)
- [ ] T048 Swap the metadata UI: fourth tab becomes Currency Schemes listing schemes with their vintages (`?tab=currency-schemes`), tab order model-profiles → output-profiles → event-rate-schemes → currency-schemes; individual currencies lose their tab but stay cached for the builder (D3); update `docs/ui_previews/templates_metadata.html`, page/fragment templates, and `tests/unit/test_templates_metadata_routes.py` (reworks T015–T017, T019)
- [ ] T049 Template-side currency pair (P-10/T-09): add nullable `currency_scheme_code` + `currency_vintage` with the `ck_analysis_template_currency_pair` CHECK to `analysis_template` in `alembic/versions/0001_initial.py` and the `ITERATION4_SCHEMA` mirror; `template_service.py` save/import validation — vintage-without-scheme rejected, scheme-requires-vintage (builder), vintage-belongs-to-scheme when both resolve in the cache (skip + unresolved otherwise, R9-style); import fills a blank vintage from the scheme's latest cached vintage, erroring when the scheme is unresolved or vintage-less; builder gains the optional scheme pick list + `GET /templates/analysis-templates/vintage-options` HTMX fragment (latest by effective date pre-selected, `app/templates/partials/vintage_options.html`); list/detail render a NULL pair as "Default"; `Currency Scheme`/`Currency Vintage` workbook columns per contracts/transfer-workbook.md (blank pair round-trips as NULL/NULL); update `docs/ui_previews/templates_admin.html`, `tests/unit/test_template_service.py`, `test_templates_routes.py`, `test_template_workbook_import.py`, and the import validation matrix (extends T021, T025, T027, T032, T036, T040)
...
- **Phase 1 (external)**: T-06 **complete 2026-08-18** — shipped in `irp-integration==0.6.0rc1` (T003 done); T017/T021/T032 are no longer gated. T001/T002 are deferred with accumulation. T045 (currency-scheme + vintage reads — in the sibling working copy, awaiting a release) gates the T046–T049 rework.
