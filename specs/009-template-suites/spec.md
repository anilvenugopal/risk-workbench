...
| P-13 | **Workbench-owned active flag on event rate schemes** (design note 18 D1/D2/D3, closes note 17 O17-11): Risk Modeler filters schemes by a model-version mapping Moody's publishes only as an Excel download, so the Workbench cannot replicate it programmatically. Instead, admins curate a `workbench_is_active` flag per scheme — every scheme is still synced and listed on the metadata screen; only the template builder's scheme pick list is filtered to active schemes. Default is active; the sync never writes the flag; curation is periodic maintenance, not one-time setup. Scope is event rate schemes **only** — do not generalize to profiles or currencies. A template already referencing a scheme later marked inactive stays valid and saveable. | Approved 2026-08-21 (built 2026-08-23) |
...
### Session 2026-08-21 (design session, note 18; incorporated 2026-08-23)

- **Event-rate-scheme over-listing root-caused and resolved with a Workbench-side active flag** (D1/D2/D3 → P-13, FR-022). The metadata screen's Event Rate Schemes tab gains its first write affordance: an admin-only per-row checkbox. The flag is Workbench state about an IRP object, not an edit to the IRP object — the screen still never writes the synced values (O18-14). Ingesting Moody's Excel model-version mapping was considered and rejected (note 18 §2.2).
- Q: Column on `irp_event_rate_scheme` or a separate visibility side table? → A: **Column** (`workbench_is_active`, default active; user-decided 2026-08-23). The sync's keyed upsert lists its columns explicitly, so the flag survives re-sync; trade-offs recorded in research.md R16.
- The Event Rate Schemes tab now links out to Risk Modeler after all — the schemes live under **Model Composer** (`riskmodeler/modelcomposer#event-rate-schemes`), not a `datasources` settings screen (amends FR-003 / contracts/routes.md, which said no RM equivalent exists).
...
- **FR-022** (P-13): Each event rate scheme MUST carry a Workbench-owned active flag, default active, that admins toggle per row on the metadata screen's Event Rate Schemes tab (non-admins see the value read-only). The template builder's scheme pick list (FR-006/FR-007) MUST offer only active schemes. The sync MUST NOT overwrite the flag, all schemes stay synced and listed on the metadata screen regardless of the flag, and FR-005 pairing validation ignores it so a template saved against a later-deactivated scheme remains saveable.
...
- **FR-007**: The event-rate scheme pick list MUST populate with the chosen model profile's peril/region matches as soon as the profile is chosen — no keystroke needed (O17-9, 2026-08-20) — and SHOULD pre-fill automatically when a default is determinable — a DLM profile with exactly one active match, since only DLM requires a scheme. The admin can always change or clear it, and FR-005 validation applies to the saved value.
