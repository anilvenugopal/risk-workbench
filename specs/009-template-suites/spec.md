...
| P-02 | **Excel export/import and starter-suite seeding are out of MVP scope** — nothing is seeded and there is no transfer file; initial setup (including any starter suites) is manual via the admin page. The Excel-based flows are a nice-to-have enhancement; the worked design is retained in `contracts/transfer-workbook.md`. | Deferred 2026-08-19 |
...
| P-07 | The workbench caches and uses **all three currency objects — currencies, currency schemes, and currency-scheme vintages** — because analysis submission requires a specific value for each: the submit-time currency block is `{code, scheme, vintage, asOfDate}`, with `asOfDate` derived from the chosen vintage's effective date. A template therefore stores all three — currency, currency scheme, and scheme vintage, each required (P-10 as reversed 2026-08-19: never NULL, no default resolution anywhere). Schemes are also a display unit (design note 16 D3: CIC works in schemes; a currency appears in several schemes with different FX rates): the metadata screen gains a fifth tab listing currency schemes with their vintages (tab order: model profiles, output profiles, event-rate schemes, currencies, currency schemes). The scheme and vintage reads shipped in `irp-integration==0.6.0rc2` (released & pinned 2026-08-19); the built currency sync stays and the schemes/vintages sync + new tab add on top of it. D3's "no tab for currencies" call was **reversed 2026-08-19** (user-corrected): the Currencies tab stays alongside Currency Schemes. | Approved 2026-08-18, amended same day (store all three, not schemes-instead-of-currencies); tab reversal 2026-08-19 |
...
| P-10 | **Currency scheme + vintage are required** on every template, alongside the always-required currency — all three currency values are stored, never NULL, and there is **no default logic at submit time**: no "Default" state exists, and Iteration 7 submits exactly the stored values (a full `{code, scheme, vintage, asOfDate}` block, with `asOfDate` derived from the stored vintage's effective date — the submission API never defaults these values either). The builder pre-selects the chosen scheme's latest vintage by effective date (changeable); a scheme with no cached vintages cannot be saved (the rejection names the scheme). Currency-in-scheme membership (the scheme must carry an FX rate for the chosen currency) is **not validated** — the admin is trusted to pair them correctly; a mispairing surfaces at submit time in Iteration 7. | Approved 2026-08-19 — **reversed later the same day** (was: optional pair, blank = "Default" resolved workbench-side at submit time) |

**How to verify.** Sync metadata against the IRP sandbox and see profiles/schemes/currency schemes on the metadata screen; create a DLM template (event-rate scheme enforced) and an HD template (optional); compose a mixed suite.
...
An administrator opens the suite-administration page. They create an analysis template by picking a model profile, an output profile, a currency, a currency scheme, and a scheme vintage from the synced lists — all required (P-10), the vintage pre-selected to the chosen scheme's latest by effective date; the event-rate scheme field is required when the chosen profile is DLM and optional otherwise, pre-filled when a default is determinable. The builder also surfaces the analysis settings — min loss threshold, number of max-loss events, franchise deductible, unrecognized-occupancy treatment — plus optional tags. They then compose a suite: name it (the name conveys region and output level, P-03) and add templates — an unordered group, mixing DLM, HD, and accumulation templates freely. Suites are built by hand: nothing is seeded (P-02).
...
**Independent Test**: On the administration page, create a DLM template (save blocked until an event-rate scheme is chosen), an HD template (saves without one), and a suite containing both plus an accumulation template.

**Acceptance Scenarios**:

1. **Given** the synced cache, **When** the admin creates a template, **Then** model profile, output profile, event-rate scheme, currency scheme, scheme vintage, and currency are chosen from cached values only (filterable pick lists, no free text for these fields); currency scheme and vintage are required like the rest (P-10 — no blank value can be saved; the pick lists open on a placeholder rather than pre-selecting a real value).
...
8. **Given** a template submission missing the currency scheme or the vintage (a direct POST bypassing the form), **When** it is saved, **Then** the save is rejected naming the missing field — no template ever stores NULL for either (P-10).
9. **Given** a chosen currency scheme that has no vintages, **When** the admin tries to save, **Then** the save is rejected with a message naming the scheme (a vintage is required whenever a scheme is chosen).
...
- Any of a template's six saved reference values (the FR-011 list) disappears or is renamed on re-sync → template flagged unresolved (US2 scenario 7), pick lists show only current values, saved values are never silently rewritten.
...
- A currency scheme or vintage missing from the payload (direct POST bypassing the form) → rejected naming the field; both are required and never stored NULL (P-10).
- A vintage that does not belong to the chosen scheme (direct POST bypassing the form's filtered options) → rejected when both resolve in the cache; when either is absent the check is skipped and the template is flagged unresolved (FR-011), never blocked.
- A template's chosen currency has no FX rate in its chosen scheme → not detected here (membership deliberately unvalidated, P-10); it fails at submit time in Iteration 7.
...
- **FR-005**: An admin MUST be able to create an analysis template with:
  - **Required**: name; model profile; output profile; currency, currency scheme, and scheme vintage (P-07/P-10 — submission's currency block is `{code, scheme, vintage, asOfDate}`; all three are stored, never NULL, and submitted as stored — no default logic at submit time; only the as-of date is derived, from the stored vintage's effective date, never stored itself. The builder pre-selects the chosen scheme's latest vintage by effective date, and a scheme with no cached vintages blocks the save, naming the scheme; when both the chosen scheme and vintage are present in the cache, the vintage must belong to the scheme — the check is skipped when either is absent from the cache, which flags the template unresolved instead, FR-011); event-rate scheme (required when the profile is DLM, optional otherwise; when both the chosen profile and scheme are present in the cache, the scheme's peril/region must match the profile's — the same rule Risk Modeler enforces at submit).
...
- **FR-011**: A template whose saved reference value — model profile, output profile, event-rate scheme, currency, currency scheme, or currency-scheme vintage — is absent from the current cache MUST be flagged as unresolved — kept and editable, never silently changed or deleted.
...
- **Analysis template**: one analysis definition ("one row in Analysis Builder") — name, model profile, output profile, event-rate scheme, currency, currency scheme, and scheme vintage (all required, P-10), min loss threshold, number of max-loss events, franchise deductible, unrecognized-occupancy treatment, tags, authorship. Global. (`analysis_template`, `analysis_template_tag` — DATA_MODEL §7.)
...
- The admin picks a currency the chosen currency scheme actually carries an FX rate for; the workbench does not validate membership (deferred, P-10) and a mispairing fails at analysis submit in Iteration 7.
