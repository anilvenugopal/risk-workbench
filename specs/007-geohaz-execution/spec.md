...
| P-05 | Settles PRD O8-3 — what the workbench saves per lookup and shows as a portfolio's lookup history. Saved at submit: the parameter set, launching analyst, and submit timestamp. Saved at completion: terminal status, completion timestamp, and the `tasks[].output.summary` string. The portfolio's history displays the saved summary string without parsing it | Approved | Updated from captured response 2026-08-13 |
...
- **FR-022**: Expanding a portfolio row MUST show the portfolio's lookup history — one record per lookup with parameters, analyst, timestamps, and completion summary — as the only place lookup detail is displayed.
...
- **SC-003**: 100% of lookups completed through the workbench appear in their portfolio's history with the stored Risk Modeler summary, parameters, analyst, and timestamps.
...
- **P-05 settles PRD O8-3.** The workbench saves the parameter set, analyst, submit timestamp, terminal status, completion timestamp, and `tasks[].output.summary`. The portfolio history displays the saved summary string unchanged.
