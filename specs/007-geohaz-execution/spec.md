...
| P-03 | No geocode/hazard version stamp is displayed or read; the summary page shows the workbench's own execution history and in-line geohaz job status instead | Approved | Design session 2026-08-07; PRD §10B.4 |
...
| P-05 | Settles PRD O8-3 — what the workbench saves per lookup and shows as a portfolio's lookup history. Saved at submit: the parameter set (data version, model family, perils, missing-location handling), the launching analyst, the submit timestamp. Saved at completion: terminal status, completion timestamp, per-layer locations-looked-up counts. The saved record is also the display — the portfolio's history expands into it; nothing else is captured, and Risk Modeler's version stamp is never part of it | Approved | This spec; approved 2026-08-12 |
...
| P-07 | Display: the portfolios table gains one **"Hazard looked up?"** column — **No** (never looked up through the workbench), the job's **in-line status** while a lookup is non-terminal, **Yes** (at least one lookup completed successfully), **Failed** (lookups exist but none succeeded). Everything beyond the one-word answer lives in the **expanded portfolio row**: the per-lookup history with parameters, analyst, timestamps, and per-layer locations-looked-up counts | Approved | Approver direction, 2026-08-13 |
...
**In**: multi-portfolio launch with editable pre-populated defaults; one geohaz job per portfolio tracked by the poller; a "Hazard looked up?" column in the portfolios table (No / in-line job status / Yes / Failed) with the per-lookup detail in the expanded portfolio row; per-layer locations-looked-up summary on completion; the EDM + portfolio prerequisite gate.
...
**Exit** (PRD §21): select two portfolios on the EDM summary page and run hazard lookup with the default parameters; both jobs are tracked via the poller with in-line per-portfolio status; on completion the per-layer locations-looked-up summary is shown and each portfolio shows it has been hazard-looked-up through the workbench; the gate requires a portfolio before geohaz is enabled.
...
**Why this priority**: The launch is the iteration — everything else (status, history, summary) exists to track what the launch submitted. It is the from-screen action the 2026-08-07 design session approved ("Ability to execute hazard lookup from the screen — yes").
...
### User Story 2 - See lookup status and history per portfolio (Priority: P2)

After launching, the analyst stays on the EDM summary page. The portfolios table carries one new **"Hazard looked up?"** column that answers the question at a glance: **No** for a portfolio never looked up through the workbench, the job's **in-line status** while a lookup runs, **Yes** once a lookup has completed successfully, **Failed** when lookups exist but none succeeded. The column refreshes without a manual reload. When the analyst wants more than the one-word answer, they click the portfolio row to expand it: the expanded view lists the portfolio's lookup history — each lookup's parameters, who launched it, when, and its result. A portfolio never looked up through the workbench simply shows **No**; that is normal, because the workbench assumes exposure arrives geocoded and hazard-retrieved. No geocode/hazard version stamp appears anywhere, and nothing reads Risk Modeler's stamp to gate any action.

**Why this priority**: The status/history display is what makes the launch trustworthy — it is the app-side lineage the design session chose *instead of* displaying version stamps. Without it the analyst cannot tell whether a lookup ran, is running, or failed.
...
2. **Given** a portfolio whose lookup completed through the workbench, **When** the analyst views the portfolios table, **Then** its "Hazard looked up?" column reads Yes, and expanding the portfolio row shows the lookup history with each per-lookup record (parameters, analyst, timestamps, result).
3. **Given** a portfolio never looked up through the workbench — including one hazard-looked-up directly in Risk Modeler, **When** the analyst views the page, **Then** its column reads No and its expanded row shows no lookup history, presented as a normal state, not a warning or error.
...
5. **Given** a portfolio whose only geohaz job Risk Modeler reports as failed, **When** the analyst views it, **Then** its column reads Failed, the expanded row shows the failed lookup, the portfolio's own state is unchanged, and a new lookup can be launched on it. A failure after an earlier success leaves the column at Yes; the failed lookup is visible in the expanded history.

---

### User Story 3 - Read the per-layer completion summary (Priority: P3)

When a lookup completes, the analyst reads its result inside the expanded portfolio row: a summary of locations looked up per hazard layer — the count per peril — alongside the parameters the job actually ran with. An inapplicable peril — earthquake requested on a windstorm-only book — shows zero for that layer, not a failure. Because the record keeps the parameters, six months later the team can answer "what hazard data does this portfolio carry, and who ran it" by expanding the row.
...
1. **Given** a lookup that reached terminal success, **When** the analyst expands the portfolio row, **Then** the lookup's record shows the per-layer locations-looked-up counts alongside the submitted parameter set, the launching analyst, and the submitted/completed timestamps.
2. **Given** a lookup that included an inapplicable peril, **When** its summary is shown, **Then** that layer displays zero locations, presented as a value, not a failure.
3. **Given** a completion response with missing or partial per-layer detail, **When** the record is shown, **Then** the available fields render and the missing counts show a graceful unavailable state, never an error page.
4. **Given** a portfolio looked up more than once through the workbench, **When** the analyst views its history, **Then** each lookup is a distinct record with its own parameters and result.
...
- **Job fails in Risk Modeler**: the portfolio's history shows the failed lookup; no portfolio, EDM, or submission status changes; analysis is never blocked by it.
- **Completion response missing per-layer counts**: the record keeps status, parameters, analyst, and timestamps; counts show as unavailable.
- **Lookup run directly in Risk Modeler**: invisible to the workbench by design — the history is the workbench's own execution record, not a mirror of Risk Modeler.
- **Repeat lookups on one portfolio**: allowed once the prior job is terminal; history accumulates one record per lookup.
...
**Status & history on the summary page (US2)**
...
- **FR-014**: A geohaz job that fails — in submission or in Risk Modeler — MUST be visible as a failed lookup in the portfolio's expanded history (and as **Failed** in the column when no lookup has succeeded), MUST NOT change the portfolio's or EDM's own state, and MUST NOT block a later launch on that portfolio.
...
- **FR-020**: When a geohaz job reaches terminal success, the workbench MUST record the per-layer locations-looked-up summary from the completion response, in the background — never on a web request path.
- **FR-021**: The per-lookup record MUST comprise (P-05): the submitted parameter set (data version, model family, perils, missing-location handling), the launching analyst, the submitted and completed timestamps, the terminal status, and the per-layer locations-looked-up counts.
- **FR-022**: Expanding a portfolio row MUST show the portfolio's lookup history — one record per lookup (parameters, analyst, timestamps, per-layer counts), when a portfolio has been looked up more than once — as the only place lookup detail is displayed; the column carries nothing beyond its four states (P-07).
- **FR-023**: A layer with zero locations looked up MUST display as zero, and missing or partial per-layer detail in the completion response MUST render as an unavailable state — neither is ever presented as a failure of the page or the lookup.
...
- **Hazard-lookup record**: the P-05 lineage for one completed (or failed) lookup — parameter set, launching analyst, submitted/completed timestamps, terminal status, per-layer locations-looked-up counts. The workbench's substitute for Risk Modeler's version stamp. Where it is stored (on the job row or its own table) is a plan decision.
- **Portfolio**: the target of a lookup and the row the summary page hangs status and history on. Unchanged structurally — a lookup never alters the portfolio's own state.
...
- **SC-003**: 100% of lookups completed through the workbench show a per-layer locations-looked-up summary and appear in their portfolio's history with parameters, analyst, and timestamps.
...
- **P-05 settles PRD O8-3, and the record and the display are the same thing.** The workbench saves, per lookup, everything it knows at submit time (parameter set, analyst, submit timestamp) plus everything the completion response reports (terminal status, completion timestamp, per-layer counts) — and that saved record is what the summary page shows when a portfolio's lookup history is opened. There is no second, richer internal record and no display-only field; nothing is read from Risk Modeler's version stamp. The PRD assigns the decision to this spec.
...
- **Spec 004's EDM summary page** — the portfolios table gains the selection, the launch action, and the "Hazard looked up?" column; the lookup history reuses the table's existing per-row expand.
...
- **`irp-integration` active wheel** — confirm the geohaz submit parameter set (data version, model family, perils, missing-location handling) and the completion response's per-layer summary shape against the *active* wheel (`make irp-status`) before implementing; the library is pre-release and its signatures move. If the per-layer counts need a fetch beyond the status check, that fetch is worker-side, never in the poller loop body or on a request path.
- **WORKBENCH schema** — storage for the P-05 per-lookup record (parameter set + per-layer counts). The §21.0 DB-lifecycle prompt applies at planning: choose Rebuild / Refresh / Skip for `WORKBENCH`; `EXPOSURE`/`LOSS` are untouched and DATABRIDGE is never in schema scope.
