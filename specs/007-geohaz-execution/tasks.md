...
- [X] T005 Add `submit_geohaz(*, edm_name, portfolio_name, version, perils, skip_prev_hazard, override_user_def) -> SubmitResult` and `get_geohaz_job(irp_id) -> JobStatus` to `app/services/irp_gateway.py` (Protocol, `_RealGateway`, module free functions, `__all__`): build one hazard layer per peril — `{"type": "hazard", "name": <peril>, "engineType": "RL", "version": <version>, "layerOptions": {"overrideUserDef": <bool>, "skipPrevHazard": <bool>}}` — hazard-only, no geocode layer ever built (FR-005, research R4/R5); wrap `client.portfolio.submit_geohaz_job(portfolio_name, edm_name, layers)`; `resource_uri` comes from the returned request body, not the completion response (contracts/worker-poller.md)
...
- [X] T015 [US1] Add the selection form and launch button to `app/templates/partials/edm_portfolios_section.html`: button disabled until ≥1 eligible portfolio is checked, absent when the gate fails (no portfolios — FR-004)
...
- [X] T019 [P] [US2] Add `"geohaz": irp_gateway.get_geohaz_job` to `_GETTERS` in `app/poller/run.py` and refresh Get Portfolio Metadata after a successful terminal status (FR-020)
- [X] T020 [US2] Add the read model to `app/services/geohaz_service.py`: `read(*, edm_id=None, portfolio_id=None)` — one query returning, per portfolio, a `CellState` (geohaz `irp_job` rows + pending/running `run_geohaz` heads → SUBMITTING / in-line status / stored `hazardVersion`, carrying `live: bool`) and the newest geohaz `irp_job` as a `LatestLookup` (parsed `request_params`, `completion_summary`, status, timestamps); `edm_id` scopes the table render, `portfolio_id` the poll fragment
- [X] T021 [US2] Add `GET /edms/{edm_id}/portfolios/{portfolio_id}/geohaz-cell` to `app/routers/edms.py` (missing portfolio → terminal empty cell, never an error page), and attach the per-portfolio geohaz state and latest lookup (`read(edm_id=…)`) to the `get_edm_detail` read model in `app/services/edm_service.py` — both detail routes render from it (contracts/data-access.md) — depends on T020
...
- [X] T023 [US2] Add "Hazard Version" as the final column in `app/templates/partials/edm_portfolios_section.html`, updating `--cols` and the table `min-width` together
...
**Checkpoint**: User Stories 1 and 2 work together (launch → in-line status → refreshed `hazardVersion` → latest details). **STOP** for the approver.
...
- [X] T026 [US3] Add `_geohaz_completion_summary(result) -> str | None` to `app/poller/run.py`; the poller copies `tasks[].output.summary` into `irp_job.completion_summary` on the terminal write, and `geohaz_service.read` returns the stored string (research R7)
...
- [X] T030 Run `uv run pytest tests/unit` and report the tier and count; the SQL Server tier (`make test-sql` — drift guard for the three columns + kind row) and the sandbox capture (T029) need the developer's stack/credentials — name what ran and what did not (AGENTS.md Reporting results)
- [X] T031 Review the diff for subtraction per AGENTS.md: remove comments and tests that restate the implementation, inline helpers that only rename a call, remove speculative branches (e.g. the R7 `fetch_geohaz_summary` contingency stays unbuilt unless the capture demands it)
