...
**Ready for tasks:** Yes, with one assumption carried into the build (T-10 Parquet column names). No open items. Amendment history in [research.md](research.md#status-history).
**Blocked by:** Nothing for development. Verification against Risk Modeler needs a sandbox analysis run with treaties that take loss (quickstart.md). The exports section is the one export status screen and the model version is chosen on the export form (014 as amended 2026-09-15); this plan carries the Treaty column into that section, and every treaty data set runs through the same stage and load code as a portfolio data set.
...
- **`stage_results_export`** now acts on every manifest row of (`export_id`, `irp_analysis_id`) that is not staged, loaded, or closed. At TY it downloads once, reads `ELT/Treaty/TY/*.parquet`, splits the table per (`TreatyNum`, `TreatyName`) with pandas, and writes one derived Parquet file per treaty into the working directory holding that treaty's rows as Risk Modeler wrote them (P-11). Each treaty is matched to the analysis's eligible manifest rows by (`treaty_number`, `treaty_name`): a match is staged and stamped with `treaty_ids`, `aal`, and the Risk Modeler job id; a treaty no row claims is skipped and counted in one log line; a row whose treaty the table does not hold fails alone. Each treaty file is uploaded under its own `manifest_id` with `output_level = 'Treaty'`. A table with no treaty rows fails every treaty row of the analysis with "Risk Modeler returned no treaty (TY) loss rows for this analysis" (FR-007). One load job per analysis is enqueued (T-03, T-05).
- **`load_results_export`** loads every manifest row of the analysis that is staged, not loaded, and not closed, calling `stage.usp_load_elt_result @manifest_id` once per row. Each call is its own transaction; a failed row is stamped alone and the job reports every failure. The procedure takes an application lock so loads run one at a time ([014 load procedure contract](../014-results-export/contracts/load-procedure.md), issue #124); nothing else in it changes: `Data.Perspective` takes `TY` and `Data.DataName` the name the analyst gave that treaty on the form (FR-008, FR-009).
...
| Database (loss repository) | Four nullable manifest columns and a widened unique constraint in `db/bootstrap/loss_schema.sql`; `rwb_loss_result_file.output_level` now also takes `Treaty`. The procedure takes an application lock (`sp_getapplock`) so loads run one at a time, fixing issue #124; see the [014 load procedure contract](../014-results-export/contracts/load-procedure.md). |
| Worker | `retrieve_analysis_results` records treaties and, per treaty, whether its TY stats read answered rows; `submit_results_export` builds the treaty request; `stage_results_export` splits the table per treaty; `load_results_export` loads every eligible row of the analysis. |
...
| T-03 | The stage worker splits the treaty-level table per treaty with pandas into one derived Parquet file per treaty, the rows unchanged; the load procedure is unchanged | Approved | [research.md#R3](research.md#r3--split-in-the-stage-worker-not-in-the-procedure-t-03) |
...
| T-08 | A treaty row's AAL is the sum over its rows of rate times loss, computed by the stage worker and stored as `manifest.aal`; portfolio rows keep reading `loss_results` at render time | Approved | P-10 |
...
├── plan.md                  # This file
├── research.md              # R1–R8, status history
...
├── workers/export_jobs.py                 # one request per analysis; split per treaty and match; per-row load
