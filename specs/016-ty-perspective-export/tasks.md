...
- [x] T006 [US1] [FR-001] [P-05] `app/services/export_service.py` `list_exportable_analyses`: `TY` appended to `perspectives` when `loss_results.treaties` holds an entry with `has_loss` true; `create_export`'s per-analysis message at TY reads "{name} has no treaty with TY loss." (P-13)
...
**Goal**: a group exported at TY yields one data set per treaty, its rows the group table's rows for that treaty.

**Independent Test**: quickstart.md Story 3.

- [x] T014 [US3] [FR-003] [P-01] [T-04] No new code: T005 records a group's treaties by the same call and T009 stages a group's table as any other. This task is the sandbox check of the group half of T-04 (quickstart.md "Sandbox checks") and closes plan O-01 one way or the other.
  - Proof: quickstart.md Story 3 step 1 observed on the running stack 2026-09-21 (research R8); step 2, a group exported at TY, is unverified until someone runs it.
...
- [x] T016 [P] [T-02] [FR-009] `tests/sqlserver/test_loss_export_procedure.py`: a manifest row with `perspective_code 'TY'`, `treaty_number`, `treaty_name`, `treaty_ids`, and the analyst's `data_name` loads with `Data.Perspective = 'TY'` and `Data.DataName` unchanged; two treaty rows of one analysis insert and a third with the same treaty raises. Unverified until `make test-sql` runs.
...
- [x] T024 [US1] [FR-005] [FR-007] `app/workers/export_jobs.py` `_stage_treaties`: match each treaty in the table to an eligible row by (number, name), skip and log the treaties no row claims, fail a row whose treaty the table lacks. Delete `_treaty_data_name`, `_TREATY_ROW_COPIED_COLUMNS`, `_insert_treaty_row`, the `rows` parameter of `_stage_treaties` and `_stage`, and the `DATA_NAME_MAX_LEN` import (contracts/jobs.md §4 step 6).
