...
- Per analysis at TY: the treaties Risk Modeler applied when the analysis ran, each listed with its type, risk limit, attachment point, and occurrence limit. The analyst ticks the treaties to export and types the data name of each one while its terms are on screen (P-03, P-06, P-12).
...
| O-05 | Whether Risk Modeler reports a group analysis's treaties at all. The cart lists what the treaties endpoint returned when the results were retrieved, so an endpoint that answers nothing for a group leaves a group at TY with nothing to tick and story 3 with no path. | Open | plan O-01; user, 2026-09-17 |
...
3. **Given** TY is picked, **Then** each cart row lists that analysis's treaties with their terms and nothing ticked, the form shows the text "one loss set per ticked treaty, per analysis", and Export stays refused until every selected analysis has a treaty ticked (P-03, P-04, P-12).
...
- **FR-002**: When TY is picked the form shows a line stating that the export writes one loss set per ticked treaty, per analysis, and every cart row lists its analysis's treaties as tick boxes — treaty number, treaty name, type, risk limit, attachment point, occurrence limit (P-12) — with an all / none link over that analysis's list. Nothing is ticked until the analyst ticks it, a ticked treaty shows the data name field of FR-009, and Export is refused while any selected analysis has no treaty ticked (P-03). The form asks for no financial perspective (P-08).
