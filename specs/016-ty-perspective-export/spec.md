...
**Phase:** Built (plan.md, tasks.md T001–T033). Unit tier green. The SQL Server tier and the sandbox pytest checks of the treaties endpoint and the treaty-scoped stats read are unverified until someone runs them (quickstart.md). Amendment history in research.md "Status history".
...
- Per analysis: one treaty-level loss table requested from Risk Modeler, one data set written per ticked treaty, holding that treaty's rows as Risk Modeler returned them (P-11). Each treaty data set is classified, corrected, and loaded exactly as a portfolio-level data set is in 014.
...
2. A treaty is identified by its treaty number and treaty name: the treaty the analyst ticked is matched to the rows Risk Modeler wrote into the loss table by those two values. Treaty number is unique within an analysis. Treaties are never matched by name across analyses, and the analysis-time treaty ID is recorded for traceability only, never used as the identity.
3. Each treaty's rows are written as Risk Modeler returned them; Risk Modeler aggregates a group's members into one row per treaty and event (P-11). The Workbench never combines rows across treaties or across analyses: an export of N analyses at TY, each with M ticked treaties, writes N × M data sets.
4. A TY export whose loss table holds no treaty rows fails with a message naming TY and loads nothing.

## Product decisions

P-01 to P-13 are all decided; nothing awaits a product decision. The table is below the fold.
...
| P-11 | Each treaty's rows are written as Risk Modeler returned them. The treaty-level loss table holds one row per treaty and event; for a group, Risk Modeler aggregates the members before writing it. The Workbench combines nothing. | Approved | note 29 D22; user, 2026-09-23 (research R3) |
...
The analyst has five peril analyses in one EDM that all ran with the same two treaties and wants one loss set per treaty across all five, as the workflow tool gives them today. In the Workbench they first create the analysis group (spec 012), then export the group at TY. Risk Modeler aggregates the members into the group's one treaty-level table, one row per treaty and event; the Workbench reads it and loads one data set per treaty.
...
2. **Given** the group's two treaties ticked and the group exported at TY, **When** processing finishes, **Then** exactly one data set per ticked treaty is loaded, holding one row per event as Risk Modeler wrote it (P-11), and no per-member data set is written.
...
- **FR-012**: The status filter, Retry, and Close of 014 FR-017 and FR-019 work per treaty row. Retry of a treaty row re-arms the analysis's job from the last completed step for that row; other failed, unclosed treaty rows of the same analysis are retried alongside, and a loaded or closed row is never re-run. Retry never re-downloads an archive already held. Close records who closed it and when.
...
- **Treaty-level loss table**: the file Risk Modeler returns for one analysis at treaty output level, holding every treaty's events in one table, one row per treaty and event, with the treaty ID, number, and name on every row.
- **Treaty identity**: the treaty number and treaty name as defined in the EDM and copied into the loss table. The treaty ID on the same rows is minted when the analysis runs and differs between runs of the same treaty; every ID found is recorded and none is matched on.
- **Analysis group**: a Risk Modeler analysis that combines member analyses (spec 012). The route to one treaty data set across perils: Risk Modeler aggregates the members into one row per treaty and event in the group's treaty-level table, and the group is exported like any other analysis.
...
- **SC-005**: A failed treaty data set is diagnosed from its row's message and recovered with one Retry, which never re-runs a loaded or closed treaty of the same analysis.
