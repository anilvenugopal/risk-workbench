...
> **⚠️ Real Risk Modeler wiring status (2026-07-14):** `_RealGateway` in `app/services/irp_gateway.py` is now wired against the **active 0.2.0 wheel** (manager-based: `edm.submit_edm_import_job`, `rdm.submit_rdm_import_job`, `edm.submit_delete_edm_job`, `analysis.delete_analysis`, `analysis.search_analyses_paginated`, `import_job.get_import_job`, `risk_data_job.get_risk_data_job`, `edm.search_edms`, `rdm.search_imported_rdms`). The per-story tasks below that depended on this wiring (T020/T021/T022/T026/T027/T027a/T039/T040) are **re-opened** because the real path is **unverified against the IRP sandbox** (`pytest tests/irp --run-irp`) and defects remain:
...
- [ ] T026 [US2] Implement `app/services/rdm_service.py` (depends T010, T012, T014): `import_rdm` (enqueue one `upload_rdm` head; the worker fans out to one apply per applied EDM; **≥1 target EDM required — reject a no-EDM import with `EmptyPackageError`; RDM-only/review-only deferred, D3/FR-016**), `check_name_collision`, `list_rdms`, `get_rdm`, `replace_source_file`, `retry_import` (mirrors `edm_service`).
