...
- [X] T063 Run `pytest tests/sqlserver --run-sqlserver` and confirm the migration + atomic-claim + idempotent-insert tier is green (quickstart §3). ✅ Approver ran `make wsl-test-sql` (2026-07-15) — full SQL Server tier green, incl. `test_job_tables_migration` (irp_job/rwb_job/irp_analysis families + FKs + UNIQUE keys + seeds; atomic claim 1-then-0; idempotent chained insert) and `test_schema_drift`.
