...
- [ ] T061 Run `make db-rebuild` and confirm a clean drop-create-seed with all new tables + seeds present (quickstart §1). ⏳ Needs the container/WSL2 stack and is destructive (drops the 3 app DBs) — deferred to the approver's env so the working manual-test DB isn't wiped. The schema build itself is exercised non-destructively by T063's `test_job_tables_migration`, and the running app (manual-tested 2026-07-15) confirms the seed. Run: `make db-rebuild`.
- [X] T062 Run `pytest tests/unit` and confirm the full unit tier (fake IRP) is green (quickstart §2). ✅ `uv run pytest tests/unit` → **463 passed** (2026-07-15), including the new T064 guards.
- [ ] T063 Run `pytest tests/sqlserver --run-sqlserver` and confirm the migration + atomic-claim + idempotent-insert tier is green (quickstart §3). ⏳ `infra-sqlserver-1` is up/healthy, but the tier needs the `MSSQL_*` connection env (loaded via `infra/scripts/wsl-env.sh` / the linux-box container) which isn't present in a bare shell. Deferred to the approver's env. Run: `make test-sql` (or `source infra/scripts/wsl-env.sh && uv run pytest tests/sqlserver --run-sqlserver`).
- [ ] T063a Run `pytest tests/irp --run-irp` and confirm the real `irp_gateway` path against the IRP sandbox: submit + single-status-check `get_*` round-trips for `import_edm`/`import_rdm` and `delete_edm`, plus the synchronous `delete_analysis` call. This is the one remaining formal gate on the Phase 1–7 import/delete path (code-complete + manually verified end-to-end 2026-07-15; see the ⚠️ Real Risk Modeler wiring status note in Phase 2). While here, confirm the deferred `search_imported_rdms` field names (`rdmName`/`rdmId`) and resolve the RDM-entity-id consistency item (`irp_rdm.irp_id` should be the RDM entity id, not the apply job id) if the sandbox confirms them. 🚧 Still open (2026-07-15): `tests/irp/` holds only an empty `__init__.py` — the sandbox round-trip tests have not been authored yet, so there is nothing to run. This remains the single outstanding formal gate on the import/delete path (code-complete + manually verified end-to-end); it needs the sandbox creds + the tests written before it can be closed.
- [X] T064 [P] Guard-test that `poll_*_to_completion` appears nowhere in `app/poller/` and no `customer`/scope construct exists on any EDM/RDM/package/job (Article 11 / Article 6 / FR-041) — add to `tests/unit/test_architecture_guards.py`. ✅ Created `tests/unit/test_architecture_guards.py` (2026-07-15): (1) comment-safe regex confirms no `poll_*_to_completion` under `app/poller/`; (2) same for the sole `irp-integration` wrapper `app/services/irp_gateway.py` (T007); (3) no `customer_id`/`apply_scope`/`scoped_execute`/`user_customer_access` in the `app/` tree **or the Alembic revision** (line comments stripped so the migration's affirmative "no customer_id/scope column" note isn't a false positive) — extends `test_no_scope.py` (which does not scan `alembic/`). 3 tests, green.
- [X] T065 [P] ITCSS token audit of `app/static/css/packages.css` — no hardcoded hex, tokens layered correctly (Article 9). (`jobs.css` dropped — descoped with US6.) ✅ 2026-07-15: the only color literal was the modal scrim `rgba(15,22,41,0.4)` — promoted to a semantic `--overlay-scrim` token in `tokens.css` (TIER-2, matching the shadow scrim base) and referenced via `var(--overlay-scrim)`. `packages.css` now carries zero hardcoded hex/rgba/hsl.
- [X] T066 Run the quickstart §5 manual walkthrough (import → assemble → sync → cards → delete → libraries → closed-submission gate) and confirm the in-scope success criteria (SC-001…SC-014 except the descoped SC-003 notifications and SC-008 Jobs-list filter). ✅ Manually validated end-to-end by the approver (2026-07-15).
