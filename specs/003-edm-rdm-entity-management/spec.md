...
- **D2 — Enumeration** is via local `irp_analysis` rows, captured at `import_rdm` completion by a `backfill_rdm_analyses` worker calling `search_analyses('sourceRdmName="…"')` *(the `AND exposureName="…"` half was dropped when D3 was reversed — one RDM means one import, so the RDM name identifies its analyses on its own)*; delete drives off that table. This **reverses** the earlier "no local analysis tracking" scope note — `irp_analysis` (+ its status kind) is now created this iteration (analysis **counts still render empty**, D5).
- **D3 — Review-only / RDM-only packages are DEFERRED to follow-up.** 0.2.0's `submit_rdm_import_job` requires a target EDM, so standalone-RDM import needs a library change. **This supersedes the review-only language elsewhere in this spec (FR-002, FR-016, US2, US3, SC-004, Edge Cases):** every package this iteration has ≥1 EDM, every RDM apply targets an EDM, and Save-and-Sync rejects an RDM-only package.
  - **REVERSED 2026-08-11 (issue #22 follow-up).** The library change landed in `irp-integration` 0.4.0 (`submit_rdm_import_job(exposure_set_name=…)`), and the reason for targeting an EDM at all — tying analysis results back to portfolios — does not hold for broker data. **Every** RDM now imports standalone, no RDM apply targets an EDM, a package of any shape syncs, and the EDM↔RDM association is package membership alone. D3 and the review-only language it superseded are both history; `docs/DATA_MODEL.md` §4/§5/§6/§8 owns the current rules.
