...
| SQL Server | `make test-sql` (CI runs it on every push) | `usp_load_elt_result` behavior, `execute_procedure` autocommit, `upload_parquet` into `stage`, migration shape, the `db/bootstrap/changes/` scripts converging on `loss_schema.sql` |
