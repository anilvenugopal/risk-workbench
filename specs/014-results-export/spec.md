...
3. **Historical classification is read from CIC's historical event lookup**, never inferred from analysis settings. A model version the lookup does not carry loads every event as stochastic; the detail page's historical row count of 0 is the signal (P-24).
...
7. **Given** an analysis whose model version has no rows in the historical event lookup, **When** its load runs, **Then** it loads with every event stochastic and a historical row count of 0 (P-24).
...
- **FR-006**: Submitting an export records, per analysis: requester email, request time, the submission it was requested from (P-16), analysis identifiers and name, perspective, client, treaty inception, treaty year, CRM ID, data name, data vintage, currency, model vendor, the Risk Modeler web UI origin, and the Workbench database name (P-23). The recorded values are what gets loaded; nothing is recomputed later.
...
- **FR-009**: Loss table type, engine type, and model version are read from the archive and recorded on the export for that analysis. Model version is stored as the whole number the repository's lookup holds (`25`), never the engine label (`RL25`) and never the build number.
