...
7. **Given** an export whose chosen model version has no rows in the historical event lookup, **When** its load runs, **Then** it loads with every event stochastic and a historical row count of 0 (P-24).
...
5. **Given** an analysis exported for GR and again for RL, **Then** the exports section shows two rows, one per perspective, and the analyses grid row for that analysis is unchanged.
...
- **FR-004**: An analysis that has an export for the chosen perspective, in any status, is warned about with the newest such export's date, requester, and status, a link to it, and how many there are; the analyst can export anyway and the repeat writes a second data set under a new data ID (P-17).
