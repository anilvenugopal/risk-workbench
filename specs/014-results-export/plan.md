...
| O-14 | Whether `Data.DataModelVersion` should come from the analysis's engine version rather than the archive's `ModelVersion`, and whether `db/bootstrap/seed/lookup_rms_historical_rds.csv` should be rebuilt from CIC's own lookup export instead of Moody's `EVENT` reference data (design session 9/11 D9, D10) | On hold | Nothing changes: the stage worker keeps reading `metadata.csv` `ModelVersion` and the seed stays as T-30 built it. Ben reviews the recording |
| O-13 | Whether CIC's `Lookup_RMS_HistoricalRDS` holds the `EventID` of each storm's `HIST` row or of its `STOC` twin. Analyses report the `STOC` id (`NOTNAMED, 06/25/1851` is `2847001` in the ELT, `2896402` in the `HIST` rows the dev seed was built from, R17), so the classification join matches nothing on dev data | Open | The seed and the join are unchanged; every loaded analysis classifies as stochastic |
