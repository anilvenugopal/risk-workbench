...
**Ready for tasks:** Yes, for development against the local loss-repository mirror.
**Blocked by:** O-05 for end-to-end verification against CIC's repository: CIC's load of the five tables into `CRE_Trial_ELT_Repository` on the shared server failed, so the repository is empty until they load it again, and the Workbench login and grants are not yet created. Development is not blocked (note 25 D9): the dev mirror holds the same five tables. T-23 (the export→download path) is unvalidated until the IRP sandbox spike in quickstart.md runs.
...
- Client comes from `dbo.Client` over `LOSS`; treaty inception and CRM ID default from `submission.inception_date` and `submission_crm_id`. The form refuses an analysis whose `irp_app_analysis_id` is not an integer or whose `settings_metadata` has no `perilCode` (FR-005).
...
| T-04 | Classification is a SQL join to `dbo.Lookup_RMS_HistoricalRDS` in the repository on event ID, model version, and peril (peril term held back pending O-11); absence means stochastic; one match per event asserted first | Approved | [research.md#R4](research.md#r4--historical-classification-is-a-join-to-cics-lookup-with-a-one-match-assertion-first-t-04-t-16) |
...
| T-16 | A multi-peril group (`perilCode` `YY`) joins the lookup on event ID and model version only | Approved | [research.md#R4](research.md#r4--historical-classification-is-a-join-to-cics-lookup-with-a-one-match-assertion-first-t-04-t-16) |
...
| O-03 | Purge schedule for staged loss rows, defined and run by CIC (Nagi). Decided 2026-09-09: the Workbench deletes nothing; manifest and file rows are permanent (the manifest is the duplicate block, T-09); any purge touches `rwb_loss_result_elt_data` only | Open | Nothing is deleted (FR-024) |
| O-04 | Live lookup `Peril`/`[PCS#]` widths vs `varchar(5)` targets; `Data.Perspective` holds the code; the workflow tool's `DataInforce` form | Open | Copy without truncation (overflow fails the load); ISO `yyyy-mm-dd`; queries in quickstart.md |
...
| O-10 | Which share, its mount point on the VM, and the service account's write grant (Ross, Randy). `Data.ArchiveFile` stays unpopulated (decided 2026-09-09); the detail page shows the path | Open | `EXPORT_ARCHIVE_DIR` under the `rwb-data` volume in dev; production value is the mount |
| O-11 | Lookup `Peril` values are Risk Modeler codes; repeated `EventID` within one `ModelVersion` | Open | Peril term off (procedure flag `@use_peril = 0` default); one-match assertion on; the dev seed holds Risk Modeler codes (R16) |
| O-12 | Which CIC DBA runs `loss_schema.sql` (a `db_owner` login) and reviews it. Mechanism decided 2026-09-09: the idempotent repo file is installed by the DBA before each Workbench release that changes it; the Workbench never runs DDL against CIC | Open | File authored for unchanged install; grants listed in contracts/load-procedure.md |
...
Reviewed against all 13 articles in `.specify/memory/constitution.md`: **one
deviation**, Article 3, justified below (re-checked after Phase 1 design —
unchanged).

- **Article 3 (kind tables) — deviation**: `stage_status`, `load_status`,
  `loss_table_type`, `engine_type`, `event_type` on the stage tables are
  `CHECK`-constrained `VARCHAR` columns, not kind tables, because the tables
  live in the client's repository (T-19, Complexity Tracking). Workbench-side
  categoricals follow the article: the three job types and the
  `result_export` context type are seed rows.
...
├── templates/partials/exports_section.html           # NEW: exports table
...
|---|---|---|
| Article 3: `CHECK` constraints instead of kind tables on five stage columns (T-19) | The tables live in CIC's repository and are written by three workers and one procedure; the CIC DBA installs the file | Kind tables in `stage` would be five more tables seeded on every CIC install and joined on every load; kind tables in the Workbench database cannot be referenced by a foreign key from another database, so the constraint would be advisory |
...
- **Unit** (`uv run pytest tests/unit`, SQLite for `WORKBENCH` and a second
  ...
  non-integer `irp_app_analysis_id` and missing `perilCode` refused);
...
- **SQL Server integration** (`make test-sql`; unverified until someone runs
  ...
  correction, `YY` join, `RMS_HistoricalRDS` conversions (`TreatyYear`,
