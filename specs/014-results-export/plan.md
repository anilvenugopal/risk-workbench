...
| T-30 | Dev seeds: `dbo.Client` gets made-up rows; `dbo.Lookup_RMS_HistoricalRDS` gets CIC's own export of that table (`cic-reference/Lookup_RMS_HistoricalRDS.xlsx`, 2,589 rows, all `ModelVersion` `25`), converted verbatim by `infra/scripts/convert_lookup_export.py` into the committed `db/bootstrap/seed/lookup_rms_historical_rds.csv` and loaded by `bootstrap_loss.py` | Approved | User 2026-09-14; [research.md#R16](research.md#r16--the-dev-lookup-seed-is-cics-own-lookup_rms_historicalrds-export-t-30) |
...
├── scripts/convert_lookup_export.py   # NEW: CIC's lookup xlsx → seed CSV, every row verbatim
