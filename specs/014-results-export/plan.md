...
- Client comes from `dbo.Client` over `LOSS`; treaty inception and CRM ID default from `submission.inception_date` and `submission_crm_id`; the model version `<select>` lists `export_service.model_version_choices()`, the lookup's distinct `ModelVersion` values newest first, and defaults to the first (T-35). The form refuses an analysis whose `irp_app_analysis_id` is not an integer (FR-005).
- **POST** re-validates, generates `export_id`, inserts one `stage.rwb_loss_result_manifest` row per analysis, each carrying `requested_from_submission_id` (T-32), in one `LOSS` transaction (a repeat export of an analysis and perspective inserts its own rows beside the earlier export's, T-09), then enqueues one `submit_results_export` `rwb_job` (`requestor_type analyst_request`, `requestor_id export_id`) through `rwb_job_service.enqueue_rwb_job`, dispatches it, and redirects to the submission page's exports table (`#submission-exports`). The manifest rows are the approved plan; nothing is recomputed later (T-03, T-12).
...
- **`stage_results_export`** resumes from the manifest (loaded → nothing; staged → enqueue the load only; else discard partial stage rows and restart). It fails the analysis when the `export` job is not `FINISHED` or `EXPORT_ARCHIVE_DIR` / `EXPORT_STAGING_DIR` is missing, downloads through `irp_gateway.download_export_results` into `{EXPORT_ARCHIVE_DIR}/{export_id}/{irp_analysis_id}/` unless `zip_file` is already there, unzips into `{EXPORT_STAGING_DIR}/{export_id}/{irp_analysis_id}/`, checks `metadata.csv` `AnlsId` and `AnalysisCurrency` against the manifest, records `loss_table_type` and `engine_type`, writes one `stage.rwb_loss_result_file` row per Parquet file under the export's perspective folder, streams each with `db.elt.upload_parquet(schema="stage", connection="LOSS")` into `stage.rwb_loss_result_elt_data`, marks `staged`, deletes the working directory, and enqueues `load_results_export` with `ensure_pending_rwb_job` + `dispatch.dispatch` (T-15, T-07).
...
- **The procedure** (in `db/bootstrap/loss_schema.sql`) refuses to run inside a caller's transaction, claims the manifest row as the first statement of its own transaction, asserts every event matches at most one lookup row, classifies `event_type` by the lookup join on event ID and model version, applies the two corrections with counts, inserts `dbo.Data` capturing `DataID`, inserts `dbo.RMSELT` and `dbo.RMS_HistoricalRDS`, sets `loaded` and commits; `CATCH` rolls back, writes `failed`, rethrows (T-05, T-17, T-04).
- **Submission page** gains one exports table below the analyses (`partials/exports_section.html`): `export_service.list_export_rows` returns one `ExportAnalysisDetail` per manifest row requested from that submission, joined to `dbo.Client`, newest export first, each carrying its export's ordinal and columns (data-model.md §7); the section renders one `.drow-static` per row with the ordinal in the first column and a heavier rule where the export changes. Per row: a status derived from the manifest, last change time, `data_id`, the AAL read from `irp_analysis.loss_results` at render time, the five counts, the error, Retry and Close. Four `<select>`s (status, client, CRM ID, perspective) filter the rows and combine; their values ride on every poll, Retry, and Close URL; a Copy table button serialises the visible rows with headers (P-21, P-22). Another submission that reaches an exported analysis shows it as exported on its form, linked to that submission's exports table (T-32). Manifest reads on the request path carry the dialect-aware `READUNCOMMITTED` hint because RCSI is off on the repository (T-25).
- **Retry** (`POST …/exports/{export_id}/analyses/{irp_analysis_id}/retry`) re-arms exactly one job by the manifest state: staged → load; archive present or `export` job `FINISHED` under seven days → stage; else reset the row and re-arm submit (T-28). Crash recovery is the existing heartbeat reconciler; every job resumes from the manifest. **Close** (`…/close`) stamps `closed_at` and `closed_by` on the manifest row instead, runs no job, and derives the `closed` status ahead of every other (P-22). Both actions share the refusal (`ExportActionRefused`) and render the exports section back.
- **DDL and bootstrap, built first (T-31)**: `db/bootstrap/loss_schema.sql` (stage schema, tables, procedure; what the CIC DBA installs) and `db/bootstrap/loss_dev_mirror.sql` (CIC's five tables, `dbo.Lookup_RMS_HistoricalRDS` included, dev only); new `infra/scripts/bootstrap_loss.py` and make target `bootstrap-loss` apply both files to `rwb_loss`, then seed `dbo.Client` with made-up rows and `dbo.Lookup_RMS_HistoricalRDS` from `db/bootstrap/seed/lookup_rms_historical_rds.csv`, the 2,589 rows of CIC's own `Lookup_RMS_HistoricalRDS` export (T-22, T-21, T-30). The procedure reads the lookup by its two-part name `dbo.Lookup_RMS_HistoricalRDS`; nothing in the file names a database (T-29).
...
| Database (loss repository) | New `stage` schema: `rwb_loss_result_manifest`, `rwb_loss_result_file`, `rwb_loss_result_elt_data`, `rwb_loss_schema_version` (stamped by a fresh install and by each script in `db/bootstrap/changes/`), procedure `usp_load_elt_result`. `rwb_loss_result_plt_data` designed, not built. Dev mirror of CIC's five tables. |
...
| T-09 | A repeat export of an analysis and perspective is allowed and writes its own `Data` row. The form warns from a non-unique index on (`irp_app_analysis_id`, `perspective_code`), naming the newest earlier export and counting them; the procedure's claim on the manifest row (T-17) is what stops one row loading twice. Replaces the unique index of 2026-09-09 | Approved | Spec P-17, 2026-09-14; [research.md#R2](research.md#r2--one-data-set-per-analysis-per-perspective-enforced-by-a-unique-index-t-09) |
...
| T-30 | Dev seeds: `dbo.Client` gets made-up rows; `dbo.Lookup_RMS_HistoricalRDS` gets CIC's own export of that table (`cic-reference/Lookup_RMS_HistoricalRDS.xlsx`, 2,589 rows, all `ModelVersion` `25`), saved by hand as the committed `db/bootstrap/seed/lookup_rms_historical_rds.csv` with only the `PCS#` `NULL` cells blanked, and loaded by `bootstrap_loss.py` | Approved | User 2026-09-14; [research.md#R16](research.md#r16--the-dev-lookup-seed-is-cics-own-lookup_rms_historicalrds-export-t-30) |
...
| T-33 | The stage worker stores the whole number of the archive's `ModelVersion` | Superseded 2026-09-16 | By T-35: the export chooses the version and the worker no longer reads the archive's |
| T-35 | `data_model_version` is written at submit from the form's model version, one of `SELECT DISTINCT ModelVersion FROM dbo.Lookup_RMS_HistoricalRDS` read over `LOSS` at form time, sorted numeric-descending in Python (the SQLite unit tier has no `TRY_CAST`) with any non-numeric form after, the first as the default. No environment variable: an env list could offer a version the lookup lacks. The stage worker stamps `loss_table_type` and `engine_type` only | Approved | User 2026-09-16; [research.md#R19](research.md#r19--the-export-chooses-the-model-version-options-come-from-the-lookup-d11d14) |
| T-34 | The procedure zero-pads a numeric lookup `[PCS#]` of one to three digits to four when it copies it into `RMS_HistoricalRDS.PCS` (`137` → `0137`); `NULL`, `N/A`, and four or more digits copy unchanged. The seed stays a verbatim copy of CIC's table | Approved | User 2026-09-14; [research.md#R16](research.md#r16--the-dev-lookup-seed-is-cics-own-lookup_rms_historicalrds-export-t-30) |
| T-32 | The manifest carries `requested_from_submission_id`; the exports table and Retry filter on it, and the form's exported mark links to `/submissions/{requested_from_submission_id}#submission-exports`. `irp_job.requested_from_submission_id` alone cannot list an export whose Risk Modeler jobs were never created | Approved | Spec P-16, 2026-09-10; [research.md#clarifications](research.md#clarifications) |

## Open items carried from the design

| ID | Question | Status | What is built meanwhile |
|---|---|---|---|
| O-01 | Whether CIC ever reloads an analysis and perspective, and who clears the manifest row | Closed 2026-09-14 | Nothing to clear: CIC reloads by exporting again and renaming the data set (spec P-17) |
...
| O-03 | Purge schedule for staged loss rows, defined and run by CIC (Nagi). Decided 2026-09-09: the Workbench deletes nothing after a stage completes; manifest and file rows are permanent (the manifest is the export record, T-03); any purge touches `rwb_loss_result_elt_data` only | Deferred | Nothing is deleted (FR-024) |
| O-04 | `Data.Perspective` holds the code; the workflow tool's `DataInforce` form (the width half closed 2026-09-14: CIC's lookup has `Peril` at 2 characters and `[PCS#]` at up to 4, R16) | Deferred | Copy without truncation (overflow fails the load); ISO `yyyy-mm-dd`; queries in quickstart.md |
| O-05 | When CIC's load of `dbo.Client`, `dbo.Data`, `dbo.RMSELT`, `dbo.RMS_HistoricalRDS`, and `dbo.Lookup_RMS_HistoricalRDS` into `CRE_Trial_ELT_Repository` completes (the 2026-09-09 check found the database empty: 5 MB data, no `dbo.Data`, RCSI off, because that load failed); the `LOSS` login and its grants; the Workbench database name on the same server | Open | Local mirror; blocks CIC verification only. The Workbench database name needs no code when it is chosen: `create_export` reads `MSSQL_WORKBENCH_DATABASE` into `Data.Database` (P-23) |
...
**Filesystem**: `EXPORT_ARCHIVE_DIR` (permanent archives; a share mount in production, a directory under the `rwb-data` volume in development) and `EXPORT_STAGING_DIR` (transient extraction under `{export_id}/{irp_analysis_id}/`; local disk on the machine the workers run on). Both are created by hand and neither is created by the app.
...
├── cic-reference/           # CIC DDL and the Lookup_RMS_HistoricalRDS export (xlsx)
...
└── bootstrap/                         # NEW directory
    ...
    ├── changes/001-engine-type-varchar5.sql   # NEW: one re-runnable script per altered table (O-12)
...
├── services/export_service.py         # NEW: form model, duplicate check, manifest insert, exports table rows, retry
...
├── routers/submissions.py             # exports/new GET+POST, perspective fragment, exports section, retry, close
├── nav/manifest.py                    # submissions.export_new (hidden)
├── templates/partials/analyses_merged_section.html   # Export button
├── templates/partials/exports_section.html           # NEW: the exports table
...
└── static/js/app.js                                  # tableToTsv copies .drow-static rows
tests/
...
- **Unit** (`uv run pytest tests/unit`, SQLite for `WORKBENCH` and a second
  ...
  stamp with `execute_procedure` faked; status derivation for the exports
...
- **SQL Server integration** (`make test-sql`; CI runs it on every push):
  ...
  shape for `irp_job.export_id` and the seed changes; a fresh
  `loss_schema.sql` install and the same install plus every
  `db/bootstrap/changes/` script end at the same columns, check constraints,
  and stamped version.
