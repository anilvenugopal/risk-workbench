...
| `data_model_version` | `metadata.csv` `ModelVersion`, written by the stage worker as the whole number of a numeric value: `25` (DLM example, from `25.0` or `25.0.2450.0`) or `HDv2.1` unchanged (HD example). CIC's lookup holds `25` (plan T-33). Null until staged. `NVARCHAR(10)`, the type of both `Data.DataModelVersion` and the lookup's `ModelVersion`, so the join compares like with like. | `Data.DataModelVersion`; lookup join and lookup-loaded assertion (§4.4 steps 1–2). |
...
| `DataModelVendor` | `RMS`. |
| `DataModelVersion` | `manifest.data_model_version`: `25` for the DLM example, the whole number CIC's lookup holds (plan T-33). Never `RL 25` and never the build number. The HD example carries `HDv2.1`, which fits `nvarchar(10)` but is not numeric; whether CIC accepts it is in O-08. |
...
| `Loss` | `elt_data.loss`. |
| `PCS` | Lookup `[PCS#]`; a numeric value of one to three digits is zero-padded to four (`137` → `0137`), since a PCS code is four digits downstream and CIC's lookup stores 27 of its 142 codes as three (plan T-34). `NULL`, `N/A`, and four or more digits copy unchanged. |
| `Perspective` | `manifest.perspective_code`. |
| `AReLossSet` | Not populated. |
...
load fails rather than storing a cut value. CIC's lookup export (2026-09-14,
plan research R16) has `Peril` at 2 characters and `[PCS#]` at up to 4, so
both fit.
...
| T-20 | Type conversions happen once, at a named place: `irp_app_analysis_id` becomes `INT` on submit (the form rejects a non-integer); the procedure writes `TreatyYear` with `CONVERT(varchar(4))` and `DataInforce` as ISO `yyyy-mm-dd` (`CONVERT(varchar(15), ..., 23)`); `data_model_version` and `peril_code` are `NVARCHAR(10)` to match the lookup; the stage worker keeps the whole number of a numeric `ModelVersion` (plan T-33, 9/14). `PCS` and `Peril` are copied without truncation so an overflow fails the load; a numeric `PCS` of one to three digits is zero-padded to four (plan T-34, 9/14). | Approved | User, 9/9 |
...
| O-04 | Confirm `Data.Perspective` holds the perspective code, and confirm the form the workflow tool writes into `RMS_HistoricalRDS.DataInforce` (the procedure writes ISO `yyyy-mm-dd`, §4.4). The width check closed 9/14: CIC's lookup export has `Peril` at 2 characters and `[PCS#]` at up to 4. | Ben |
...
| O-11 | Closed 9/14 against CIC's lookup export (plan research R16): `Peril` is `HU`/`EQ`/`WT`, not Risk Modeler's `WS`, and the join has no peril term (plan R4). One `EventID` repeats within `ModelVersion` `25`: `15000012`, both rows `EQ`, which the one-match assertion (50003) refuses to load (plan O-15). | Ben, Cheng |
