...
3. **Given** an expanded analysis row, **Then** the left column names the source (the full analysis name on an own row, the RDM on a broker row, plus the analysis id) and lists the Metadata group of O-11, and the right column renders OEP and AEP losses at the condensed set (FR-005) together, followed by AAL and standard deviation, with a perspective toggle and no condensed/expanded display toggle.
