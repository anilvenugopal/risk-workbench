...
- The expanded analysis row in two columns: metadata and settings on the left in two named groups (O-11), the condensed results on the right — one EP type at a time, perspective and EP-type toggles in the row, no display toggle.
...
| O-11 | Expanded-row field set, one **Metadata** group in this order: engine version, analysis type, subperil, framework, event rate scheme, unrecognized construction and occupancy, run by. The source line carries the full analysis name on own rows and the RDM name on broker rows. Peril and analysis template read from the merged table's Peril, Portfolio and Template columns; currency, min loss threshold and franchise deductible are dropped, as are Construction, Line of business, Term and Loss amplification (PLA) — to be re-added as the team asks for them. Engine type and Region move to the merged table's Engine and Region columns. | Approved | 8/26 design session D4/D10 — `docs/design_session_notes/20_results_views_demoed_grid_columns_name_split_ep_type_selection_export_first_requirements.md` |
...
3. **Given** an expanded analysis row, **Then** the left column names the source (the full analysis name on an own row, the RDM on a broker row, plus the analysis id) and lists the Metadata group of O-11, and the right column renders the selected EP type's losses at the condensed set (FR-005), followed by AAL and standard deviation, which do not change with that selection, with perspective and EP-type toggles and no condensed/expanded display toggle.
...
- **FR-011**: Expanding an analysis row shows two columns: on the left the source (portfolio or RDM, and the analysis id) followed by the O-11 field groups; on the right the condensed results — the selected EP type, then AAL and standard deviation, which sit outside that selection; a perspective toggle and an EP-type toggle in the row, no condensed/expanded display toggle. The two columns stack when the row is too narrow to hold both.
