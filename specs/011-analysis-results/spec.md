...
## Status
**Phase:** Draft | **Blocking:** nothing — O-01 closed 2026-08-25 (REST endpoints, live response captures in `research.md#R3`).
...
- One merged analyses table on the submission Results section and the EDM detail page: own and broker analyses together, origin derived from `rdm_id`, broker rows grouped under the RDM that produced them, with `currency` and `AAL` columns added.
- Condensed results inline in the expanded analysis row: both EP types at once, perspective toggle, no display toggle.
...
- Copy table with headers, a ones/thousands/millions units selector, and user-controlled left-to-right ordering.
...
| O-02 | Broker exposure pointer: the retrieval worker passes RM's own reported pointer — stored `exposure_resource_id` for broker rows (one `get_analysis_by_id` re-read when NULL), `irp_portfolio.irp_id` for own rows. Verified in the IRP-sandbox tier. | Approved (plan T-03) | `research.md#R2` |
...
The analyst runs a suite (spec 010) and waits for an analysis to finish. Without doing anything else, its loss numbers appear: AAL, standard deviation, and OEP/AEP losses at the fixed return periods, switchable across the five perspectives, on the analysis detail.

**Acceptance**

1. **Given** an executed analysis that reaches FINISHED, **When** retrieval completes, **Then** its AAL, standard deviation, and OEP/AEP losses at the 11 stored return periods are readable on the analysis detail for each perspective the analysis produced.
...
The analyst opens the analyses table on the submission Results section or the EDM detail page. Own and broker analyses appear in one table: own rows directly (identifiable by the `CRE_` prefix and no RDM), broker rows grouped under the RDM that produced them, expandable. Each row shows currency and AAL; expanding a row shows the condensed results inline — both EP types at the 6 condensed return periods — with a perspective toggle. The analyst copies the table with headers into Excel.
...
2. **Given** a ready analysis row, **Then** it shows its currency (captured at analysis-detail backfill) and its AAL in the selected perspective; no return-period column appears at row level.
3. **Given** an expanded analysis row, **Then** OEP and AEP losses at the condensed set (50 / 100 / 250 / 500 / 1000 / 10000) render together, with a perspective toggle and no condensed/expanded display toggle.
4. **Given** a rendered results table, **When** the analyst uses copy, **Then** the table lands in the clipboard with headers, pasteable into Excel.
5. **Given** displayed loss values, **Then** units never switch automatically; a ones/thousands/millions selector controls them (millions default).
...
4. **Given** more analyses selected than the ~10 guideline, **Then** selection is not blocked and the table scrolls horizontally; the full list and drill-down of every analysis stays reachable.
...
- **FR-010**: The merged table carries `currency` and `AAL` columns; no return-period column; the AAL-only display mode is dropped. Currency is captured when the analysis detail is backfilled.
- **FR-011**: Expanding an analysis row shows the condensed results inline: both EP types at once, a perspective toggle, no condensed/expanded display toggle.
- **FR-012**: Every results view offers perspective switching across GR, RL, WX, QS, GU; Gross is the default; the selection applies to the whole screen, not per analysis.
...
- **FR-015**: The dedicated page defaults to the expanded return-period set with both EP types, one column per analysis. Selection count is not hard-blocked: past the ~10 guideline (O-09) the table scrolls horizontally, and the full listing and drill-down of every analysis remains reachable.
- **FR-016**: The analyst controls the left-to-right order of analyses (and, later, groups) on the dedicated page.
- **FR-017**: Displayed units never switch automatically; a ones/thousands/millions selector controls them.
...
- **SC-004**: Every analysis of a submission is reachable — no listing cap; parity with RiskLink's full list.
