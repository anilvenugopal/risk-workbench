...
| O-11 | Expanded-row field set, no group heading, in this order: analysis type, subperil, framework, event rate scheme, unrecognized construction and occupancy, run by. The source line leads with the full analysis name on own rows and the RDM name on broker rows, at a larger size than the rest of the line. Peril and analysis template read from the merged table's Peril, Portfolio and Template columns; currency, min loss threshold and franchise deductible are dropped, as are Construction, Line of business, Term and Loss amplification (PLA) — to be re-added as the team asks for them. Engine type, engine version and Region read from the merged table's Engine and Region columns, since D3 forbids restating a collapsed value here. | Approved | 8/26 design session D3/D4/D10 — `docs/design_session_notes/20_results_views_demoed_grid_columns_name_split_ep_type_selection_export_first_requirements.md` |
