...
| O-11 | Expanded-row field set, no group heading, in this order: analysis type, subperil, framework, event rate scheme, run by. The source line leads with the full analysis name on own rows and the RDM name on broker rows, at a larger size than the rest of the line. Peril and analysis template read from the merged table's Peril, Portfolio and Template columns; unrecognized construction and occupancy, currency, min loss threshold and franchise deductible are dropped, as are Construction, Line of business, Term and Loss amplification (PLA) — to be re-added as the team asks for them. Engine type, engine version and Region read from the merged table's Engine and Region columns, since D3 forbids restating a collapsed value here. | Approved | 8/26 design session D3/D4/D10, amended 2026-09-17 |
...
3. **Given** an expanded analysis row, **Then** the left column names the source (the full analysis name on an own row, the RDM on a broker row, plus the analysis id) and lists the O-11 fields, and the right column renders the selected EP type's losses at the condensed set (FR-005), followed by AAL and standard deviation, which do not change with that selection, with perspective and EP-type toggles and no condensed/expanded display toggle.
...
- **FR-011**: Expanding an analysis row shows two columns: on the left the source (portfolio or RDM, and the analysis id) followed by the O-11 fields; on the right the condensed results — the selected EP type, then AAL and standard deviation, which sit outside that selection; a perspective toggle and an EP-type toggle in the row, no condensed/expanded display toggle. The two columns stack when the row is too narrow to hold both.
