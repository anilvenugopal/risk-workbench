...
- The expanded analysis row in two columns: the analysis name and the metadata fields on the left (O-11), the condensed results on the right — one EP type at a time, perspective and EP-type toggles in the row, no display toggle.
...
| O-11 | Expanded-row field set, no group heading, on one line in this order: analysis type, subperil, framework, run by; the full-width lists follow (members, regions & rates, treaties; spec 015). The source line is the same on every row: the full analysis name, at a larger size than the rest of the line, then the analysis id, the Submitted date (FR-024) and, on a broker row, the RDM name. Peril and analysis template read from the merged table's Peril, Portfolio and Template columns; unrecognized construction and occupancy, currency, min loss threshold and franchise deductible are dropped, as are Construction, Line of business, Term and Loss amplification (PLA) — to be re-added as the team asks for them. Engine type, engine version and Region read from the merged table's Engine and Region columns, since D3 forbids restating a collapsed value here. | Approved | 8/26 design session D3/D4/D10, amended 2026-09-17 and 2026-09-23 |
| O-12 | The merged table carries no perspective control and no units control: AAL reads Pre-Cat Net in millions, the perspective toggle sits inside the expanded row, and the units selector belongs to the dedicated results page where analyses sit side by side. | Approved | same preview |
...
The analyst opens the analyses table on the submission Results section or the EDM detail page. Own and broker analyses appear in one table: own rows directly (identifiable by the `CRE_` prefix and no RDM), broker rows grouped under the RDM that produced them, expandable. Each row shows currency and AAL; expanding a row shows how the analysis was run on the left — metadata and analysis settings in two named groups — and the condensed results on the right, the selected EP type at the 6 condensed return periods, with perspective and EP-type toggles in the row. The analyst copies the table with headers into Excel.
...
2. **Given** a ready analysis row, **Then** it shows its currency (captured at analysis-detail backfill) and its Pre-Cat Net AAL in millions; no return-period column and no perspective or units control appear at table level.
3. **Given** an expanded analysis row, **Then** the left column names the source (the full analysis name, the analysis id, the Submitted date, and the RDM on a broker row) and lists the O-11 fields, and the right column renders the selected EP type's losses at the condensed set (FR-005), followed by AAL and standard deviation, which do not change with that selection, with perspective and EP-type toggles and no condensed/expanded display toggle.
...
The analyst multi-selects analyses — from the submission page (which alone can reach cross-EDM analyses) or from the EDM detail page — and chooses View. A dedicated results page opens in a new browser tab: expanded view by default (all 11 return periods), one EP type at a time, one column per analysis, with perspective and EP-type dropdowns applying to the whole page. The browser tab is titled with the submission or EDM name; breadcrumbs lead back to where the analyst came from.

**Acceptance**

1. **Given** analyses selected on either the submission page or the EDM detail page, **When** the analyst chooses View, **Then** the dedicated page opens in a new browser tab with one column per selected analysis, expanded return periods, one column per analysis for the selected EP type.
...
- **FR-011**: Expanding an analysis row shows two columns: on the left the source (the analysis name, the analysis id, the Submitted date, and the RDM on a broker row) followed by the O-11 fields; on the right the condensed results — the selected EP type, then AAL and standard deviation, which sit outside that selection; a perspective toggle and an EP-type toggle in the row, no condensed/expanded display toggle. The two columns stack when the row is too narrow to hold both.
- **FR-012**: Every results view offers perspective switching across GR, RL, WX, QS, GU, with Pre-Cat Net (`RL`) the default; the merged table's AAL column reads Pre-Cat Net. On the dedicated page the selection applies to the whole screen; in the merged table it applies to the expanded row that holds it.
...
- **FR-015**: The dedicated page defaults to the expanded return-period set for OEP, plus AAL and standard deviation, one column per analysis; the EP-type selection applies screen-wide and never changes AAL or standard deviation. Selection count is not hard-blocked: past the ~10 guideline (O-09) the table scrolls horizontally.
...
- **FR-022**: The expanded row lists every O-11 field for both origins. Run by reads as not returned on a broker analysis rather than being hidden.
