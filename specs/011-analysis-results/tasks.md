...
**Building the UI**: two previews are approved —
`docs/ui_previews/results_ep_table.html` (EP table) and
`docs/ui_previews/merged_analyses_table.html` (merged table, two-column expanded row,
results states, section summary line, empty states). **They are guidance, not markup to
paste.** Build against the components and CSS that already exist — `.dtable` in
...
- [ ] T001 Confirm `get_stats` / `get_ep` signatures and row shapes against the active wheel (`make irp-status`; irp-integration 0.6.0rc2 `AnalysisManager`) before writing the gateway — the wheel is pre-release and moves (contracts/irp-gateway.md)
- [ ] T002 [T-02] Track the one external dependency: [irp-integration#28](https://github.com/premiumiq/irp-integration/issues/28) widens `PERSPECTIVE_CODES` to the full RM vocabulary. App work proceeds against FakeIRP; before the IRP sandbox tier can pass for WX/QS, switch to a build carrying the change (`make irp-testpypi` or `make irp-local`)
...
UI reference: the expanded row in tile 1 of `docs/ui_previews/merged_analyses_table.html` and the EP table in `docs/ui_previews/results_ep_table.html` — both approved, both guidance (see **Building the UI** above).

### Implementation for User Story 1

- [ ] T010 [US1] [FR-003] [FR-004] [FR-005] [FR-021] [T-04] [O-03] Extract builder in `app/workers/analysis_jobs.py`: named constants for the 11 stored return periods and the 6-point condensed subset (data-model.md §4); pure function building the contracts/loss-results.md document from the verbatim stats/EP row lists — `aal` = `purePremium`, `std_dev` = `totalStdDev`, epType filter keeps OEP/AEP and discards TCE-OEP/TCE-AEP (O-04), exact-match lookup of the 11 points in `value.returnPeriods`/`value.positionValues`, all five perspective keys always present with `null` for an unproduced perspective, `engine_type`/`engine_version` snapshotted from the analysis metadata with absent fields stored as `null`
...
- [ ] T019 [US1] [T-02] [T-08] IRP sandbox test in `tests/irp/`: one own FINISHED analysis — stats + EP for all five perspectives (WX/QS acceptance needs the T002 wheel build) and the T-08 empty-perspective shape (empty list, not an error)
  - Proof: `make shell` → `uv run pytest tests/irp --run-irp` (developer-run; WX/QS blocked until irp-integration#28 ships)
...
- [ ] T036 [P] [US4] [T-07] [FR-014] [FR-016] Unit tests in `tests/unit/test_shell_routes.py` and `tests/unit/test_nav_manifest.py`: hidden nav node, column order follows `ids`, perspective param, both breadcrumb variants, tab title carries the entity name, absent-id notice, pending column renders
...
- [ ] T038 Run the quickstart.md walkthrough end-to-end (developer-run stack) and report tiers by name and count — unit tier count, SQL Server tier (`make test-sql`), IRP sandbox tier (`--run-irp`, WX/QS only after irp-integration#28)
...
- **US4 (Phase 6)** depends on US3's merged section (T027) for the View button and on T013 for read models. T032 → T033 → T034; T035 last.
...
(dedicated page). The IRP sandbox tier for WX/QS stays blocked on irp-integration#28
(T002) and never blocks app work — FakeIRP accepts all five perspectives from T008 on.
