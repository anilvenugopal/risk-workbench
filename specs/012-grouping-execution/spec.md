...
- Inspection of the selected members with an explicit event-rate-scheme choice for each conflicting peril/region/model-version partition; PLT/ELT output classification; simulation periods from Risk Modeler's fixed list, for the group and for every partition of a PLT group (O-11)
...
| O-11 | Simulation periods are chosen from Risk Modeler's fixed list — 3,125, 6,250, 12,500, 25,000, 50,000, 100,000, 200,000, 400,000, 800,000 — with 50,000 preselected regardless of the members' PLT lengths, twice over for a PLT group: once per partition on the Inspection screen (every row, HD partitions included, whether or not a simulation set choice is pending) and once for the group on the Settings screen, labelled Group simulation periods. No member-length hint | Approved | note 26 D18; research.md Clarifications 2026-09-04; decided 2026-09-04 |
...
6. **Given** two or more finished analyses that ran in one currency, share their event-rate schemes, need no simulation set choice, and have no blocking problem, **When** the analyst presses Finish on the Members screen, **Then** the group is submitted with that currency, the env scheme and vintage, Propagate ON, and — for a PLT group — 50,000 simulation periods for the group and every partition, and the dialog is replaced by a confirmation pane naming the group, its settings, and its members, which stays until Close. With a scheme conflict, a pending simulation set choice, a blocked inspection, or differing or unknown currencies, Finish lands on the Inspection screen with a notice, nothing is submitted, and Next continues as usual.
...
- **FR-019**: For a PLT group the analyst chooses the simulation periods from Risk Modeler's fixed list (3,125, 6,250, 12,500, 25,000, 50,000, 100,000, 200,000, 400,000, 800,000) with 50,000 preselected, for every peril/region/model-version partition on the Inspection screen — HD partitions included, independent of any simulation set choice — and for the group (the target group PLT length, labelled Group simulation periods) on the Settings screen. For an ELT group the dialog shows neither choice and submits 1. A submit with any other value is blocked (O-06, O-11).
