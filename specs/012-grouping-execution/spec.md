...
6. **Given** two or more finished analyses that ran in one currency, share their event-rate schemes, need no simulation set choice, and have no blocking problem, **When** the analyst presses Finish on the Members screen, **Then** the group is submitted with that currency, the env scheme and vintage, Propagate ON, and — for a PLT group — 50,000 simulation periods for the group, and the dialog closes with the toast and the group row appearing in the grid. With a scheme conflict, a pending simulation set choice, a blocked inspection, or differing or unknown currencies, Finish lands on the Inspection screen with a notice, nothing is submitted, and Next continues as usual.
...
- **FR-019**: For a PLT group the analyst chooses the simulation periods from Risk Modeler's fixed list (3,125, 6,250, 12,500, 25,000, 50,000, 100,000, 200,000, 400,000, 800,000) for every peril/region/model-version partition on the Inspection screen, preselected at 100,000, and for the group (the target group PLT length, labelled Group simulation periods) on the Settings screen, preselected at 50,000. A partition needing no simulation set choice whose PLT members all ran on one PET of a known length shows that length read-only and submits no value for it, so Risk Modeler keeps the PET's own count — Risk Modeler does not let it be changed either. For an ELT group the dialog shows neither choice and submits 1. A submit with any other value is blocked (O-06, O-11).
