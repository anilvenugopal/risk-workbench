...
- [X] T012 [US1] [FR-001] [T-12] **Group** button (`data-group-analyses`) in the summary bar of `app/templates/partials/analyses_merged_section.html`, rendered only on the submission page's Results grid, enabled at ≥2 ticked rows, opening the T010 GET with the ticked ids; picks JS in `app/static/js/app.js`
...
- [X] T026 [P] Reconcile `docs/DATA_MODEL.md` with the landed schema: `irp_analysis.submission_id` + the three-leg origin CHECK + `uq_irp_analysis_live_submission_name`, the `irp_analysis_group_member` table, the `submit_grouping` job type
...
- [X] T028 Diff subtraction review per AGENTS.md: remove comments/tests restating the implementation, inline single-use helpers, remove speculative branches, compare diff size with requirement size
...
- [X] T046 Dropped: the 26 `GroupingProblemCode` messages irp-integration emits are already analyst-readable sentences carrying the analysis ids, and `describe_problem` is shared with the worker's `failure_reason`, so a Workbench-side text table would duplicate them. The grep targets are dead: `group-inspect-indicator` has no hits in `app/`, and `Inspect members` survives only as a negative assertion in `tests/unit/test_grouping_routes.py`
- [X] T048 [FR-021] Explicit simulation-set selection on irp-integration `0.8.0rc7`: `SimulationSetChoice` and the two `PartitionRow` fields in `grouping_view.py`; the Simulation set column and `select[name=simulation_set_selection]` in `group_inspection.html`; the Alpine gate; `simulation_set_selections` through `request_grouping`, the plan, `submit_grouping` (gateway, worker, FakeIRP) as `SimulationSetSelection`; the sandbox-shaped `tests/unit/grouping_inspections.py` fixture (HD JP/WS on PET 15, DLM NA/EQ with five sets, nested group NA/WS on 738/739) and its view, route, and worker tests; spec.md FR-021, contracts, data-model.md, quickstart.md §3, research.md 2026-09-03 session
- [X] T049 [FR-021] The HD partition's own simulation set on irp-integration `0.8.0rc8` (`GroupingRegionFact.pet_name` from `PETMetadata.petName`, `FINGERPRINT_VERSION` 5): `ObservedPet` and `PartitionRow.observed_pets` in `grouping_view.py`; `InspectionScreen.simulation_sets_required` replaced by `simulation_sets_shown` (the column follows the PLT group output, not the presence of a choice); the resolved PET rendering in `group_inspection.html`; `pet_name` through the `grouping_inspections.py` and `FakeIRP` fixtures; view and route tests; spec.md FR-021, contracts/routes.md, plan.md, quickstart.md §3, research.md 2026-09-03 HD PET names session
...
- [X] T058 [FR-019] irp-integration `simulation_periods_selections` on `GroupingManager.submit` (`SimulationPeriodsSelection`, the three `simulation_periods_selection_*` problem codes, the `regionPerilSimulationSet[].simulationPeriods` override with the PET / `defaultPeriods` fallback; tests, README, `docs/api.md`) — commit `8b92512` on `feature/grouping-inspect-submit`; the `0.8.0rc9` tag and TestPyPI publish are the developer's step
- [X] T059 [FR-019] [FR-021] [O-11] Per-partition simulation periods: the Simulation periods column and `select[name=simulation_periods_selection]` on every row of a PLT group in `group_inspection.html`; `simulation_periods_selections` through `request_grouping` (parsed, every value on the fixed list), the plan, the worker, `irp_gateway.submit_grouping` (as `SimulationPeriodsSelection`), and FakeIRP; the "Group simulation periods" header above `#group-sims` with the largest-member hint and `GroupingInspectionView.largest_member_periods` removed; `setsChosen` / `periodsChosen` beside `schemesChosen` in `app.js` and the three review rows in `group_compose_modal.html`; `.insp-table__periods` widths; service, route, view, and worker tests
- [X] T060 [FR-025] [O-14] Finish on a PLT group: `finish_blockers` no longer stops on PLT output; the finish route posts `DEFAULT_SIMULATION_PERIODS` for the group and `default_simulation_periods_selections(view)` per partition; the Simulation periods row in `group_finish_confirmation.html`; route and service tests; spec.md O-11, O-14, FR-019, FR-025, US-1 acceptance 6, US-2 acceptance 2; contracts/routes.md, contracts/grouping-worker.md, data-model.md, plan.md, quickstart.md §3 steps 4 and 9, research.md, FUNCTIONAL_REQUIREMENTS.md §6
- [ ] T061 Click-through per quickstart.md §3 steps 4 and 9 on a stack running irp-integration `0.8.0rc11` or the local checkout (`make irp-local`), then the diff subtraction review
- [X] T062 [FR-025] [O-14] Finish loses its confirmation pane (note 27 D8): `POST …/analyses/group/finish` answers 204 with the submit's `HX-Trigger` and no retarget headers, the Finish button in `group_compose_modal.html` clears `#group-modal` on a 204, `partials/group_finish_confirmation.html` and `grouping_service.requested_group_name` are deleted; route and service tests; spec.md FR-025 + O-14 + US-1 acceptance 6, contracts/routes.md finish section, plan.md, quickstart.md §3 step 9, research.md, FUNCTIONAL_REQUIREMENTS.md §6
- [X] T063 [FR-004] [O-10] The currency hint reads as a decision, not background (note 27 D11/D12): `wf-field__hint--warn` in `submissions.css` and the modifier on `[data-currency-hint]` in `group_inspection.html` in the two defaulting cases only; route tests; contracts/routes.md, quickstart.md §3 step 9, research.md
- [X] T064 [FR-007] [FR-021] Text-filter the scheme and simulation-set dropdowns (note 27 D17): the `.ta` + `selectSearch()` + `forms.typeahead_controls` wrapper on both selects in `group_inspection.html`, and `overflow-x` off `.insp-table-wrap` in `submissions.css` so `.modal-body` scrolls the wide table and the menu is not clipped; route tests; contracts/routes.md, quickstart.md §3 step 4, research.md
- [X] T065 [FR-019] [O-11] Two simulation-period defaults (note 27 D19): `DEFAULT_SIMULATION_PERIODS` splits into `DEFAULT_GROUP_SIMULATION_PERIODS` (50,000) and `DEFAULT_PARTITION_SIMULATION_PERIODS` (100,000) in `grouping_service.py`, both in `_inspection_context`, one per select in `group_inspection.html`; service and route tests; spec.md FR-019 + O-11, contracts/routes.md, quickstart.md §3 step 4, research.md, FUNCTIONAL_REQUIREMENTS.md §6
- [X] T066 [FR-019] [O-11] A PET-bound partition's periods are read-only (note 27 D20): `partition_facts` and `fixed_simulation_periods` in `grouping_service.py`, `PartitionRow.fixed_simulation_periods` in `grouping_view.py`, the `.insp-resolved` cell with no input in `group_inspection.html`, and `default_simulation_periods_selections` skipping those partitions so the submit keeps the PET's count; service, view, and route tests; spec.md FR-019 + O-11, contracts/routes.md, contracts/grouping-worker.md, quickstart.md §3 steps 4 and 9, research.md, FUNCTIONAL_REQUIREMENTS.md §6
- [ ] T067 Click-through per quickstart.md §3 steps 4 and 9 for the note-27 changes (Finish closes with no pane, the amber currency hint, the two filterable dropdowns with an unclipped menu on the bottom row, 100,000 per partition against 50,000 for the group, the read-only HD periods cell), then the diff subtraction review
