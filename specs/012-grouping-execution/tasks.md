...
- [X] T019 [P] [US1] [FR-011] [FR-012] [T-11] Unit tests in `tests/unit/`: poller `grouping` routing and terminal handling (FINISHED → backfill enqueue, FAILED reason extraction); `backfill_analysis_detail` group branch (name-only resolution success, ambiguous/zero hits fail the job)
