...
### Session 2026-09-05 (design note 27 — the 9/4 CIC session)

Source: design note 27 (the 2026-09-04 CIC session — D8, D11/D12, D17, D19,
D20) and the approver's decisions of 2026-09-05. The build CIC saw was the
one this branch already had, so each entry below revises a decision taken the
day before.

- Q: Finish landed on a confirmation pane. CIC wants it gone (D8) — what tells the analyst the inspection passed? → A: Nothing new. Finish answers 204 with the same `HX-Trigger` the Group submit sends, the button clears `#group-modal`, and the toast plus the group row arriving with live status are the confirmation. Wendy: *"when I hit finish, I just wanted to finish"*; Cheryl agreed on her own reasoning — she checks which analyses went in by opening the group row, which she would do anyway. Rejected: rewording the toast to say "Inspection passed" and naming the group in it — both were offered and neither is what the pane was for; the analyst reads the outcome off the row. `requested_group_name` went with the pane, its only caller.
- Q: With the currency prefilled, a currency-only mismatch stops the analyst on a screen where nothing looks wrong. Fix the routing? → A: No — CIC wants the pause. Wendy: *"anytime you've got mixed currencies, you got to pause."* The only change is the `[data-currency-hint]` line, which was grey (`--text-tertiary`) and read as background: it takes `wf-field__hint--warn` (`--color-warning`) in the two defaulting cases and stays grey where every member agrees. That is the colour `.insp-choose .wf-field__hint` already uses for the scheme and simulation-set prompts, so one colour means "decide this" on both screens (Wendy: *"I like the color of that wording to be the same on each page"*). **Note the premise this rests on and its weakness:** Cheryl accepted the invisible stop by deducing there are only two stop reasons, scheme and currency. `finish_blockers` has five, and a treaty mismatch — which Ben counts among his three — does not stop Finish at all today, so the deduction happens to hold. If a treaty stop is ever added, the notice must name the reason instead. Rejected: naming the reason in the notice now (`finish_blockers` already computes the strings) — it is a change CIC did not ask for and did not want.
- Q: One partition offered 34 simulation sets. Pare the list? → A: Not in v1. Ben: *"a hard requirement for grouping immediately is going to be text filter this list"*, with the paring and the defaults treated as follow-ups. The scheme and simulation-set selects gain the existing `selectSearch()` enhancement — the `.ta` container plus `forms.typeahead_controls`, exactly as the currency block uses it — and nothing new is written. The nine-value simulation-periods select stays plain. Rejected: a bespoke filter for the table.
- Q: `.ta__menu` is absolutely positioned and `.insp-table-wrap` had `overflow-x: auto`, which clips on both axes. → A: The wrap keeps its border and loses its overflow; the wide table scrolls sideways in `.modal-body`, which is already the scroll box the currency block's typeahead opens in on screen 3, so a menu behaves the same on both screens. Rejected: leaving the wrap scrolling and accepting a menu cut off at the table's edge.
- Q: The per-partition simulation periods preselected 50,000 (D19). → A: 100,000, on Cheryl's *"most of the stuff that I'm getting is probably 100,000 years."* The group-level value stays 50,000, mirroring Risk Modeler. `DEFAULT_SIMULATION_PERIODS` splits into `DEFAULT_GROUP_SIMULATION_PERIODS` and `DEFAULT_PARTITION_SIMULATION_PERIODS` — two business rules that must not drift back together.
- Q: Cheryl caught that the Workbench lets an HD partition's simulation period be edited where Risk Modeler does not (D20). Bind it to what? → A: To the PET's own period count, by **posting no `SimulationPeriodsSelection` for that partition** — the package's documented default for a partition without one is the member PET's period count, so the correct implementation is an omission, not a number computed here. The row shows the count read-only. Ben: *"this is definitely defaulted to 50,000, so it should be equivalent"* — the current agreement between the two 50,000s is coincidence, which is the reason not to implement by observing them equal. The rule needs `simulation_set_selection_required` in it and not just "has a PET": a partition holding both an ELT and a PLT member requires a set choice and its conversion length is still the analyst's. Rejected: computing the count and posting it (duplicates the package's own rule and would drift from it); dropping the cell entirely (the analyst cannot then see what the partition runs at).
- Q: What if the partition's PETs disagree on a period count, or report none? → A: Keep the editable dropdown at the partition default rather than guess. Neither case is reachable from the sandbox fixtures, and a wrong read-only number is worse than a choice.
