...
- [ ] T001 Point the repo at the editable `irp-integration` checkout with `make irp-local` and record the resolved version from `make irp-status`
- [ ] T002 [T-02] Re-confirm the six consumed signatures against the **active** wheel — `search_accounts_by_portfolio_paginated`, `search_policies_paginated`, `search_locations_paginated`, `create_portfolio`, `manage_portfolio_accounts`, `search_portfolios` — against [contracts/irp-library.md](contracts/irp-library.md); note any drift in [research.md](research.md) before writing gateway code
  - Proof: the signature list in contracts/irp-library.md matches `python -c "import inspect, irp_integration..."` output for each method
- [ ] T003 Record the DB lifecycle choice for this schema-affecting iteration — **Rebuild** recommended per [data-model.md](data-model.md) §8; `make db-rebuild` runs at T010, after the DDL and seeds land
...
- [ ] T004 [P] [T-04] Create `breakout_dimension_kind` (`code` PK NVARCHAR(32), `label` NVARCHAR(128) NOT NULL, `sort_order` INT NOT NULL) in `alembic/versions/0001_initial.py`, **before** `irp_portfolio` for FK ordering and dropped after it in `downgrade()`; seed `lob` / `state`
...
- [ ] T006 [FR-011] [T-04] Add the idempotency index to `alembic/versions/0001_initial.py`: `CREATE UNIQUE NONCLUSTERED INDEX uq_irp_portfolio_breakout ON irp_portfolio (source_portfolio_id, breakout_dimension_code, breakout_value) WHERE source_portfolio_id IS NOT NULL AND deleted_at IS NULL`
- [ ] T007 [P] [T-03] Seed `run_breakout_lob` and `run_breakout_state` into `rwb_job_type_kind` in `alembic/versions/0001_initial.py`. `rwb_job_requestor_type_kind` needs **no** new row — the breakout enqueues under the already-seeded `analyst_request` code (T023)
- [ ] T008 [T-04] Add idempotent MERGE blocks for `breakout_dimension_kind` (2 rows) and the two `rwb_job_type_kind` rows to `infra/scripts/seed_db.py`
- [ ] T009 [P] [T-04] Extend `tests/sqlserver/test_detail_tables_migration.py`: the three lineage columns, the self-FK, `breakout_dimension_kind`, and the filtered unique index are built; a second live generated portfolio on the same (source, dimension, value) is rejected; a soft-deleted one does not block re-creation
  - Proof: `pytest tests/sqlserver --run-sqlserver` green
- [ ] T010 Run `make db-rebuild` and confirm the seeds landed (`breakout_dimension_kind` has 2 rows, `rwb_job_type_kind` has the 2 new codes)
...
- [ ] T012 [P] [FR-009] Unit-test the integrity rule and the race-duplicate path in `tests/unit/test_portfolio_lineage.py`

### Summary extension — dimension-agnostic keys (data-model.md §5, research R11)

- [ ] T013 [P] [FR-007] [P-13] New `sql/databridge/portfolio_account_total.sql` returning each portfolio's account total — the overlap denominator; read-only, worker-side
- [ ] T014 [FR-002a] Capture the source portfolio's Risk Modeler `stampDate` into `exposure_detail` alongside the summary in `_backfill_edm_detail_body` (`app/workers/package_jobs.py`), read **before** the DataBridge read so the stored stamp is conservative
- [ ] T015 [FR-005] [P-13] Extend the summary builder in `app/services/irp_gateway.py` to write `account_total` and the `breakout_values` container keyed by `breakout_dimension_kind.code`; readers parse defensively, as the spec-004 readers do
- [ ] T016 [P] [FR-005] [FR-002a] Extend `tests/unit/test_backfill_edm_detail.py`: the stored snapshot carries `stampDate`, `account_total`, and a `breakout_values` dict; a snapshot written by the pre-iteration builder still parses
...
- [ ] T018 [P] [FR-003] Gate truth table in `tests/unit/test_breakout_gate.py`: EDM status × deleted × `breakout_values` present/absent/malformed × 0/1/2+ values × in-flight `run_breakout_*` × in-flight `backfill_edm_detail`, including a pre-iteration summary (has `states`, no `breakout_values`) reading as absent
...
- [ ] T020 [FR-007] [P-13] Add `Overlap` and `compute_overlap(values, account_total)` to `app/services/breakout_service.py`: `summed` = Σ accounts over the dimension's values, `repeats` = `summed − account_total` floored at 0 (`None` when the total is absent), `partition` = `repeats == 0`
...
- [ ] T033 [US1] [FR-013] On completion including partial success, `_run_breakout_body` idempotently enqueues `backfill_edm_detail` for the EDM and records `backfill_enqueued`; the job succeeds when ≥ 1 entry created/adopted/skipped and fails only when zero succeeded
...
- [ ] T045 [US2] [T-01] [P-12] Add the **state** branch to `select_breakout_accounts` in `app/services/irp_gateway.py`: one `search_locations_paginated` call per value filtered `accountId IN (…) AND admin1Code = "<value>"`, the account id read from `row["location"]["property"]["accountId"]`, same composed-filter chunking as the LOB branch. `admin1Name` is never a filter input — it returns zero rows with HTTP 200 until the EDM is geocoded
...
- [ ] T049 [P] [US2] [FR-004] [FR-005] [P-12] Extend `tests/unit/test_irp_gateway.py`, `tests/unit/test_run_breakout_worker.py`, and `tests/unit/test_breakout_routes.py` for the state dimension: location response-shape parsing against a recorded body, per-value read isolation, a non-US division enumerating with no separate mode, an un-geocoded EDM producing values with null labels, and the geography disclosure copy
