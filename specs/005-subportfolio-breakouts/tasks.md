...
- [X] T007 [P] [T-03] Seed the `run_breakout_*` codes into `rwb_job_type_kind` in `alembic/versions/0001_initial.py` — one per dimension. Quick breakouts enqueue under the already-seeded `analyst_request` requestor type (T023); T083 added the `breakout_group` requestor type for group jobs
...
- [X] T009 [P] [T-04] Extend `tests/sqlserver/test_detail_tables_migration.py`: the four lineage columns, the self-FK, `breakout_dimension_kind`, `breakout_group`, and both unique indexes are built; a second live generated portfolio on the same (source, dimension, value) is rejected; a soft-deleted one does not block re-creation
  - **Unverified**: the tests are written; the SQL Server tier has never been run on this branch. `make test-sql` or `make wsl-test-sql` is the proof, and it is the developer's to run
- [ ] T010 Run `make db-rebuild` and confirm the seeds landed (`breakout_dimension_kind` 5 rows, `rwb_job_type_kind` 5 new codes, `rwb_job_requestor_type_kind` + `breakout_group`). The dev `rwb_workbench` is still at a pre-005 schema (T077/T078/T086/T094)
...
- [X] T043 [US2] [FR-006c] [FR-007] Add two states to `docs/ui_previews/breakout_modal.html`: the geography preview with the measured overlap statement and the explicit multi-state-account consequence, and a 40+ value fan-out rendered untruncated with the FR-006c queue-occupancy statement. Approved before the template edit — both states shipped with the T026 preview (its scaffold note reads "T026 + T043") and were covered by the 2026-08-04 approval

### Implementation for User Story 2

- [X] T044 [US2] [FR-005] [P-12] Rewrite `sql/databridge/portfolio_states.sql` to return `Admin1Code`, `MAX(Admin1Name)`, and an account count, grouped and filtered on the **code** — the `COALESCE(Admin1Name, Admin1Code)` goes. Fill `breakout_values["state"]` (`value` = `Admin1Code`, `label` = `Admin1Name` or null, `accounts`) and switch the summary's `states` list to codes, in `app/services/irp_gateway.py`
- [X] T045 [US2] [T-01] [P-12] Add the **state** selection script `sql/databridge/breakout_state_accounts.sql` — `(Value, AccountId)` pairs with `Value` = `Admin1Code`, mirroring the rewritten `portfolio_states.sql` joins so the vocabulary matches the summary (R1 as revised 2026-08-05) — and register it under `state` in `_SELECTION_SCRIPTS` in `app/services/irp_gateway.py`. `Admin1Name` is never a filter input (P-12)
- [X] T046 [US2] [FR-004] Add the `run_breakout_state` actor to `app/workers/portfolio_jobs.py`, sharing `_run_breakout_body`
- [X] T047 [US2] [FR-007] [FR-006c] Confirm the untruncated preview list and the FR-006c statement render for a 40+ value fan-out. The geography-specific disclosure this task added was **deleted by P-21** (2026-08-09 — one short line per disclosure)
- [X] T048 [US2] [P-12] Render state **codes** in the `states` column of `app/templates/partials/portfolio_row.html`; pre-change snapshots keep showing names until the next Sync and both must render
...
- [X] T050 [US3] [FR-014] Extend `list_portfolios(edm_id)` in `app/services/portfolio_service.py` with a LEFT JOIN on `source_portfolio_id`, a join to `breakout_dimension_kind`, and a LEFT JOIN to `breakout_group` (adding `breakout_group_label` and `breakout_group_filters`), adding `source_name`, `breakout_dimension_code`, the dimension `label`, and `breakout_value` to each row; ordering is `inserted_at, name` (new sub-portfolios at the bottom; name breaks ties within one backfill) — grouping and indent are display concerns
- [X] T051 [US3] [FR-014] Render the lineage `↳ from {source_name} · {dimension label}: {value}` on generated rows in `app/templates/partials/portfolio_row.html`, showing the **immediate source only** for chained lineage; broker-arrived rows unchanged. Add the style to `app/static/css/details.css` via tokens. **P-27 moved it off the collapsed row** into the expanded panel (2026-08-11)
- [X] T052 [US3] [FR-015] [P-08] Add the business-event log lines: `"breakout %s requested for portfolio %s by analyst %s (n_sub_portfolios=%d)"` in `breakout_service.request_breakout`, and per-sub-portfolio created/adopted/failed plus the completion summary in `app/workers/portfolio_jobs.py`, carrying the actor id from `input_data`
- [X] T053 [US3] [FR-015] [P-08] Confirm every audited field is recoverable with no audit-log table: actor from `input_data.actor_id` and the log line, timestamp from the job row, source portfolio from `requestor_id`, dimension from `rwb_job_type`, outcomes from `output_data.sub_portfolios`, and the confirming analyst on each generated row's `inserted_by`
  - Proof: a single worker test reads all six back after a partial-failure run
- [X] T054 [P] [US3] [FR-014] [FR-015] Extend `tests/unit/test_snapshot_upsert.py` and `tests/unit/test_edm_detail_rollup.py` for the lineage-aware list read model (generated rows with NULL `exposure_detail` render the pending state; chained lineage shows the immediate source), and add the breakout log assertions to `tests/unit/test_business_event_logs.py`
...
- [X] T060 [T-02] Re-pin `irp-integration` to the published build once PR #21 lands on TestPyPI: `make irp-testpypi`, confirm with `make irp-status`, and run `make test` against it — pinned at T001 (0.3.0 from TestPyPI, confirmed installed); full unit tier green against it (798 passed, 2026-08-05)
- [X] T061 [T-01] [T-07] [SC-003] *(authored 2026-08-05; the tier last ran green against the sandbox 2026-08-10 — 9 passed in 78s, including the write-visibility assertion: the DataBridge member count saw the REST add on the first read, no lag observed → W-20's open question closed. That run **predates** T090/T092/T094/T095, so it does not cover the country, peril, or custom-group paths)* New opt-in `tests/irp/test_breakout.py`: the real select → create → add round-trip through the gateway against the sandbox, re-verifying selection tokens, chunking, idempotent re-add returning `completed 0`, the 40/20-character name and number limits, and whole-account bucketing. Add one large-fan-out case — a state breakout over 40+ first-level administrative divisions — asserting every entry lands with an outcome and none is refused for the size (SC-003)
  - **Assert the DataBridge read-back's write visibility first.** `test_create_add_readback_chunking_and_idempotent_readd` must show `portfolio_member_count.sql` returning the full count on the **first** read after `manage_portfolio_accounts`, with no retry and no sleep. This is the one step of the composition nothing has proved: probe-findings.md's read-back equality (W-1) was measured against the paginated REST enumeration on 220- and 209-account selections, and W-20 moved the read-back to a DataBridge scalar on 2026-08-05 without re-proving that the EDM database reflects the PATCH immediately. If it lags, every entry fails with its sub-portfolio already created in Risk Modeler — and the failure looks like a short add, not like a stale read. If the sandbox shows lag, record the observed delay in probe-findings.md and add a bounded re-read to `_member_count` (worker-side, so no constitution conflict); do not add one speculatively
  - Proof: `pytest tests/irp --run-irp` green
- [X] T062 [FR-016] Confirm nothing outside this iteration's scope was added: no portfolio edit, delete, or merge; no rollback path; everything else on the EDM page stays read-only as shipped in spec 004 — confirmed 2026-08-05, when the only routes added were the breakout GET/POST in `app/routers/portfolios.py`. **Re-confirm needed**: `name-check`, `group-preview`, and `groups` shipped after it, and Article 11 was amended to v3.2.0 for `count_breakout_match`
- [ ] T063 [SC-003] *(pending the approver's run against the live stack + sandbox; also re-verifies the US1 W-20 checkpoint)* Run the [quickstart.md](quickstart.md) manual walkthrough end to end, including the gate states, the idempotent re-run, and the partial-failure banner. Record the wall-clock from confirm to the sub-portfolios appearing in the list for a ≤ 15-value breakout and check it against SC-003's 30-second bound
- [X] T064 [P-12] [FR-010] [FR-014] *(P-12 revision 2026-08-05)* Name and display by label, never the numeric code: `build_breakout_plan` composes the name token from `label or value` (the value keeps the filter, `breakout_value`, and number-token roles), the worker's RM description appends the label beside the value, `portfolio_service.list_portfolios` resolves each generated row's `breakout_value_label` from the source portfolio's stored summary, and `portfolio_row.html` renders labels in the lineage badge and the States column — unit-tested in `test_breakout_plan.py`, `test_snapshot_upsert.py`, `test_portfolio_lineage.py`, `test_run_breakout_worker.py`, `test_breakout_routes.py`
- [X] T065 [T-11] [FR-012] *(2026-08-05)* Poll the Portfolios section during a breakout instead of the whole EDM detail body — `#edm-detail` is the page's scrolling element, so the every-3s `outerHTML` swap scrolled the analyst back to the top for the whole run. Split the section into `app/templates/partials/edm_portfolios_section.html` with its own poll, add `GET /edms/{edm_id}/portfolios-section` (`app/routers/edms.py`) returning that section plus an out-of-band swap of the new `edm_detail_meta.html`, drop `breakout_running` from the body's `live` condition, restore the body poll's populated-mid-sync 204 short-circuit unconditionally, and record every `<details id>` open state plus the scroll offset across any swap in `app/static/js/app.js`. Route tests in `tests/unit/test_edm_sync.py`
- [X] T066 [FR-007] [P-13] [SC-002] *(review follow-up 2026-08-05)* Measure the overlap instead of deriving it. New `sql/databridge/portfolio_state_coverage.sql` / `portfolio_lob_coverage.sql` count, per portfolio and per account, how many accounts carry at least one value of the dimension and how many carry more than one; `irp_gateway._COVERAGE_SCRIPTS` runs them inside `get_edm_exposure_summary` and writes `summary.breakout_coverage` (data-model §5). `compute_overlap` reads those two counts and returns `covered`/`uncovered`/`repeats`, with `partition` requiring no repeats **and** no uncovered accounts; absent coverage falls back to the qualitative disclosure. `breakout_modal.html` states the coverage shortfall as a number where it is known. Replaces the derived `Σ per-value counts − account_total` (research.md R11). Tests in `test_breakout_plan.py`, `test_breakout_routes.py`, `test_irp_gateway.py`
- [X] T067 [FR-012] *(review follow-up 2026-08-05)* Put the worker's per-entry guard around the whole entry, lineage write included: `_run_breakout_body`'s loop wraps `_execute_entry`, and the duplicate-name branch calls `_adopt_entry` after its handler returns rather than inside it (an exception raised in a handler is not caught by that handler's siblings). Before this, `insert_generated`/`adopt_generated` raising on a lineage conflict aborted the job after sub-portfolios existed in Risk Modeler, with no `output_data`, no per-entry error lines, and no `backfill_edm_detail`. Also fold the four identical failure logs into `_failed`, read `adopt_generated`'s return so a lost UNIQUE race reports `skipped_existing`, drop the redundant `edm_id` parameter, and use `_common._uid`. Test: `test_a_failing_lineage_write_fails_only_that_entry`
- [X] T068 [FR-012] *(review follow-up 2026-08-05)* Cover `page_state` — the FR-012 durable-failure and banner derivation had no test at all and reported zero coverage across its whole loop body. New `tests/unit/test_breakout_page_state.py`; in the function itself, hoist `_count` out of the loop (dropping its `# noqa: B023`), select the banner on "no banner yet" rather than the raw row index, drop the dedup the `rwb_job` uniqueness makes unreachable, and order the `filling_in` follow-up read like its `edm_service` sibling
- [X] T069 [FR-011] *(review follow-up 2026-08-05)* Make the dimension vocabulary fail loudly instead of guessing: `_dimension_letter` raises on a code absent from `_DIMENSION_LETTER` (a derived letter would silently change the `portfolio_number` scheme adoption resolves on, and two codes sharing a first letter would collide), `DimensionEligibility` carries the noun the gate already resolved so the modal reads it instead of looking it up again without a fallback, and `test_architecture_guards.py` asserts every seeded `breakout_dimension_kind` code has a letter, a noun, a selection script, and a coverage script
- [X] T070 *(review follow-up 2026-08-05)* Smaller correctness and duplication: `BreakoutGate` carries the two rows it read (`rows_live`, `edm_irp_id`, `source_name`, `source_irp_id`, `stored_stamp`) so `modal_context` and `request_breakout` stop re-reading them — four SELECTs become two, eight become four on a refused confirm, and the confirm can no longer 500 on a portfolio deleted mid-request; `_portfolio_name_taken` uses `search_portfolios_paginated` like every other exposure-scoped lookup; a member-count read returning no rows raises instead of counting zero; the read-back failure reason says which way the count missed and that an over-populated adoption needs the extra accounts removed in Risk Modeler; `fetch_portfolio_stamp` takes `exposure_irp_id` as a string like its four siblings; `_write_generated` stops rebinding `source`/`value` over the row and parameter of those names
- [X] T071 [T-11] *(review follow-up 2026-08-05)* Key `app.js`'s swap-state record by the swap target (a `WeakMap`) instead of one module-level slot: the section poll's three swaps and an overlapping modal GET shared one record, so the first `afterSettle` consumed whichever record was written last and the rest restored nothing. Also treat a recorded scroll offset of `0` as an offset rather than as absent

---

## Phase 7: Follow-on — CIC demo feedback (plan: `docs/pm/2026-08-09_spec005_followon_plan.md`)

Custom grouping (FR-018–021), peril as a grouping dimension (P-19), the prune →
re-breakout bug fix (T-16), and disclosure shortening (P-21). Decision IDs
continue from the tables in spec.md/plan.md.

### Workstream 1 — reclaim soft-deleted lineage rows (T-16)

- [X] T072 [T-16] [FR-011] `_snapshot_prune` gains `name_resurrect_filter` (portfolio caller passes `AND source_portfolio_id IS NULL`; treaty caller unchanged); `_UPDATE_BY_NAME` gains `AND deleted_at IS NULL`; `_write_generated` resolves against rows including soft-deleted — (edm, irp_id) first, lineage triple second — and reclaims a dead match in place (`_RECLAIM_GENERATED_BY_ID`); the unique-violation recovery re-resolves the same way
- [X] T073 [T-16] Regression tests: `tests/unit/test_breakout_prune_rerun.py` (the full demo sequence: breakout → prune → re-breakout reclaims → prune again stays healthy; resurrect-by-name skips generated rows, irp_id match still revives) and the reclaim / conflicting-dead-row / dead-irp-revive cases in `tests/unit/test_portfolio_lineage.py`
  - Proof: `uv run pytest tests/unit` green (928 passed, 2026-08-09)

### Workstream 2 — disclosure shortening (P-21, D11)

- [X] T074 [P-21] [FR-007] Replace the overlap + blank-value prose in `app/templates/partials/breakout_modal.html` with the two short quantified lines (qualitative one-liners when coverage is absent); mirror in `docs/ui_previews/breakout_modal.html`; update copy assertions in `tests/unit/test_breakout_routes.py`. No preview round — copy tweak (UI_WORKFLOW rule 1 carve-out)

### Workstream 3 — peril as a grouping dimension (P-19)

- [X] T075 [O-01] [O-02] Probe `loccvg.PERIL` vocabulary + label source in the sandbox EDMs (read-only, DataBridge executor); findings → probe-findings.md W-21 + research.md R14 — numeric smallint codes, no blanks, no in-EDM lookup, one peril per sandbox portfolio; O-01 closed, O-02 rides the grouping-modal preview
- [X] T076 [P-19] New `sql/databridge/portfolio_perils.sql`, `portfolio_peril_coverage.sql`, `breakout_peril_accounts.sql` (join path `portacct → Property (LOCID) → loccvg`); `peril` registered in `_SELECTION_SCRIPTS` / `_COVERAGE_SCRIPTS`; summary-builder leg writes `breakout_values["peril"]` (stringified codes, label null); `_DIMENSION_NOUN["peril"]` + `DimensionEligibility.quick` (False for peril) — no `_DIMENSION_LETTER` entry, no job type, no actor
  - Proof: `tests/irp/test_breakout.py::test_selection_vocabulary_matches_the_stored_summary` green against the sandbox with peril included (2026-08-10)
- [X] T077 [P-19] Seed `breakout_dimension_kind ('peril','Peril',30)` in `0001_initial.py` + `seed_db.py` + the unit mirror; quick-mode chooser filters to `d.quick`; architecture guard reworked to per-dimension requirements (lob/state: full letter/noun/scripts/job-type/actor lockstep; peril: noun + scripts, letter and job type asserted ABSENT). SQL Server seed assertion updated but **unverified** — the dev `rwb_workbench` is at a pre-005 schema and needs the developer's `make db-rebuild`

### Workstream 4 — custom grouping cart (FR-018–021)

- [X] T078 [T-12] Schema: `breakout_group` table (UUID PK = the job's `requestor_id`, `group_key`, `label`, `filters` JSON, approved `name`/`number`, `cart_id`, UNIQUE(source_portfolio_id, group_key)); `irp_portfolio` += `breakout_group_id`; seeds `('custom','Custom group',40)`, `run_breakout_custom`, requestor type `breakout_group`; unit mirror + drift guard. Dev DB pending the developer's `make db-rebuild`
- [X] T079 [T-12] [FR-019] Group identity: canonical `group_key` hash (`compute_group_key`), `_DIMENSION_LETTER["custom"] = "G"`, name token = analyst label through `_compose_name`, adopt-not-rename on an existing member set
- [X] T080 [T-13] [FR-018] [FR-020] `request_group_breakout` (validate against stored summary → as_of/stamp checks → per group upsert + `ensure_pending_rwb_job` keyed on the group row UUID → dispatch, shared `cart_id`); `compose_group_cart`/`load_approved_group`; one-episode rule extended over `breakout_group` (both directions); `page_state` custom flights + cart-aggregated banner
- [X] T081 [T-14] [FR-020] Worker `run_breakout_custom` (`_run_breakout_group_body`): per-dimension selection reads → union within / intersect across → empty intersection fails with a recorded reason → create/adopt via the shared `_execute_entry` machinery → lineage write with `breakout_group_id` (incl. reclaim)
- [ ] T082 [T-15] [FR-018] [FR-021] UI preview revised on `docs/ui_previews/breakout_modal.html` (§3: custom pane empty cart, 3-group cart incl. peril + overlap warning, disabled pills, in-flight; O-02 code-only peril display flagged for the reviewer). Template/routes BUILT on the session's goal directive, ahead of the informal approval the workflow normally front-runs: modal custom pane (Alpine `dim`/`n` slivers over server-rendered rows), `POST …/breakout/group-preview` (409 → `#bo-cart-error`), `POST …/breakout/groups` (same refusal mapping as the quick confirm), cart-row partial, row-badge tooltip, `details.css` styles. **Open: the approver's review** — preview §3 + O-02, then clicking the running feature (with T063); redlines fold back into the template
  - Proof: 8 new route tests in `tests/unit/test_breakout_routes.py` (custom pane render, pill gating, group-preview fragment/suffix/overlap/refusals, cart confirm success + four refusal variants, CSRF)
...
- **Polish (T055–T071)**: T055–T059 can run any time after the design settles; T060–T063 need all stories complete; T064–T071 are review follow-ups on completed work
