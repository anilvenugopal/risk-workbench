...
- [X] T020 [FR-007] [P-13] Add `Overlap` and `compute_overlap(values, account_total)` to `app/services/breakout_service.py`: `summed` = Σ accounts over the dimension's values, `repeats` = `summed − account_total` floored at 0 (`None` when the total is absent), `partition` = `repeats == 0` — **arithmetic superseded by T066**: it measures memberships against a denominator that includes accounts carrying no value, and the two errors cancel
...
- [X] T047 [US2] [FR-007] [FR-006c] Add the geography-specific disclosure to `app/templates/partials/breakout_modal.html` — the multi-state-account consequence stated explicitly — and confirm the untruncated list and the FR-006c statement render for a 40+ value fan-out
...
- [X] T051 [US3] [FR-014] Render the lineage badge `↳ from {source_name} · {dimension label}: {value}` on generated rows in `app/templates/partials/portfolio_row.html`, showing the **immediate source only** for chained lineage; broker-arrived rows unchanged. Add the badge style to `app/static/css/details.css` via tokens
...
- [X] T061 [T-01] [T-07] [SC-003] *(authored 2026-08-05; full tier green against the sandbox 2026-08-10 — 9 passed in 78s, including the write-visibility assertion: the DataBridge member count saw the REST add on the first read, no lag observed → W-20's open question closed)* New opt-in `tests/irp/test_breakout.py`: the real select → create → add round-trip through the gateway against the sandbox, re-verifying selection tokens, chunking, idempotent re-add returning `completed 0`, the 40/20-character name and number limits, and whole-account bucketing. Add one large-fan-out case — a state breakout over 40+ first-level administrative divisions — asserting every entry lands with an outcome and none is refused for the size (SC-003)
...
- [X] T062 [FR-017] Confirm nothing outside this iteration's scope was added: no portfolio edit, delete, or merge; no rollback path; everything else on the EDM page stays read-only as shipped in spec 004 — confirmed 2026-08-05: the only routes added are the breakout GET/POST in `app/routers/portfolios.py`; the US2/US3 template changes are render-only
