...
- [X] T050 [US3] [FR-014] Extend `list_portfolios(edm_id)` in `app/services/portfolio_service.py` with a LEFT JOIN on `source_portfolio_id` and a join to `breakout_dimension_kind`, adding `source_name`, `breakout_dimension_code`, the dimension `label`, and `breakout_value` to each row; ordering stays by name — grouping and indent are display concerns
