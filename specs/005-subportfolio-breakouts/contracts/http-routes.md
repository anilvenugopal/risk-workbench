...
- Renders `partials/breakout_modal.html` (Alpine open/close — the `package_modal.html` precedent; fetched into a dedicated `#breakout-modal-mount` div in `pages/edm_detail.html`, **outside the self-polling `#edm-detail` wrapper** so the 3-second poll never removes an open modal; self-removing on completion):
  ...
  - **Dimension chooser**: LOB / Geography (state) / Geography (country) / Peril, plus the **Custom groups** pane (`?mode=custom`) — each option enabled per `DimensionEligibility`; ineligible options rendered disabled with their `reason`, which names **Sync** where a missing summary is the reason). The modal posts no Sync of its own: it names the action and the analyst runs it from the page-header **Sync** button after closing the dialog. The modal renders one summary snapshot, so it could not stay open across a sync anyway, and a Sync submitted from here had no submission context to render back into — it erased the submission breadcrumbs and the EDM picker.
  ...
  - **Disclosures (FR-007)**: one short line each (P-21, 2026-08-09 — the exposure-inflation note and the geography-specific call-out are deleted). (a) account-bucketing overlap, **quantified for this portfolio** from `compute_overlap` — *"Warning: overlapping accounts — 12 of 1,701 accounts match more than one state and are included in full in each matching sub-portfolio"*, the clean-partition wording when nothing repeats, or the qualitative sentence alone when the summary carries no measured coverage. (b) blank/unassigned values: *"N of M accounts carry no {dimension} value and are left out."*
...
- Form fields: `dimension` (`lob`|`state`|`country`|`peril`), `summary_as_of` (echoed from the preview, FR-002b), `csrf_token`. HTMX + no-JS PRG both supported (the `sync()` route precedent).
- Flow: `validate_csrf_token` → `breakout_service.request_breakout`, whose five ordered steps are the gate re-check, **the FR-002b summary-unchanged check** (the stored summary's `as_of` must equal the one the preview carried), **the FR-002a summary-freshness check** (current RM `stampDate` via `search_portfolios` must equal the stamp captured at backfill), **composing and persisting the approved plan** into `input_data` — the list the worker executes rather than recomputing (FR-006a / AGENTS.md rule 8) — and the idempotent enqueue.
  - Success → returns `partials/edm_portfolios_live.html` retargeted at `#edm-portfolios` (`HX-Retarget`/`HX-Reswap: outerHTML`), carrying the breakout-in-flight indicator and its own `every 3s` trigger; `HX-Trigger` toast *"Breakout started — N sub-portfolios"*. The section is the swap unit, not the `#edm-detail` body: this route carries no submission id, so a body render would drop `source_submission` and erase the submission breadcrumbs, the EDM picker, and the Broker analyses section from the contextual page.
...
### `GET /edms/{edm_id}/portfolios/{portfolio_id}/breakout/name-check` — as-you-type group name (P-25)

- Query: `group_label`. Renders `partials/name_collision.html` with `breakout_service.check_group_name`'s verdict — the EDM's own portfolio names plus one cached Risk Modeler `portfolioName` search, which **fails open** (`checked=False`) when RM is unreachable.
...
- Form fields: `values:{dimension}` (repeated per ticked value), `group_label`, the already-carted `group` JSONs, `csrf_token`.
- `breakout_service.compose_group_preview` composes the row server-side from the stored summary — name, canonical member set, upper-bound account count (P-23) — and refuses a name already taken in the EDM, in Risk Modeler, or in the cart (P-25) and a group no account matches (P-29, the bounded single-row DataBridge count, fail-open, threadpooled off the event loop).
- Success → the `partials/breakout_cart_row.html` fragment, appended (`hx-swap beforeend`). Refusal → **409** retargeted at `#bo-cart-error`. No writes on either.

### `POST /edms/{edm_id}/portfolios/{portfolio_id}/breakout/groups` — the cart confirm (FR-018/FR-020)

- Form fields: the carted `group` JSONs, `summary_as_of`, `csrf_token`.
- `breakout_service.request_group_breakout` re-validates every posted group against the stored summary and applies the quick confirm's ordered refusals once per cart (gate re-check, FR-002b `as_of`, FR-002a stamp) — **409 + the re-rendered modal** on each, with no row written. On pass: one `breakout_group` upsert and one `run_breakout_custom` job per group, sharing one `cart_id`.
- Success → the same response as the quick confirm: `partials/edm_portfolios_live.html` retargeted at `#edm-portfolios` plus the "Breakout started" toast.
...
- **Lineage** (`portfolio_row.html`): a generated row's **expanded panel** shows its base portfolio and breakout criteria — `from {source_name} · {dimension label}: {value}`, or the group's label and filters for a custom group; chained lineage shows the immediate source only. The collapsed row carries no badge (P-27, 2026-08-11). Broker-arrived rows unchanged. The `states` column on every row now renders state codes (P-12) — pre-change snapshots keep showing names until the next Sync, and both render.
