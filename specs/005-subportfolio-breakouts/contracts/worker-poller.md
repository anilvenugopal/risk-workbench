...
3. selection = gateway.select_breakout_accounts(exposure, source, dimension,
   ...
   — hoisted out of the loop: LOB is one policy pass grouped client-side, state is one
     location read per value (R1). Per-value read failures land in errors_by_value.
   The source account-id read failing (the input to every value) → JobResult.fail;
     nothing has been written to RM at this point.
...
- **`completed < total` from the add is not a failure.** `manage_portfolio_accounts` counts ids *newly* added, so a healthy re-run reports `completed 0` (W-9). Success is decided by reading the portfolio back and comparing against the ids sent — which is what `create_sub_portfolio` returns as `account_count`.
...
- One breakout job occupies the single worker for its duration. A LOB fan-out shares one selection pass and then runs 3 RM calls per sub-portfolio (create, add, read-back); a state fan-out adds one location read per value. At ≤ 15 values with sub-second calls that stays inside the 30-second SC-003 budget; 40+ values or large selection paging runs longer and simply keeps heartbeating. Accepted at current scale (plan, Article 10 note).
...
- selection read error for a value (`IRPAPIError` from pagination, W-14): that entry fails, the rest run.
