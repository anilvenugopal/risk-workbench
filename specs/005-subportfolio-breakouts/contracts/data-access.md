...
- `test_irp_gateway.py` — the read-back comparison: a short membership and an over-populated adopted portfolio both raise, so `test_run_breakout_worker.py` sees the entry fail with no lineage row.
