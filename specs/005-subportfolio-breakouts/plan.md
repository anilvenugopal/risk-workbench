...
- `backfill_edm_detail` gains four edits: it captures the portfolio's RM `stampDate` alongside the summary (the FR-002a anchor), the two DataBridge summary scripts return a per-value account count with `Admin1Code` as the state value plus `Admin1Name` as a nullable label, a new script returns each portfolio's account total, and two more return each portfolio's per-dimension coverage — the accounts carrying at least one value and the accounts carrying more than one. The summary JSON gains `breakout_values`, `account_total`, and `breakout_coverage`; no new column. Measured cost of the first three +1.44s worker-side (R11); the coverage pair is measured at T063. The FR-007 disclosure reads those two counts and never derives them from the per-value counts ([research.md#R11](research.md), revised 2026-08-05).
- Schema, all in `0001_initial.py`: `irp_portfolio` += `source_portfolio_id` (self-FK), `breakout_dimension_code` (FK), `breakout_value`, `breakout_group_id` (FK); new `breakout_dimension_kind` (seeds `lob`, `state`, `country`, `peril`, `custom`) and `breakout_group` (T-12); `rwb_job_type_kind` seeds one `run_breakout_*` per dimension; `rwb_job_requestor_type_kind` seeds `breakout_group`; filtered unique index (source, dimension, value) — the idempotency key. Full column and constraint detail in [data-model.md](data-model.md).
...
| T-04 | Lineage is 4 nullable columns on `irp_portfolio` + `breakout_dimension_kind` + a filtered unique idempotency index; no breakout table | Approved | [research.md#R3](research.md), [data-model.md](data-model.md) |
...
| T-14 | Group selection is **app-side set algebra** over the existing per-dimension DataBridge reads: union account ids within a dimension, intersect across dimensions | Approved | [research.md#session-2026-08-09](research.md#session-2026-08-09) |
...
- **Article 10 (SQL Table Is the Queue)**: five new job types on the existing `rwb_job` queue with its atomic claim, heartbeat, and reconciler; idempotent enqueue on the `UNIQUE(requestor_type, requestor_id, rwb_job_type)` key — one type per dimension so the key distinguishes them per portfolio. A long fan-out briefly occupying the single worker is accepted at this scale.
- **Article 11 (IRP Behind the Gateway)**: the creation loop runs worker-side through `irp_gateway`; the web layer's single RM call is the confirm-time freshness read (permitted request-path submission pattern, never a `get_*` poll); both writes are synchronous, so there are no `irp_job` rows and the poller is untouched; the DataBridge summary reads stay worker-side, and the one request-path DataBridge read is the bounded single-row count v3.2.0 permits (`count_breakout_match`, fail-open); `poll_*_to_completion` appears nowhere.
