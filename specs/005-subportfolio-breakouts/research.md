...
- **Q: The group number `P{rm id}-G-{key hash}` "wasn't useful at a glance" (Ben) and had no technical necessity. Replacement?** → **A: The number is the name inside 20 characters.** The number's one job — resolving the Risk Modeler portfolio in the duplicate-name adopt path (FR-011) — survives: a portfolio created by a prior run of the same group carries the same number, because the number depends only on the name. A name that fits is the number verbatim; a longer one keeps its first 14 characters and takes 6 hex digits of `sha256(name)`, the `_compose_number` technique. Plain truncation was the 2026-08-10 decision and is **wrong**: two names sharing their first 20 characters compose one number, `_adopt_entry` resolves adoption on the number and fails an entry when `len(hits) != 1`, so the create-then-heal path is dead for both groups from the second run on (corrected 2026-08-24). `group_key` remains the group identity for row adoption (P-22), and rows approved before this session keep their stored composed name and `P…-G-…` number — the approved plan is immutable (rule 8). `_DIMENSION_LETTER` loses its `custom` entry; quick-mode numbers (`P{rm id}-{S|L}-{token}`) are unchanged. Supersedes the naming/number half of the 2026-08-09 grouping entry. → spec **P-26** (FR-019)
...
- **Q: Moody's files every Caribbean island under one country. Which field carries the country and which the island (note 12 §2, D5)?** → **A: `CountryRMSCode = 'CB'` is the country and the address's own ISO3A `CountryCode` is the state, in place of `Admin1Code`.** Measured against a CIC EDM holding Caribbean exposure (2026-08-11): every CB address carries `CountryScheme = ISO3A` with `CountryCode` = the island (`PRI`, 2,709 addresses; `VIR`, 154) and `Admin1Code` **below** island level — USVI splits into `010` St Croix, `020` St John, `030` St Thomas, while Puerto Rico is a single `200`. So the two shipped derivations both miss: the country dimension offered `PRI`/`VIR` and no whole-region value, and the state dimension offered districts where the analyst asks for islands. Under the CB branch the same portfolio enumerates one country (`CB`) and two states (`PRI`, `VIR`) — the two workflows CIC runs, the whole region or a single island. **Consequences accepted:** the USVI districts stop being separately breakout-able (9 addresses on St John is not a portfolio anyone runs), and Puerto Rico's sub-portfolio is named `cbhu - PRI` rather than `cbhu - Puerto Rico`, because the label must be NULL on the CB branch — `Admin1Name` names the district, so `MAX()` over the `VIR` group would label the whole territory "St Thomas". A code→name map for the ~30 islands was rejected: the EDM has no country-name column and no geography lookup table (`RMS_EDM.Lookup.*` is geocoding and hazard only), so the names could only be invented, which P-12 forbids. The branch is duplicated across all six geography scripts rather than centralized — the selection read must filter on a string byte-identical to the enumerated summary value or the sub-portfolio matches nothing (FR-008). → spec **P-28** (FR-004, FR-005)
- **Q: With the badge gone, a collapsed row no longer says a portfolio came from a breakout at all — the name convention was not enough (Ben, 2026-08-11). What comes back?** → **A: The `↳` arrow alone, on the name cell's sub-line before `Portfolio #{id}`.** Its hover title names the base portfolio; the criteria stay in the expanded panel. A `Breakout` word-tag was tried first and rejected as far too heavy for what it says (Ben, 2026-08-11) — the thing that crowded the column was the `↳ from {source} · {dim}: {value}` string and its tooltip, and the arrow was never the problem.
