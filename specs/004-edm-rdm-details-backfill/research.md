...
**Rationale**: Article 1 — a page is one nav node + handler + template; this iteration edits the template + handler payload of existing nodes, adding no scattered config. The design record (design note `04` §4–§5, FR §2.2) fixes the layout: per-portfolio primary, aggregate as quick orientation, treaties EDM-level.
