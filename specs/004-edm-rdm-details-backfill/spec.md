...
- **FR-013**: For each portfolio, the system MUST show its location, account, and policy counts; perils and sub-perils covered; geography (regions and/or states, or a CIC-defined region label); currency; and record volume. **Record volume == location count** — it is surfaced via the Locations figure, not a separate "Records" column (ui.md §2). (Total insured value MAY be shown per portfolio where available.) *Source note (2026-07-23):* counts + perils come from the RM `/metrics` read; sub-perils, geography, currency, and TIV come from the DataBridge exposure-summary aggregate and render "—" when that summary is unavailable.
...
- **Detail counts/perils are sourced from Risk Modeler REST; TIV/geography/currency/sub-perils from a read-only DataBridge aggregate.** *(Amended 2026-07-23: the RM `/metrics` endpoint carries only counts + a perils string; no RM REST endpoint returns TIV, currency, or geography at any level — confirmed against wheel 0.2.1 + sandbox.)* Those fields come from one aggregate SQL read per EDM via an irp-integration DataBridge method — never raw SQL from app code (constitution Art. 11). DataBridge validation/profiling/exposure-modification (Phase A, §10) remains out of MVP.
