...
The mechanism is a **forward extension of the existing Iteration-2 completion path**: the poller detects an import reaching a terminal `FINISHED` status and enqueues a background work item; a worker performs the Risk Modeler REST fetch (EDM per-portfolio figures, treaty attributes, and broker-analysis metadata) and persists the detail. No detail fetch ever runs on a web request path, and the poller loop body never does the fetch itself (it remains a single-status-check batch). Detail is **backfilled forward only** for the automatic path — imports that complete after this capability ships are populated automatically; entities imported earlier stay in a graceful empty state until re-imported **or manually synced** (the EDM detail page's Sync action re-runs the same worker on demand).
...
- **Q: How do EDMs/RDMs imported *before* this capability ships get their detail data?** → **A: Forward-only automatic backfill, plus a manual per-EDM Sync.** Only imports that reach terminal `FINISHED` *after* this capability is deployed are backfilled automatically, via the extended poller/worker completion path. There is **no bulk one-time sweep**. *(Amended 2026-07-23 post-US1: the originally deferred per-entity manual refresh was adopted as the EDM detail page's "Sync" action — it re-runs the same `backfill_edm_detail` worker on demand, covering pre-capability EDMs and failed fetches.)* This aligns with the Iteration-3 exit wording "a newly completed import backfills its detail data automatically."
...
### Session 2026-07-28 (follow-up — exposure attribute set & summary source)

Two corrections taken while closing out US1's exposure-summary columns:

- **Sub-perils removed from the exposure detail; lines of business added.** Sub-perils are an analysis-settings attribute (FR-031 keeps them there), not an exposure attribute — the per-portfolio table and the aggregate rollup drop them. Lines of business (already a per-portfolio concept per the 7/23 header decision) takes their place in both. TIV is promoted from MAY to a shown attribute; geography is states (the summary source carries no country-level read).
- **Summary source (interim).** The requested wheel method `get_portfolio_exposure_summary` (FOLLOWUPS §6c) does not exist yet. Rather than update irp-integration now, the gateway runs repo-owned set-based SQL (`sql/databridge/` — TIV, states, lines of business, currencies; adapted from `knowledge/sql scripts/`) through the wheel's **generic** DataBridge executor, against the EDM database resolved from RM's `search_edms` `databaseName` (exposureId-matched). Summary shape: `{portfolioId: {portfolio_name, total_tiv, states, lines_of_business, currencies}}` — `total_tiv` is Moody's precomputed `exposure_metrics` account rollup (a single figure, no currency attribution; currencies remain their own list). Still read-only, worker-side, via irp-integration; degradation semantics unchanged (`summary: null`).
...
**Independent Test**: Import an EDM with multiple portfolios and let it reach *ready*; open its detail page and confirm every portfolio is listed inline with its own exposure figures (location/account/policy counts, perils, lines of business, geography, currency, TIV, record volume) populated from Risk Modeler — as a textual snapshot, no map, no Power BI rebuild — and with no create/edit/split control offered.
...
2. **Given** an EDM with N portfolios whose detail has been backfilled, **When** the analyst opens the redesigned EDM detail page, **Then** all N portfolios are listed inline, each showing its location/account/policy counts, perils, lines of business, geography, currency, TIV, and record volume — as a textual snapshot, with no map.
...
1. **Given** an EDM whose per-portfolio detail has been backfilled, **When** the analyst opens its detail page, **Then** a compact aggregate strip shows rolled-up figures (total location/account/policy counts, portfolio count, union of perils and lines of business, combined geography, currency set, total record volume) above the per-portfolio breakdown.
...
- **FR-003**: *Automatic* backfill MUST be **forward-only**: only imports completing after deployment are backfilled automatically, and the system MUST NOT run a bulk sweep of previously-imported entities. A **per-EDM manual "Sync" action** on the EDM detail page re-runs the same `backfill_edm_detail` worker on demand (analyst-triggered; approved post-US1 change, 2026-07-23) — it is the recovery path for pre-capability EDMs and failed fetches, and its scope grows with the worker (portfolios now, treaties with US2).
...
- **FR-013**: For each portfolio, the system MUST show its location, account, and policy counts; perils covered; lines of business; geography (states); currency; total insured value; and record volume. **Record volume == location count** — it is surfaced via the Locations figure, not a separate "Records" column (ui.md §2). *Source note (2026-07-23, amended 2026-07-28):* counts + perils come from the RM `/metrics` read; geography, lines of business, currency, and TIV come from the DataBridge exposure-summary aggregate and render "—" when that summary is unavailable. Sub-perils are NOT shown — they are an analysis-settings attribute (FR-031), not an exposure attribute (2026-07-28 scope call).
...
- **FR-040**: The EDM detail page MUST show a **compact aggregate rollup strip** above the per-portfolio breakdown, rolling up the per-portfolio figures (total location/account/policy counts, portfolio count, union of perils and lines of business, combined geography, currency set, total record volume — record volume == total locations, per FR-013) for quick orientation.
...
- **Portfolio (`irp_portfolio`)**: the **primary unit** of the EDM detail page — a named exposure view within an EDM, arriving with the EDM. This iteration enumerates every portfolio and stores its per-portfolio exposure figures (location/account/policy counts, perils, lines of business, geography, currency, TIV, record volume) for read-only inline display; it does not create, filter, or split them (Iteration 4).
...
- **SC-002**: An analyst can read every portfolio's key exposure figures (location/account/policy counts, perils, lines of business, geography, currency, TIV, record volume) for an imported EDM entirely in the workbench — zero clicks into Risk Modeler — and from them choose which portfolio to run and spot a missing peril or an oversized portfolio.
...
- **Forward-only automatic backfill (this session's scope call, amended 2026-07-23).** No bulk sweep. The per-EDM manual **Sync** action (adopted post-US1) re-runs the backfill on demand, so pre-capability EDMs can be populated without re-importing.
...
- **Detail counts/perils are sourced from Risk Modeler REST; TIV/geography/LOB/currency from a read-only DataBridge aggregate.** *(Amended 2026-07-23: the RM `/metrics` endpoint carries only counts + a perils string; no RM REST endpoint returns TIV, currency, or geography at any level — confirmed against wheel 0.2.1 + sandbox. Amended 2026-07-28: the aggregate is the repo-owned set-based SQL in `sql/databridge/`, executed through the wheel's generic DataBridge executor until the bespoke `get_portfolio_exposure_summary` method ships — see the 2026-07-28 clarification.)* One aggregate SQL read set per EDM, read-only, worker-side, via irp-integration (constitution Art. 11). DataBridge validation/profiling/exposure-modification (Phase A, §10) remains out of MVP.
