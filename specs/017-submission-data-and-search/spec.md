...
| P-17 | No contract is primary. The list row's CRM ID, treaty type and inception all read one contract: the first entered, or with a contract-level filter the first that matched; the other matches sit behind "+N more". The default sort is that same inception descending, then name; a submission with no contract sorts by its creation date in the same key. Any other place that needs one contract's value asks the analyst | Approved | note 34 D5, D6; user 2026-09-24 | 2026-09-24 (was the latest inception, with the CRM ID column first-entered) |
...
8. **Given** the submissions list, **Then** each row shows its first-entered contract's CRM ID, treaty type and inception with the other CRM IDs and distinct treaty types as "+N more", and the default order is that inception descending, a submission with no contract placed by its creation date; **When** the analyst filters on a CRM ID, treaty type, inception, Contract status or in force, **Then** the row shows the first contract that satisfied those filters together and counts only the other matching contracts.
...
- **FR-007**: Each row's CRM ID, treaty type and inception columns read one contract: with no contract-level filter, the first entered; with contract-level filters (CRM ID, treaty type, inception, Contract status, in force), the first entered among the contracts that satisfy them together (P-18). The other contracts in that set follow as "+N more" on the CRM ID column and, for their other distinct treaty types, on the treaty type column (P-17, note 34 D5). The default order is the row's inception descending, then name; a submission with no contract is placed by its creation date in the same key. The inception filter matches a submission whose contract incepts on that date. Contract status is not a list column (the Contract status filter and the deal card carry it).
