...
- [x] T077 [O-01] Answered on the 23 Sep call without the demo (note 34 D9): CRM's words are Open, Won, Lost; In Process becomes Open and nobody maps anything. O-01 closed. The demo moves to Thu 24 Sep against Cheryl's table (T083)
...
### Phase 10b: the source table and Open (note 34 D9–D12, 2026-09-23 afternoon)

- [x] T079 [P-02] [O-01] Rename the Contract status code `IN_PROCESS` → `OPEN` and its label "In Process" → "Open" inside `alembic/versions/0001_initial.py` before the last rebuild (note 34 D14), with the SQLite mirror, `submission_service.OPEN`, the route error text, the form default, the seeder, the unit and SQL Server tests, constitution Article 4 (4.2.3), the FR doc, `DATA_MODEL.md`, `PRD.md` and spec 017's files. The unused `.status-chip--in_process` rule is deleted
