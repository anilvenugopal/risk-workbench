...
- **Templates are executed exactly as stored** — the submitted analysis uses the template's stored model profile, output profile, event-rate scheme, and analysis settings with no defaulting or recomputation at submit time (constitution: approved plans are immutable).
- **Currency is a submit-time setting, not template config** (design note 17 D4/D5, reversing spec 009 P-10) — the analyst confirms analysis currency + currency scheme + scheme vintage in the modal, per chosen suite; the confirmed values are part of the approved plan and submitted exactly. Defaults are pinned in application environment variables; the system never advances them on its own (D7 — "CIC flips the switch").
...
| P-02 | Several suites — or several templates — per execution, never mixed; Submit disabled until at least one is chosen. Templates are not deduplicated across suites: a template chosen in two suites submits once per suite, each with that suite's currency (dedup dropped 2026-08-20). | Approved (PRD §11.3a; amended 2026-08-20) |
...
| P-05 | Fixed analysis naming: `CRE_{portfolio name}_{template name}` — `CRE_` prefix, underscore delimiter — right-truncated to the 64-character cap; the full name is stored. | Approved (PRD §2.6, 2026-08-20; format amended per design 18, 2026-08-24) |
...
| P-10 | Rerun naming: when the fixed name already exists, the suffix `_2`, `_3`… is appended within the 64-character cap (the unsuffixed original is implicitly #1; parentheses are not valid in Risk Modeler names). Reruns never block, and analysis names stay unique — Iteration 8 grouping resolves member analyses by name. The stored full name carries the same suffix. | Approved 2026-08-20; suffix format amended per design 18, 2026-08-24 |
| P-11 | Background submit: clicking Submit closes the modal immediately; the confirmed run (portfolios × templates + per-suite currency + treaties) is persisted and submitted in the background, each analysis appearing in the user-executed section as its submission lands. Navigating away never abandons the run. | Approved 2026-08-20 |
...
| P-15 | Currency (analysis currency + currency scheme + scheme vintage) is chosen in the execution modal — per chosen suite for a suite run, once for a template run — never stored on templates. Vintages are picked nested under the chosen scheme; the as-of date derives from the chosen vintage's effective date. Mixed-currency books run as separate executions. (Design note 17 D4/D5.) | Approved 2026-08-20 |
| P-16 | Currency defaults are pinned application environment variables (code, scheme, vintage) that pre-fill the modal's pickers; the analyst can override every run. The system never auto-advances the default when a new vintage releases — ops changes the values. No admin UI or RBAC for the defaults in MVP. A default that is unset or absent from the synced cache pre-selects nothing; Submit stays disabled until the analyst chooses. (Design note 17 D6/D7.) | Approved 2026-08-20 |
| P-17 | Every analysis submitted from a submission context carries the submission's name as a Risk Modeler tag (applied at submit alongside the template's tags) so it is findable in the platform and via API; the workbench records the execution → submission association. Standalone-EDM executions carry no submission tag. (Design note 17 D12 direction.) | Approved 2026-08-20 |
| P-18 | The user-executed Analyses grid groups rows by status — Failed on top, then In progress, then Ready — date-descending within each group, with a status filter, Template and Submitted columns, and a Risk Modeler link-out on rows whose `appAnalysisId` (the RM web UI's analysis id, backfilled with the analysis details) is known. Failed-to-submit rows group under Failed. Tags on the grid are deferred (design 18 O18-8). (Design 18 D4/D5.) | Approved 2026-08-24 |
| P-19 | Analysts can multi-select terminal analyses (ready / failed / failed-to-submit) in the grid and delete them: the workbench deletes each analysis in Risk Modeler synchronously on the request path, then soft-deletes the local record, freeing the name for reruns. A row whose Risk Modeler delete fails stays visible for retry; in-flight rows cannot be deleted. (Design 18 D6.) | Approved 2026-08-24 |

**How to verify.** Select several portfolios, run a suite from the modal — confirming the pre-filled currency and overriding it for one suite — and watch one auto-named analysis per portfolio × template appear in the EDM page's user-executed section and move through statuses live; deselect a template inside an expanded suite and see it excluded; force a peril mismatch and read its reason; run a single template the same way; edit a treaty via the Risk Modeler pass-through and see the refreshed view reflect it; after the loss-retrieval phase, open a finished analysis and read its loss numbers per perspective.
...
- Q: Design note 17 D5 says currency applies "at the suite level" — how does that map to multi-suite executions and Execute Template? → A: **Configured per chosen suite** within one execution (two suites → two currency selections), **once per execution** for a template run (P-15). Template dedup across suites is dropped (P-02 amended) — currencies can differ per suite, so a shared template submits once per suite.
- Q: Is the default vintage computed at runtime (most-recent currently-effective) or pinned? → A: **Pinned** (P-16): all three defaults — currency code, scheme, vintage — are literal env-var values ops maintains. "Most-recent effective" is guidance for what ops sets, not runtime logic; nothing auto-flips (D7).
- Q: Does the submission-level tag (note 17 O17-6) land here or with Iteration 8 grouping? → A: **Here** (P-17): tagging happens at submit time, which is this spec's machinery — deferring it would mean retro-tagging. Both a Risk Modeler tag and the workbench association.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Run template suites against selected portfolios (Priority: P1)

The analyst opens an EDM detail page, selects one or more portfolios in the portfolio table, and clicks **Execute Suite**. A modal lists the suites with a simple search; they choose one or more suites, optionally expand a suite and deselect templates that don't apply (e.g. flood not covered by the treaty), confirm each chosen suite's currency (analysis currency + currency scheme + scheme vintage, pre-filled from the configured defaults and overridable), pick the treaties the analyses should apply (by name), and click Submit — which is disabled until at least one suite is chosen. The workbench submits one analysis per selected portfolio × selected template of each chosen suite, naming each by the fixed rule (portfolio name + template name). Submission failures are reported immediately and retried automatically; analyses Risk Modeler runs and fails are reported with their reason and never block the rest.
...
1. **Given** an EDM with 3 portfolios and a suite of 10 templates, **When** the analyst selects the 3 portfolios, opens Execute Suite, picks the suite, and submits, **Then** 30 analyses are submitted, each named `CRE_{portfolio name}_{template name}` (right-truncated to 64 characters; full name kept), each tracked as its own job.
2. **Given** two chosen suites that share a template, one confirmed in USD and one in EUR, **When** the run is submitted, **Then** the shared template produces one analysis per portfolio per suite — each carrying its suite's currency, the second named with the P-10 suffix.
...
6. **Given** a template with stored model profile, output profile, event-rate scheme, and analysis settings, **When** its analysis is submitted, **Then** the submission carries exactly the stored values plus the suite's confirmed currency block — nothing defaulted or recomputed at submit time.
7. **Given** the modal open with the configured defaults (say USD / RMS / latest-set vintage), **Then** each chosen suite's currency pickers are pre-filled with those values and visible — never hidden behind an "advanced" toggle; **When** the analyst overrides one suite's currency, **Then** only that suite's analyses carry the override.
8. **Given** Risk Modeler is unreachable for one submission in the loop, **When** the run is submitted, **Then** that analysis is marked as failed-to-submit with the failure visible to the analyst immediately, it is retried automatically up to the configured maximum, and the other analyses are unaffected.
9. **Given** a suite containing a flood template run against data with no flood exposure, **When** the flood analysis fails in Risk Modeler, **Then** its failure reason (e.g. "no locations match the criteria") is shown against that analysis, and sibling analyses continue unaffected.
10. **Given** a large run (say 150 analyses), **When** the analyst clicks Submit, **Then** the modal closes immediately, submissions proceed in the background exactly as confirmed, analyses appear in the user-executed section as each submission lands, and navigating away does not abandon the run (P-11).
...
The analyst selects portfolios and clicks **Execute Template**. The same modal opens, listing templates instead of suites, with the same search; several templates can be chosen, currency is confirmed once for the whole execution (P-15), treaties are picked the same way, and Submit behaves identically — one analysis per portfolio × template. Suites and templates are never mixed in one execution.
...
- A currency not carried by the chosen scheme (currency-in-scheme membership deliberately unvalidated) → fails at submit; reported with Risk Modeler's reason. Vintage cannot mispair with scheme — the picker lists vintages nested under the chosen scheme (P-15).
- A configured currency default that is unset, or names a code/scheme/vintage absent from the synced cache → the affected picker pre-selects nothing; Submit stays disabled until the analyst chooses (P-16). Never a silent substitution.
- The same portfolio × template appearing twice in one run (a template shared by two chosen suites) → both submit; the second name takes the P-10 suffix.
...
- **FR-019**: The modal MUST present a currency selection — analysis currency, currency scheme, and scheme vintage — per chosen suite for a suite run, once for a template run (P-15). All three pick from the synced reference cache; vintages are listed nested under the chosen scheme. The selection is always visible before Submit.
- **FR-020**: The currency pickers MUST pre-fill from the configured application defaults (environment variables for code, scheme, and vintage — P-16). The defaults are pinned values: the system MUST NOT advance them when a newer scheme or vintage appears in the cache. A default that is unset or absent from the cache pre-selects nothing, and Submit stays disabled until the analyst completes the selection.

**Submission (US1, US3)**

- **FR-005**: Submit MUST produce exactly one analysis per selected portfolio × selected template of each chosen suite (or per selected template, for a template run). Templates are not deduplicated across suites: a template chosen in two suites submits once per suite, each with that suite's currency (P-02 as amended).
- **FR-006**: Each submitted analysis MUST carry exactly the template's stored values — model profile, output profile, event-rate scheme (when stored), analysis settings, and tags — plus the execution's confirmed currency block (currency + currency scheme + scheme vintage, with the as-of date derived from the chosen vintage's effective date) — with no defaulting, substitution, or recomputation at submit time.
- **FR-007**: Each analysis MUST be named by the fixed rule: `CRE_{portfolio name}_{template name}` (P-05), truncated from the right to Risk Modeler's 64-character analysis-name cap; the full untruncated name MUST be stored. The analyst never types an analysis name. When the name already exists (a rerun of the same portfolio × template), the suffix `_2`, `_3`… MUST be appended within the 64-character cap so the submitted name is unique; the stored full name carries the same suffix (P-10). A rerun is never blocked by the name check.
...
- **FR-012**: Clicking Submit MUST close the modal immediately; the confirmed run — the exact portfolios × templates, per-suite currency blocks, and treaties as approved — MUST be persisted and submitted in the background, with each analysis appearing in the user-executed section as its submission lands (P-11). The run MUST complete exactly as confirmed even if the analyst navigates away or closes the browser, and MUST NOT lose or recompute any part of the approved selection.
- **FR-021**: Every analysis submitted from a submission context MUST carry the submission's name as a Risk Modeler tag, applied at submit alongside the template's tags, and the workbench MUST record which submission the execution came from (P-17). Executions started from the standalone EDM page carry no submission tag.
...
- **FR-022**: The user-executed section MUST group analyses by status — Failed, then In progress, then Ready — date-descending within each group, offer a status filter that survives the section's live refresh, show Template and Submitted columns, and link out to the analysis in Risk Modeler once its `appAnalysisId` is known — the RM web UI route takes `appAnalysisId`, not the API `analysisId` (P-18).
- **FR-023**: Analysts MUST be able to multi-select terminal analyses (ready, failed, failed-to-submit) and delete them after a confirmation. In-flight analyses — including a finished job whose analysis id has not been recorded yet — MUST NOT be deletable (P-19).
...
- **Accumulation-profile templates execute through the same path** as DLM/HD templates; accumulation-specific output shapes matter at results viewing, not at submission. Caveat (note 17 O17-10): accumulation is not implemented anywhere yet, and whether it requires an event-rate scheme is unconfirmed — a rejection surfaces as that analysis's failure reason, like any other.
...
- **Suite/template administration is unchanged** — this feature reads templates and suites as spec 009 shipped them (unordered suites, no stored currency, no per-item settings). The currency scheme/vintage reference cache (`irp_currency_scheme`, `irp_currency_scheme_vintage`) also shipped with spec 009 and feeds the modal's pickers.
