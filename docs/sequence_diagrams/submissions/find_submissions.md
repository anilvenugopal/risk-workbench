...
Two lists over the same data: **My submissions** (`/submissions/mine`, the working default) and
**All** (`/submissions`). Both take the same four filters — cedant, treaty type, inception date,
treaty year — applied as you type via HTMX, plus a client-side name/text narrowing of the
rendered rows.
...
Filters are AND-combined onto query 1. `owner_id` is the *only* difference between the two
routes: `/submissions/mine` passes the current user, `/submissions` passes `None`.
...
sequenceDiagram
    ...
    rect rgb(238,244,255)
        ...
        User->>App: GET /submissions/mine   (or /submissions for All)
        App->>DB: SELECT submission + kind labels + owner name<br/>WHERE optional cedant / treaty_type / inception / treaty_year<br/>(+ assigned_analyst_id only on /mine)
    ...
    rect rgb(238,244,255)
        ...
        loop as the analyst edits a filter
            User->>App: GET /submissions/mine?cedant=…&treaty_type=…&inception=…
...
- **"My submissions" is a filter, not a permission** (Article 6 / FR-019). It matches
...
- **The default view is the narrow one.** `/submissions/mine` is what the nav points at, so the
  common case is small; All exists for hand-offs and coverage. That's a UX choice with no
  security meaning.
