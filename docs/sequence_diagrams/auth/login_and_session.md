...
Two supported ways in — **Microsoft (OIDC)** and **password** — and one way out. Once in, every subsequent request is validated by
...
sequenceDiagram
    ...
    rect rgb(238,244,255)
        Note over User,DB: PASSWORD — enabled by AUTH_MODE=password or both
        ...
        alt bad credentials
            ...
            Note over App,DB: unknown email runs one dummy bcrypt check<br/>and writes one failure row
    ...
    rect rgb(238,244,255)
        Note over User,IdP: OIDC — enabled by AUTH_MODE=oidc or both
...
- **Unknown emails receive the generic failure response.** The route runs one dummy bcrypt
  comparison and writes one `login_attempt` row.
...
- **Password and OIDC authentication are production options.** `AUTH_MODE` controls which
  login methods are rendered and accepted. A password POST is rejected when password
