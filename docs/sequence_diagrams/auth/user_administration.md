...
|---|---|---|
| Admin creates a password user | `app_user` | INSERT — hashed temp password, **`must_change_password=1`** |
| Admin creates a password user | `user_role` | INSERT — the chosen role |
| Admin pre-provisions an Entra user | `app_user`, `user_role` | INSERT — email + role, **`entra_oid` NULL**; idempotent by email |
...
sequenceDiagram
    ...
    rect rgb(238,244,255)
        ...
        Admin->>App: POST /admin/users/new (email, name, role, temp password, CSRF)
        App->>App: _require_admin — non-admin ⇒ 302 to /
        App->>App: validate_password_requirements
        App->>DB: INSERT app_user (hash, must_change_password=1)
        App->>DB: INSERT user_role
        App-->>Admin: the user detail page
...
sequenceDiagram
    ...
    rect rgb(238,244,255)
        ...
        App->>DB: INSERT app_user (entra_oid NULL) + user_role — idempotent by email
