...
| Migrations | Alembic (targets `WORKBENCH` connection only). **Through spec 017:** drop-create-seed with one `alembic/versions/0001_initial.py`, edited in place. **After specs 016 and 017 merge:** rebuild RHEL9 once, freeze `0001_initial.py`, and add one revision file per later schema change; deployments use `alembic upgrade head`. |
...
- **Dev DB strategy through spec 017: rebuild, not upgrade.** The WORKBENCH schema is one Alembic revision edited in place. WSL2 and Docker rebuild with `alembic downgrade base && alembic upgrade head`; an existing RHEL9 database uses `infra/scripts/rhel9/rhel9-db-rebuild.sh` because it may have been built from an older edit. After specs 016 and 017 merge, rebuild RHEL9 once, freeze `0001_initial.py`, and add one Alembic revision per later schema change. The LOSS stage schema is bootstrapped by an idempotent SQL script (`make bootstrap-loss`).
