...
**Own results only.** Pushing **broker** RDM results to the Loss Repository is **out of MVP** (FR §7; §17.4). The treaty-level (TY) export is spec 016 (`specs/016-ty-perspective-export/`, 2026-09-16): one data set per treaty per analysis from Risk Modeler's treaty output level; cross-analysis aggregation is by grouping first in Risk Modeler. Analysts can also **copy / paste** results out for ad-hoc use (FR §7). Uploading loss sets to Analyze Re is a separate API and out of MVP.
...
**Out:** treaty-level (TY, part B) export — now spec 016 (`specs/016-ty-perspective-export/`, 2026-09-16), built on the Risk Modeler treaty output level rather than `RDM_TREATY`; pushing broker results (out of MVP, §17.4); the event-catalog check unless CIC confirms it is still needed (O21-8); an analyst-facing Parquet file download (not an MVP deliverable unless separately requested); client creation (upstream, in Cheng's workflow tool — D25).
