...
- **Iterations 6 and 7 swapped (D9).** Iteration 6 is now **templates & suites — definition and administration** (entities, admin surface, seeded starter suites, CSV/Excel export-import, IRP metadata sync, analysis metadata screen); Iteration 7 is **analysis execution — single + suite run** (execution engine, suite run flow, results retrieval). References to the old Iteration 6 (execution/results) in change-log entries dated before 2026-08-17 mean today's Iteration 7.
...
- **§11.3 rewritten for predefined suites (D11):** admin surface (comprehensive, then pared down — O14-1), starter set (US / Canada / US+Canada / global, ~10 templates each — O14-4), CSV/Excel export + import (O14-2); suites may mix DLM/HD/accumulation (D14) — separation demoted to a convention.
