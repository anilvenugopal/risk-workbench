...
| A Submission has these attributes: name, cedant name, client, treaty type, inception date, expiration date, treaty year, directory path, CRM ID(s). | Implemented | Directory path = the deal's shared-drive folder. **Added 9/18 (spec 017):** client is optional and comes from CIC's repository client list, shown as "ID - name"; expiration date is optional. Each CRM ID may carry its own inception and expiration, inheriting the deal's per column when blank, and one control resets them all. |
...
| A Submission has a **Modeling status** (Active, Completed, Cancelled) and a **Submission status** (Won, Lost, In Process). | Implemented | **Changed 9/18 (spec 017).** The 8/5 Hold request is closed as not needed: Submission status = In Process carries the paused-deal meaning (P-01). Modeling status keeps its reason-per-transition trail and its "only Active accepts changes" rule; Submission status is a plain value set in place with no reason, editable in every Modeling status because the cedant's Won / Lost answer usually arrives after modeling is Completed (P-02, P-12). The two never share a label, a filter or a menu. |
...
> **Parked — CRM integration.** A future phase, not MVP. CRM holds the submission name, company/cedant name, inception date, expiration date, Submission status (Won / Lost / In Process) and CRM ID: "especially based on that CRM ID, we could pull a lot of data in." Today the submission name is copy/pasted from CRM by hand and the analyst enters the dates and the Submission status (spec 017). Expiration date and Submission status have two consumers: event response (which Won deals are in force on the day of an event) and retention (which deals are no longer on risk). (8/4; 9/18)
...
| Submissions can be filtered by cedant name, client, treaty type, inception date, owner, CRM ID, Modeling status, Submission status and "in force as of". | Implemented | The list carries eleven filters: name search, cedant, CRM ID, owner, Modeling status, Submission status, client, treaty type, inception, treaty year, in force as of. The cedant filter matches a fragment of the name rather than the whole value. **Added 8/9 (D16):** status, treaty type and owner each take several values, picked as chips from a menu that stays open, and treaty year takes several years typed one at a time. **Added 9/18 (spec 017):** CRM IDs are typed as chips too, each matching a whole CRM ID exactly (case-insensitive, trimmed); Submission status and client are pickers; "in force as of" lists Won deals with a CRM ID whose effective inception ≤ date ≤ effective expiration, defaulting to today, never stored. Values OR within a filter and AND across filters; twenty values per filter is the cap on every multi-value filter. Owner still defaults to the signed-in analyst and its menu narrows in place as the analyst types. |
...
| Requirement | Implementation | Notes |
|---|---|---|
| Treaty inception date and CRM ID auto-populate from the submission and stay editable; treaty year is carried without being shown. | Implemented | **Added 8/27 (D7); narrowed by spec 014 FR-002/FR-006.** The form pre-fills inception date from `submission.inception_date` and CRM ID from `submission_crm_id`, both editable. `submission.treaty_year` is recorded on the export and written to `dbo.RMS_HistoricalRDS.TreatyYear`, but it is not a form field — only the inception date was ever asked for. An edit to either editable field is recorded on the export only and never written back to the submission (spec 014 P-15). |
