...
| Condensed results render inline in the expanded analysis row. | Not implemented | **Added 8/25 (D9).** Both EP types at once, perspective toggle kept, no condensed/expanded display toggle. AAL at a glance as a gut check against broker results. |
...
| Standard deviation is shown. | Not implemented | From the EP stats endpoint (`totalStdDev`), per perspective. |
...
| OEP and AEP are both shown. | Not implemented | Cheryl uses OEP more. Shown together — no EP-type toggling in the inline view (8/25 D9). |
