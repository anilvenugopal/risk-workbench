...
| A Submission is one cedant's modeling project, carrying zero or more contracts. | Implemented | **Changed 9/22 (spec 017, note 32 D17–D20).** Was "one cedant's treaty of a given type at a given inception", e.g. "6/1/2026 CAT XOL for Cedant A." One modeling project can cover several CRM IDs with different outcomes — Wendy's Allstate case is "one modeling project for 8 CRM IDs" — so treaty type, inception and expiration belong to the contract, not the submission. |
...
| A Submission has these attributes: name, cedant name, client, data vintage, treaty year, directory path, and zero or more contracts, each with a CRM ID, treaty type, inception date, expiration date and Contract status. | Implemented | Directory path = the deal's shared-drive folder. Client is optional and comes from CIC's repository client list, shown as "ID - name" (9/18). **Changed 9/22 (spec 017, note 32 D17–D23):** treaty type, inception, expiration and the Won / Lost / Open status moved to the contract; a new contract opens on the previous one's dates and a blank expiration is inception plus one year less a day. Data vintage arrived on the submission — "the in-force data as-of date of the data that's been provided to us in the EDM" (Wendy) — one per submission by convention even when its portfolios differ ("we would usually pick one"), and overridable on the export. |
| Each contract has exactly one treaty type; a Submission's contracts may differ. | Implemented | **Changed 9/22 (spec 017).** The 8/4 single-select stands, per contract: multi-select was floated and rejected. The same EDM or RDM can relate to submissions with different treaty types. |
| Treaty year defaults from the data vintage on the form and from the earliest contract inception on save, and stays editable. | Implemented | **Changed 9/22 (spec 017 P-20).** Picking a data vintage fills a blank treaty year; when the field is still blank on save the server fills it from the earliest contract inception, and it stays blank on a submission with no contract. The 8/5 rule had the inception date fill it; 8/4's wording was "derived from the inception date… never free-text entry." Reverses design note 08 §2.1 D4, which carries the reason and the open re-confirmation with CIC. |
| The cedant field is workbench-side only; it is not linked to the Moody's/EDM cedant field. | Implemented | **Added 8/4.** There is no exposure data at submission-creation time to link against — cedant arrives *with* the exposure. "Totally fine, I just wanted to make sure." Two labels only: **Cedant** is this Workbench field and **Client ID** is CIC's repository client (spec 017 P-05). The treaty grid on the EDM detail page shows Risk Modeler's treaty attribute `cedant` as a column of its own, where the context already says whose it is. |
...
| Name and cedant are required at creation; a contract, once entered, needs a CRM ID, treaty type and inception date. | Implemented | **Changed 9/22 (spec 017 FR-007, note 32 O32-7).** Treaty type and inception moved to the contract and a submission may have no contract, so they can no longer be required of the submission. The original four were confirmed 8/4 — "seems fair." |
| Required fields are marked explicitly in the form. | Implemented | **Added 8/4.** Name and cedant carry an asterisk under a "* Required" legend; a failed submit names each bad field under its own input instead of returning one combined message, and a refused contract row is named under that row. |
| CRM ID is the only guaranteed-unique attribute. | Implemented | **Confirmed 9/22 (note 33 D12).** A CRM ID names one contract on one submission across the Workbench, case-insensitively; a save that reuses one is refused with a link to the submission that holds it (spec 017 FR-003). Everything else can overlap. |
...
| A Submission has zero or more contracts; the CRM ID is the contract. | Implemented | **Promoted 9/22 (spec 017, note 32 D17)** from the 8/4 aside "CRM ID = a contract". Each contract carries its CRM ID (plain text, **unique across the Workbench since 9/22**, note 33 D12), treaty type, inception, expiration and Contract status. Standard case is one; one modeling project can carry several. Reaffirmed 8/4 — the "one CRM ID" remark that session was the argument for single-select treaty type, not a change to CRM cardinality. |
| CRM detail is shown on the submission page. | Implemented | **Built 9/22 (spec 017).** The contract table under the deal card: CRM ID, treaty type, inception, expiration and Contract status per row; rows are added, edited and removed in place while Modeling status is Active. Added 8/4. |
...
| A Submission has a **Modeling status** (Active, Completed, Cancelled); each contract has a **Contract status** (Won, Lost, Open). | Implemented | **Changed 9/22 (spec 017, note 32 D17).** The 9/18 Submission status moved to the contract and took its name from Cheryl — "your submission status really should go down to the CRM table. The modeling status is fine to have one modeling status"; "Contract, it's a contract status" — after Wendy's case: "we can have multiple CRM IDs and one was bound and one was not." The 8/5 Hold request stays closed: Open carries the paused-deal meaning (P-01). Modeling status keeps its reason-per-transition trail and its "only Active accepts changes" rule; Contract status is a plain value set in place with no reason, editable in every Modeling status because the cedant's answer usually arrives after modeling is Completed (P-02, P-12). The two never share a label, a filter or a menu. |
| Modeling status can be reopened from Completed or Cancelled back to Active. | Implemented | Transitions are reversible; not a one-way door. |
...
> **Parked — CRM integration.** A future phase, not MVP. CRM holds the submission name, company/cedant name, inception date, expiration date, the Won / Lost / Open status and CRM ID: "especially based on that CRM ID, we could pull a lot of data in." Today the submission name is copy/pasted from CRM by hand and the analyst enters each contract's dates and Contract status (spec 017); the January bulk update is `infra/scripts/bulk_update_contract_status.sql`, run by CIC against `contract`, reading one status per CRM ID from `dbo.CRMContractStatus` in CIC's loss repository (note 32 D25; note 34 D10–D12; spec 017 FR-023). Expiration date and Contract status have two consumers: event response (which Won deals are in force on the day of an event) and retention (which deals are no longer on risk). (8/4; 9/18)
...
| An analysis is described as *related to* an EDM, never as existing *in* it. | Not implemented | **Added 8/28 (note 22 D23).** EDMs contain exposures only; in RiskLink an analysis exists in an RDM, and platform-run analyses live only on the platform. Cheryl's reason is communication: phrased any other way, "they'll be completely lost." The rule for all analyst-facing copy, the FR, and the specs: *these analyses were run against portfolios in this EDM*. Ben owes the EDM detail page's description. |
...
| Submissions can be filtered by cedant name, client, treaty type, inception date, owner, CRM ID, Modeling status, Contract status and "in force as of". | Implemented | The list carries eleven filters: name search, cedant, CRM ID, owner, Modeling status, Contract status, client, treaty type, inception, treaty year, in force as of. The cedant filter matches a fragment of the name rather than the whole value. **Added 8/9 (D16):** status, treaty type and owner each take several values, picked as chips from a menu that stays open, and treaty year takes several years typed one at a time. **Added 9/18 (spec 017):** CRM IDs are typed as chips too, each matching a whole CRM ID exactly (case-insensitive, trimmed); Contract status and client are pickers; "in force as of" lists submissions with a contract whose status is Won and whose inception ≤ date ≤ expiration, defaulting to today, never stored. **Changed 9/22 (spec 017 P-18):** CRM ID, treaty type, inception, Contract status and in force as of are contract attributes, and one contract must satisfy all of them together — a Won Aggregate XOL beside an Open Per Risk XOL does not answer "Per Risk XOL + Won". Values OR within a filter and AND across filters; twenty values per filter is the cap on every multi-value filter. Owner still defaults to the signed-in analyst and its menu narrows in place as the analyst types. |
| Submissions can be sorted. | Implemented | **Added 8/9 (D15):** Name, Cedant, Inception and Year are click-to-sort headers carrying a caret; clicking the sorted column flips it. One column at a time. The default is the first-entered contract's inception descending, the date the row shows; a submission with no contract sorts by its creation date (spec 017 P-17, **changed 9/24** from the latest inception, note 34 D6). A row's CRM ID, treaty type and inception columns all read that first-entered contract, or, when a contract attribute is filtered, the first contract that matched the search, with the other matches as "+N more" (note 34 D5). Applied filters and the page survive a sort, and the order rides in the URL. |
...
| Global search runs across Submissions, EDMs, RDMs, analyses, jobs, and results. | Implemented | Ctrl-J search offers six groups: pages, submissions, EDMs, RDMs, analysis templates, users (§19). Analyses, jobs and results are not groups; an analyses list waits for its own spec (spec 017 P-08). |
| Search, sort, and filter are available on every list section. | Partial | 7/14: wanted on portfolios, treaties, analyses, and results — not just the submissions list. **Added 9/4 (D6):** the merged analyses grid — the submission Results section and the EDM Analyses section — carries click-to-sort Peril, Region, Engine, Currency and Submitted headers, copying the 8/9 D15 pattern (one column at a time, clicking the sorted column flips it). "You don't have to do all of them." The applied status filter survives a sort and the order survives the 3s poll. Broker rows in the per-RDM groups keep their capture order. Portfolios, treaties and results still have no sort or filter. **Added 9/18 (spec 017):** the EDM library and RDM library carry a name search, an import-status filter, and the submission-attribute filters owner, cedant, client, treaty type, treaty year, CRM ID, Contract status and in force as of; an EDM or RDM is listed when one of its linked submissions satisfies every filter together. |
...
| Cedant and line of business show the first available value(s), not the full list. | Implemented | **Added 8/5.** Both can be long and multi-valued; the condensed view reports presence, not granularity — the first value plus a "+N" count. The full list is on expand. |
...
| Regions are not pre-defined constants. | Not implemented | "Northeast" is defined by the treaty / how the cedant writes the business; contracts define coverage arbitrarily. |
...
> **Demoed 8/28 with wrong output (design note 22, D13). Section rewritten the same day.** Ben's prototype grouped one stochastic and one historical analysis and passed **both** event rate schemes into the group where the manual Risk Modeler flow presents **one** (defaulted, changeable); gross pure-premium losses came out ~$8M apart. Cheryl's rule, previously recorded nowhere: "You can't group with two different rate sets … the same event listed twice with two different event rates. And what we want is 1 event with the losses aggregated and one event rate." The diagnostic: the AAL "is going to be close, but it's not going to be the same. And your EP curve will look wonky." Grouping has no spec (spec 011 defers it to Iteration 9) and no group-creation path on the main line — **write the spec before the fix** (O22-2), and put D13's failure mode and D14's validation method in it.
...
> **Naming collision (8/27).** CIC's workflow tool has its own repository-side "group losses" feature that combines already-pushed loss sets (used after per-EDM treaty exports — §7 Delivery, part B). That is not workbench analysis grouping, which combines analyses in Risk Modeler before export. Same word, different layer — keep them distinct here and in the specs.
...
> **Signed off 8/28 (design note 22, D1–D9) — the first client acceptance of the results layer.** Forty minutes of review, one change request (right-justify the return periods, D5), and Cheryl's verdict: "all of this looks great … I think the experience is great." Demoed on a submission with several EDMs, eleven executed analyses including cross-EDM groups, and the RDM Cheryl supplied — the first exercise of the merged grid at that mix. Closes 19 O19-8/O19-9; confirms 20 O20-6/O20-8 as built.

| Requirement | Implementation | Notes |
|---|---|---|
| Own and broker analyses are listed in one merged table, origin visually indicated. | Implemented | **Added 8/25 (D11); accepted 8/28.** Wendy: "I'm not opposed to putting them into the same table as long as they're visually indicated." The `CRE_` prefix plus the RDM association carry the indication; own-vs-broker derives from `rdm_id`. |
| Results are displayed grouped under the RDM that produced them. | Implemented | No EDM-to-RDM relationship is inferred from sharing a Submission. **Relocated 8/25 (D11):** broker rows keep the RDM-row-then-expand shape *inside* the merged table — the requirement moved, it did not drop. |
| Broker results are deduped by RDM. | Implemented | (`rdm_id`.) |
| The analyses table carries currency and AAL columns; no return-period column. | Implemented | **Added 8/25 (D10/D12); accepted 8/28.** Cheryl: AAL "gives you a good visual of a bunch of your analyses in a row." The AAL-only display mode is dropped. Currency renders per column; mixed currency is not a factor for *viewing* (8/28 D1, confirming 20 D17's asymmetry — comparison is different, below). |
| Condensed results render inline in the expanded analysis row. | Implemented | **Added 8/25 (D9); narrowed 8/26 (D11); accepted 8/28.** One EP type at a time beside the perspective toggle, no condensed/expanded display toggle. AAL at a glance as a gut check against broker results. Closes 20 O20-6. |
| A dedicated results page is reached by multi-select from both the submission page and the EDM detail page. | Implemented | **Added 8/25 (D14).** The submission page is mandatory — cross-EDM analyses and groups live only there. |
| The dedicated results page opens in a new browser tab, with breadcrumbs, and the tab title carries the submission or EDM name. | Partial | **Added 8/25 (D21/D22).** From the EDM: breadcrumbs retain EDM + submission; from the submission: submission only. **Defect 8/28 (O22-15):** the results breadcrumb is an empty placeholder, "analyses" is not a link, and back-navigation from a submission-context EDM screen lands on the non-contextual EDM library (fix in progress). |
| Displayed units never auto-switch; a ones/thousands/millions selector controls them. | Not implemented | **Added 8/25 (D16).** Cheryl: "let's not switch between millions and thousands … keep everything in millions with the decimal points." **Display-only (clarified 8/28, D6):** the clipboard copy is always in ones at full precision, whatever the selector shows. |
...
| Viewing has no cap; columns condense to a minimum width, then the view scrolls sideways. | Implemented | **Reworded 8/28 (D2)**, replacing "up to ~5 analyses are consumable on screen": the ~5/~10 density guidelines were Ben's and never agreed with CIC. Demoed uncapped at billions-scale values; Cheryl: "Yeah, it's great." Closes 19 O19-8's viewing half. |
| Full listing / full drill-down of all analyses remains available. | Implemented | Never a step back from RiskLink, which lists them all. |
...
| AAL / pure premium is shown. | Implemented | From the EP stats endpoint (`purePremium`), per perspective. On screen throughout 8/28 and used by Cheryl — second consecutive positive signal on the grid AAL column (19 O19-2). |
| Standard deviation is shown. | Implemented | From the EP stats endpoint (`totalStdDev`), per perspective. **Confirmed in scope 8/26 (D11)** — it sits outside the EP-type selection and does not change with it. Column-group shading separating the EP curves from AAL/std dev accepted 8/28 (D3). |
| Return-period loss numbers are shown. | Implemented | **Fixed 8/25 (D7, closes O5-2):** expanded set 5/10/25/50/100/250/500/1000/2000/5000/10000; condensed set 50/100/250/500/1000/10000. Not user-editable. |
| Return-period values are right-justified. | Not implemented | **Added 8/28 (D5)** — the session's only change request in forty minutes of review. |
| One EP type is shown at a time; OEP is the default. | Implemented | Cheryl uses OEP more. **Reversed 8/26 (D11):** one at a time, selected in both the inline row and the dedicated page — showing both meant a merged header, which offset the paste into Excel. **Confirmed 8/28 (D1):** AEP offered, declined — "No, OEP is great." |
...
| Results can be switched between financial perspectives. | Implemented | Perspective switching is essential; "look at it from however you ran it." **Set fixed 8/25 + spec 011 O-07:** GR, RL, WX, QS, GU (Ground Up added by Wendy for checking treaty application, "especially on the work comp side"). Selection applies screen-wide, never per analysis. RL (pre-cat net) is the default (confirmed 8/28, D1); a perspective the analysis did not produce renders explicitly empty (WX/QS demoed empty 8/28). |
| RP (post-cat) is not exposed for viewing. | Implemented | **Added 8/27.** The platform returns RP for analyses that have it, and it could be shown "if we wanted." CIC needs it for export, where it is offered (Delivery below). A deliberate omission from the viewing set, not an oversight — revisit if CIC asks to see it. Not surfaced 8/28 either (note 22 O22-13); the export path still needs it. |
| Analysis metadata is shown alongside results. | Partial | See the metadata list below; reused for broker-result review (§2.3). |
| Treaties applied to an analysis are shown in the analysis view. | Not implemented | **Added 8/28 (D21).** "That would be helpful to see if treaties were applied. Yes, I agree." Also Cheryl's diagnostic for treaties-applied-but-no-loss (D22 below), and TY export needs the same fact for selection (21 O21-4). The one genuine remaining run-facts gap — `irp_analysis` records no applied-treaty set (O22-7). |
| A group's member analyses are listed in the expanded row, whether the Workbench composed the group or it was imported by Risk Modeler id. | Implemented | **Added 9/18 (note 32 D5).** Cheryl: "this is what's really most important to me is that I know what the underlying analyses are." An imported group's members are the analyses its Risk Modeler detail names, recorded at import (issue #123). A broker group captured from an RDM lists none: Risk Modeler does not name its members. |
| Each list in the expanded row (members, regions & rates, treaties) shows five entries, and "+N more" opens the rest in place. | Implemented | **Added 9/18 (note 32 D4, D6).** Wendy: "how are we going to handle it when that list of rate schemes is 25 long?" The full list stays in the Workbench, not behind a link to Risk Modeler. Cheryl: "our risk is the more times people have to bounce out to risk modeler… they stop bouncing back into Workbench." **Expiry condition:** needed while analysts review analyses from broker-supplied RDMs. CIC's own runs use its default setup profiles, and Wendy: "if I ran the analyses here… I don't need to like interrogate that at all." |
| What was submitted vs. what the analysis returned is not displayed. | Implemented | **Added 8/28 (D22). A deliberate non-requirement — do not re-propose.** Ben wanted the developer-facing check; Cheryl declined: "I have naive confidence that the model is doing what it's supposed to do." Her diagnostic is flipping to WX and seeing zero. `submitted_settings` is still captured (constitution Art. 8) and available for support and debugging; the analyst-facing answer to Ben's treaty case is the applied-treaties row above. |
...
**Narrowed 8/26 (D4/D10, spec 011 O-11).** The expanded analysis row shows engine version, analysis type, subperil, framework, event-rate scheme, and who ran it. Engine type, region, peril, portfolio and template are columns on the merged table instead; unrecognized construction and occupancy, currency, construction, line of business, term and loss amplification are shown nowhere today and return when the team asks for them.

**Confirmed as built 8/28 (D19/D20).** Portfolio and Template as separate columns ("Oh yeah. I like that"), the full analysis name in the expanded view, and analysis type relocated to the expanded row **instead of** an abbreviation column — Ben declined the code map ("that's a map I'd have to manually maintain"), which closes 20 O20-3 by removal; record it so the map is not proposed again.
...
**Comparison** — pairwise over any two finished analyses, user-executed or broker-provided (renamed from "broker comparison" 2026-08-27; PRD §17)

| Requirement | Implementation | Notes |
|---|---|---|
| Own and broker results can be viewed together. | Implemented | Multiple analyses in one view. **Designed 8/25 (D17):** viewing is N-up. **Built and accepted 8/26** (note 20 D1, spec 011): the merged table and the dedicated results page; the build is uncapped and scrolls sideways. **Confirmed uncapped 8/28 (note 22 D2)** — the ~10 soft guideline was never agreed with CIC and is retired. |
| Any two finished analyses can be compared side-by-side, regardless of origin. | Implemented | Ben has a prior comparison engine to build on. **Designed 8/25 (D17/D18), extended 8/26 (note 20 D16):** Compare is a separate action from View; a modal collects strictly pairwise selections into a cart, 5 pairs per screen. Base-vs-many considered and rejected. Renders on a dedicated comparison page in a new browser tab (decided 8/27, PRD §17.3). **Built 8/28 (spec 013):** the Compare modal collects pairs; `/results/comparison` renders them in a new tab. **Accepted 8/28 (note 22 D7)** — the 5-pair cap was offered for expansion and declined ("Nope"), which closes 19 O19-8's comparison half. |
| Comparison pairs are drawn from the merged analyses table the analyst is on. | Implemented | **Decided 8/27.** No cross-submission pairing — the year-over-year comparison stays served by copy-with-headers (Delivery, below). The modal offers only analyses with retrieved results. **Built 8/28 (spec 013).** |
| The side-by-side comparison includes a percent-difference column. | Implemented | e.g. CIC vs. broker — saves the manual Excel step. **Designed 8/25 (D19/D20):** selection order is the contract — first picked = base = first column, percent change follows; decoupled from list order; no row-header column; percent-change columns need no per-pair labelling. **Built 8/28 (spec 013):** one % Chg column per pair, plus AAL and Std dev rows. **Accepted 8/28 (note 22 D8)** — percent change is (comparison − base) ÷ base × 100, spot-checked live by Cheryl; closes 19 O19-9 with a recorded warning: Cheryl initially assumed she could not control the base because she could not reorder the list, so **the base marker must be legible at selection time**, not only after the comparison renders. |
| A comparison pair must share a run currency — mixed-currency pairs are blocked, never converted. | Implemented | **Added 8/26 (note 20 D17).** Wendy: "I would just not let people do it"; Cheryl: "everybody should know that's not a valid comparison." Enforced at pair-add time from the run currency stored in `submitted_settings` (spec 011). Deliberate asymmetry: N-up *viewing* still shows mixed currencies unconverted — viewing makes no arithmetic claim; comparison computes a percent change. **Built 8/28 (spec 013):** refused at pair-add time and re-checked at render, which drops the pair whole. **Accepted 8/28 (note 22 D9)**, confirming 20 O20-8's asymmetry. |
| Perspective and EP-type selections apply screen-wide, to every comparison rendered. | Implemented | **Designed 8/25 (D20) + 8/26 (note 20 D11):** one EP type at a time; AAL and standard deviation sit outside the selection. Never per pair. **Built 8/28 (spec 013).** |
| The comparison shows which engine/model version each side ran. | Implemented | Wendy's broker-vs-own requirement (design note 18 O18-10): "just because they ran it in 23 doesn't mean they used the 23 rates." Spec 011 FR-021 captured engine and model version at retrieval so this view can show them. **Built 8/28 (spec 013):** each side's header sub-line carries its run currency and engine. |
...
> **Rewritten 8/27.** Cheryl's workflow-tool walkthrough (design note 21) replaced the single "ELTs are uploaded" row with the export flow below, and the loss repository schema arrived the same day as screenshots (`dbo.Data`, `dbo.RMSELT`, `dbo.RMS_HistoricalRDS` — column detail in PRD §16.3). The flow, as built: pick analyses → pick one valid perspective → choose a client and enter the export parameters → Export. The 8/27 review step between the form and the commit went with spec 014 P-14 — each analysis processes to loaded or failed and the corrections are reported afterwards on the export detail page.

| Requirement | Implementation | Notes |
|---|---|---|
| Results can be copied / pasted out. | Implemented | **Designed 8/25 (D13); confirmed 8/28 (D6):** copy table with headers, straight into Excel — Cheryl's year-over-year working file, and brokers who supply only a digital copy instead of an RDM. Replaces a manual horizontal→vertical pivot. The paste is unformatted, always in ones, at full decimal precision; display rounding and the units selector never affect it. |
| The workbench exports finalized loss sets to the Loss Repository itself: read from the Risk Modeler API, transform, write SQL. | Implemented | **Spec 014.** **Settled 8/27 (D1), re-confirming 8/26 D18 from the other direction.** Reusing CIC's workflow tool was raised and closed: it only sees analyses that live in RiskLink, which would mean round-tripping Risk Modeler results back to RDM and keeping RMS servers alive. DataBridge is not in the path — the 8/26 open question is answered by removal. Do not reopen. |
| A handled failure is closed so it stops reading as open work. | Implemented | **Added 9/11 (D5).** Wendy: "I went, fixed whatever I need to be fixed, re-upload. I just want to click it and have it go off my list." Close is a manual status on the export's analysis, offered wherever Retry is (spec 014 P-22): it records who closed it and when, re-runs nothing, and leaves the failure message on the row. A fix is a new export. |
| One export batch covers one or more selected analyses. | Implemented | **Spec 014.** Ben's restated acceptance list, 8/27: "pick multiple analyses … provide a specific perspective that's valid across all of these analyses … search that client list by name … review versus commit." |

**Delivery — client selection**

| Requirement | Implementation | Notes |
|---|---|---|
| The export selects a client from CIC's client table, searched by name. | Implemented | **Added 8/27 (D2, D3); spec 014 FR-002.** The table lives in the loss repository database and CIC maintains it: "I don't think it's anything you have to maintain. I just think it's something you have to be able to access." **Note 22 O22-9 is answered on both halves:** a raw name search over every row of `dbo.Client` — no `ActiveFlag` filter, because CIC maintains the flag by hand (9/11 D1, spec 014 P-18) — and the chosen client is recorded per analysis on `stage.rwb_loss_result_manifest.client_id` and written to `dbo.Data.ClientID` at load. |
| The client table is read-only; the workbench never adds clients. | Implemented | **Reversed 8/28 (note 22 D25), retracting 8/27 D4's INSERT.** Cheryl, after consulting Cheng: "I think we need to view the client table. I don't think we have to add clients … we would have to set up a client ID at that point [during exposure work, in Cheng's workflow tool] … So we can take that off the list." The reasoning is a workflow fact: by the time the workbench reaches the export, the client exists; edge cases go through the workflow tool or SQL. The insert-statement ask is withdrawn. Cheng's add logic, recorded for schema shape only: last ID + 1, IDs above ~8,000 are test entries excluded from the running number; the valid flag is manually maintained off the system (confirms 21 D5 from the other direction — nothing sends it). **9/11 (D1):** because the flag is hand-maintained, the export form filters on nothing and lists every client (spec 014 P-18). |
| The client ID is not the cedant name and is not stored on the submission. | Implemented | **Added 8/27 (D6); spec 014 FR-002.** A repository client is roughly cedant × reinsurance type — an external key CIC built by hand; one submission can push under different client IDs across treaty types. The client is chosen per export. |

**Delivery — export parameters**

| Requirement | Implementation | Notes |
|---|---|---|
| Treaty inception date and CRM ID auto-populate from the chosen contract and stay editable; treaty year is carried without being shown. | Implemented | **Added 8/27 (D7); narrowed by spec 014 FR-002/FR-006; changed 9/22 (spec 017 FR-011).** The form carries a Contract select over the submission's contracts, pre-selected when there is exactly one; choosing one fills inception date and CRM ID, both editable. Client and data vintage pre-fill from the submission. `submission.treaty_year` is recorded on the export and written to `dbo.RMS_HistoricalRDS.TreatyYear`, but it is not a form field — only the inception date was ever asked for. An edit to either editable field is recorded on the export only and never written back to the submission (spec 014 P-15). |
| The analyst enters a data vintage. | Implemented | **Added 8/27; required 9/11 (D2).** The analyst types it on the export form; `dbo.Data.DataVintage` and `stage.rwb_loss_result_manifest.data_vintage` are both `NOT NULL` (spec 014 P-19). |
| A CRM ID is entered on the export and written to the header row. | Implemented | **Added 8/27; spec 014 FR-002/FR-015.** `dbo.Data.CRMID`. One ID per export — see the reworded exclusion below. |
| No in-force date is entered or populated. | Implemented | **Added 8/27 (D8).** Deliberate non-requirement: the in-force date lives only in the RiskLink database naming convention — "Nobody's gonna think twice if it's there or not there." Recorded so it is not re-derived as an oversight. |
| No expiration date is entered. | Implemented | **Added 8/27.** Listed 8/26, absent from the walkthrough and from all three schema screenshots; treated as not required unless Cheryl confirms otherwise (O21-2). |
| The data ID is the repository's auto-incrementing counter, read back after the header insert. | Implemented | **Added 8/27 (D15); spec 014 FR-014.** `dbo.Data.DataID`. The workbench never generates it; the load procedure reads it back and records it on the export. |

**Delivery — perspective**

| Requirement | Implementation | Notes |
|---|---|---|
| Export offers only the perspectives that actually have data, resolved per analysis. | Implemented | **Added 8/27 (D9); spec 014 FR-003.** "So should we just provide the list of perspectives that are valid?" — "That would be fabulous." Not a fixed list; WX and QS returned no data for a live analysis. **The source is the workbench, not a platform call:** the form offers the configured codes (`EXPORT_PERSPECTIVE_CODES`, first set GU / GR / RL / RP) that every ticked analysis already has results for in `irp_analysis.loss_results`, which the retrieval worker wrote. |
| With several analyses selected, the offered list is the intersection across all of them. | Implemented | **Added 8/27 (D10); spec 014 FR-003.** Matches the workflow tool: "all that metadata has to be the same for every analysis that I select." |
| One perspective per export batch. | Implemented | **Added 8/27 (D11); spec 014 FR-002.** Per-analysis and multi-select perspectives were offered and rejected as rare ("Generally, it's a one-to-one"). Two gross + one post-cat = two batches. |
| RP (post-cat) is offered for export. | Implemented | **Added 8/27; spec 014 FR-003.** "The RP perspective is one we definitely need to have." The export vocabulary is wider than the viewing set. |
| WX and QS are never uploaded to the repository. | Implemented | **Added 8/27 (D14); spec 014 FR-003.** A scope rule, not a display filter — neither code is in `EXPORT_PERSPECTIVE_CODES`: "it's going to be across multiple perils. And what we're looking for is for a specific treaty." |
| The extract's scope is property. | Not implemented | **Added 8/28 (D32).** The export job's `scope` field offers property or workers' compensation; Cheryl: "we want property. We have to talk about workers comp later." An explicit deferral, not an omission. |

**Delivery — integrity checks and corrections**

| Requirement | Implementation | Notes |
|---|---|---|
| No review step stands between the form and the load. | Implemented | **Reversed 9/10 (spec 014 P-14), dropping the 8/27 review pane and D17's no-AAL rule with it.** Once Export is clicked each analysis processes to loaded or failed; there is no confirmation page and no cancel. A wrong client or inception date is corrected the way any other reload is. The per-analysis facts the 8/27 review would have shown — identity, currency, and both correction counts — are on the export detail page afterwards (spec 014 FR-018), and each analysis carries its AAL at the export's perspective (9/11 D6, spec 014 P-20), reversing D17. |
| Where loss exceeds exposure value, exposure value is raised to the loss. | Implemented | **Added 8/27 (D18) as a cap on the loss; direction reversed 9/2 (note 24 D12–D15, spec 014 P-03). Must-have.** Such rows hard-error the repository load; the fix is "just set them equal … It's 100% loss" (usually pennies of difference), and CIC chose to move the exposure value rather than the loss — **loss values are never changed.** The correction runs during the load, not at a review, and is counted per analysis. |
| A negative independent or correlated standard deviation is set to 0 on stochastic rows. | Implemented | **Added 9/2 (note 24 D12–D15, spec 014 P-04).** The one silent correction CIC's current tool was known to make. Historical rows get no standard deviation correction. |
| Both corrections are counted per analysis and shown on the exports table. | Implemented | **Added 8/27; spec 014 FR-018.** Either correction mutates what CIC receives — never applied silently. The exports table shows rows staged, stochastic rows, historical rows, rows with exposure raised, and rows with standard deviation zeroed. |
| PLT results cannot be exported; the form lists them disabled. | Implemented | **Added 9/16 (note 31 D16–D17); spec 014 FR-025.** An HD analysis or a PLT group is greyed on the export form with the reason and cannot be ticked, so no export mixes PLT and ELT. PLT's future is spec 014 O-08. |
| The event-catalog check is undecided: keep for MVP or drop. | Not implemented | **Added 8/27.** Events dropped from the catalog by a model upgrade; "usually zero" and Cheryl suspects it is obsolete — she is checking (O21-8). If kept, it needs the RiskLink 25 reference data (O21-6). Decide and record; do not leave undescribed. |

> **These integrity checks are not covered by the exclusions below.** The out-of-scope "formal loss validation against broker/cedant" is a business check against external parties, and the deferred Phase A DataBridge validation is exposure-side reporting. Neither covers D18 — do not reconcile the must-have away. Also pending: the full list of silent corrections Cheng applies in the current tool — the two above are what CIC named, and a third would be a new requirement rather than a gap in an unbuilt review (O21-8, O21-13).

**Delivery — the load**

| Requirement | Implementation | Notes |
|---|---|---|
| The load writes three loss-repository tables, identified by the data ID, as one transaction per analysis. | Implemented | **Added 8/27 (D15), correcting 8/26's two-table reading; spec 014 FR-014.** `dbo.Data` (header), `dbo.RMSELT`, `dbo.RMS_HistoricalRDS`. All committed or none, and each analysis in an export commits independently of the others. Column detail: PRD §16.3. |
| The header row carries the analyst-entered parameters; only the data ID is carried into the loss tables. | Implemented | **Added 8/27; spec 014 FR-015.** "This is sort of the glue that keeps all of our other pieces together." |
| ArchiveFile, AReLossSet, LOB, and Geography are not populated on the header row. | Implemented | **Added 8/27** (Cheryl's schema note); spec 014 FR-015. `AReLossSet` belongs to the out-of-MVP Analyze Re upload. |
| The ELT table receives stochastic events only. | Implemented | **Added 8/27** (Cheryl's schema note); spec 014 FR-011/FR-014. The split is validated against a source, never inferred from business rules (D31) — and the source settled on CIC's own `dbo.Lookup_RMS_HistoricalRDS` rather than the reference-data enrichment the callout below describes: an event is historical when the lookup holds a row for its event ID and the analysis's model version, stochastic otherwise. |
| Historical events are always written to their own table; there is no opt-in. | Implemented | **Added 8/27 as a commit-time checkbox; the checkbox dropped by spec 014 FR-014.** Volume varies by peril (a few for Hawaii earthquake, ~100–250 for hurricane) and never warranted a choice — every historical row goes to `dbo.RMS_HistoricalRDS` under the header's data ID, and the count is on the export detail page. Its event metadata and PCS number come from CIC's `dbo.Lookup_RMS_HistoricalRDS`, not the RiskLink reference data tables. |
| AAL is not written to the repository. | Implemented | **Added 8/27 (D16).** "Nope … That would just be calculated." |
| A repeated export creates a new data set under a fresh data ID; it never updates the prior one. | Implemented | **Added 8/27; spec 014 FR-004/FR-021.** How the current tool behaves. **O21-7 is answered (9/11 D13/D14, spec 014 P-17):** the workbench warns rather than blocks — the form names the newest earlier export, its requester and status, how many there are, and links to it, and exports anyway. Cheryl: "sometimes you run something and you're like, oh shoot, I forgot to turn on blah, blah, blah flag. So I delete it and then I run it again" — the analyst disambiguates with a data name ("I could put a V2"). A loaded data set is never rewritten. |

> **Narrowed 8/28 — event type comes from Risk Modeler's reference data APIs (note 22 D30/D31, was the O21-5 BLOCKER).** Confirmed negatives, checked live: event type is **not in the Parquet** ("the event type is not here for me"), **not selectable on the export job** ("it doesn't give you options … If I could, then it would be solved"), and the `event info` table is not visible in Ben's DataBridge SQL despite being documented ("I think it's a matter of my permissions"). Confirmed positive: clicking an event in the Risk Modeler UI fires **reference data API** calls that return its type — "I can clearly get this via API." The plan is **retroactive enrichment**: read every event ID from the Parquet, fetch each type via the API, add a type column. Open (O22-11): whether a bulk/filtered endpoint exists or it is one call per event (decides viability at ELT scale); Moody's answer on `event info` visibility (Cheryl doubts the reference tables exist on DataBridge at all — fallback: export them from RiskLink 25); and Cheryl's CSV cross-check of one small ELT against RiskLink's event info table, which may collapse the problem. **Design rule (D31): validate the stochastic/historical split against a source; never infer it from analysis type, model profile, or event rate scheme.** Historical *rates* are long-term event rates — nothing to do with historical *events* (Cheryl's correction).

> **Retrieval is a Parquet export job, pre-fetched (rewritten 8/28, D28/D29 — supersedes the paginated two-phase framing).** The paginated `get_elt()` path is abandoned ("I'm going to not do that"). Instead the workbench submits a Risk Modeler export job naming the analysis, the **ELT at portfolio output level**, and the **perspective codes**; a zip returns one Parquet file per perspective, queryable with Python "in a very similar way that we query databases." The files are **pre-fetched** ahead of the export, so the loss-greater-than-exposure check and the review run **on demand** with no wait — Cheryl: "Okay. It's just there." The export flow is still request → review → commit; what changed is that the data is already local when it starts. Open (O22-10): the pre-fetch trigger (on analysis completion vs. an earlier signal than the export click — 20 O20-15's "lazy" call has now moved earlier twice) and the storage cost at 100+ analyses × several perspectives. The record count is a post-read count of the Parquet file, not an API response field. Settles 20 O20-11 (Parquet: yes) and 21 O21-9's retrieval economics; still unmeasured: job latency for a large ELT and a PLT, zip size, rate limits (O21-9d).

**Treaty-level export — part B. Specified as spec 016 (`specs/016-ty-perspective-export/`, 2026-09-16): one data set per treaty per analysis from Risk Modeler's treaty output level, TY offered when every selected analysis ran with treaties, cross-analysis aggregation by grouping first. The 8/27–8/28 record follows.** Deferred then (8/27, D12/D13; source located and deferral re-affirmed 8/28, D26/D27). TY in the workflow tool's perspective picker is not a financial perspective — it directs the tool to export losses for specific treaties: one treaty selected across several analyses in the same EDM aggregates into a single loss set (the same treaty run against multiple perils); two treaties give two rows. Cross-EDM aggregation is impossible ("they would be given different IDs because they came from two different sources") — CIC exports per EDM and combines with the repository-side "group losses" feature, which is not workbench grouping (§6). **8/28 refinements (O22-12):** treaty losses live in **`RDM_TREATY`** (analysis ID + treaty ID + event ID, TY perspective code), joined to a treaty-description table for names; the portfolio-level extract cannot reach them ("it's pulling the portfolio level results, not the treaty level results"), so TY needs its own retrieval call — and Cheryl's sequencing makes the design constraint explicit: "it'll be a matter of just substituting where you're getting the data from, and everything else should be the same" — **build the export pipeline with the retrieval source swappable behind one transform/validate/commit flow.** Treaty selection needs per-analysis applied-treaty facts `irp_analysis` does not record (O22-7). A missing TY Parquet is **not** evidence TY is broken — Ben's test treaties take no loss (O22-14; Cheryl is providing one that does). Cheryl: "We have to get through part A first." Do not let it leak into the part-A spec because "treaty" sits in the perspective dropdown. (O21-4.)

**Out of scope for MVP:** **treaty-level (TY) export — part B** (above); **Post-Analysis Treaty (PATE)** — adding a cat treaty onto broker results after the fact and re-simulating; a rare fringe case, portfolio-level only, deferred (design note 05 §6, O5-4); formal loss validation against broker/cedant (a business check against external parties — it does **not** cover the export integrity checks above); replicating RiskLink's compare-and-export workflow (this exclusion does **not** cover the clipboard copy-with-headers, which is in scope — 8/25 D13); loading exposure summaries to the Exposure Repository; carrying the submission's CRM-ID **tag set** through automatically (reworded 8/27 — a single CRM ID entered on the export lands on `dbo.Data.CRMID`, Delivery above); uploading loss sets to Analyze Re (separate API — the unpopulated `AReLossSet` columns).

**Where the export ends, and where the analyst goes next (9/11, D4).** The
Workbench's export finishes at the loss repository. Pushing those loss sets to
CIC's simulation engine, Analyze Re, happens in CIC's own workflow tool, so the
analyst leaves the Workbench after every export — Wendy: "I will then go over to
our workflow tool, enter their parameters and upload to our simulation engine."
This is a workflow fact, not only an MVP boundary, and it is why several
Workbench decisions read the way they do: the data name stays optional because
it is editable in the workflow tool; the export status screens carry failure
triage rather than a results browser; and the loss repository explorer Anil
proposed was deferred rather than built (PRD §20.4).
