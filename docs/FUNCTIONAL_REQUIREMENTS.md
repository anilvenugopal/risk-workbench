...
| Requirement | Implementation | Notes |
|---|---|---|
| Own and broker results can be viewed together. | Implemented | Multiple analyses in one view. **Designed 8/25 (D17); uncapped confirmed 8/28 (D2)** — the ~10 soft guideline was never agreed with CIC and is retired; the view scrolls sideways. |
| Analyses can be compared side-by-side. | Implemented | **Designed 8/25 (D17/D18); built and accepted 8/28 (D7).** Compare is a separate action from View; strictly pairwise, pairs collected into a cart, 5 pairs per screen. Base-vs-many considered and rejected. The 5-pair cap was offered for expansion and declined ("Nope") — closes 19 O19-8's comparison half. |
| The side-by-side comparison includes a percent-difference column. | Implemented | e.g. CIC vs. broker — saves the manual Excel step. **Designed 8/25 (D19/D20); accepted 8/28 (D8).** Selection order is the contract — first picked = base = first column, tagged; percent change is (comparison − base) ÷ base × 100, spot-checked live by Cheryl. Closes 19 O19-9 — with a recorded warning: Cheryl initially assumed she could not control the base because she could not reorder the list, so **the base marker must be legible at selection time**, not only after the comparison renders. |
| Cross-currency comparison is blocked; mixed-currency viewing is permitted. | Implemented | **Built and accepted 8/28 (D9)**, confirming 20 O20-8/D17's asymmetry: for viewing, currency renders per column and "is not a factor"; comparison requires one currency. |
