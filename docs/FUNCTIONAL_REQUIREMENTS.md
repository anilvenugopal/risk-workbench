...
| A Submission has these attributes: name, cedant name, client, data vintage, treaty year, directory path, and zero or more contracts, each with a CRM ID, treaty type, inception date, expiration date and Contract status. | Implemented | Directory path = the deal's shared-drive folder. Client is optional and comes from CIC's repository client list, shown as "ID - name" (9/18). **Changed 9/22 (spec 017, note 32 D17–D23):** treaty type, inception, expiration and the Won / Lost / In Process status moved to the contract; a new contract opens on the previous one's dates and a blank expiration is inception plus one year less a day. Data vintage arrived on the submission — "the in-force data as-of date of the data that's been provided to us in the EDM" (Wendy) — one per submission by convention even when its portfolios differ ("we would usually pick one"), and overridable on the export. |
