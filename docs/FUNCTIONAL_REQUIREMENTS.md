...
| Requirement | Implementation | Notes |
|---|---|---|
| The analyst selects which analyses to group. | Not implemented | Demoed 8/28 from a prototype: select analyses → group → add more → rename → pick currency → submit. Pick-list scoped to the submission (note 17 O17-6). |
| The event rate scheme is selected at group creation, defaulted — never derived from the members. | Not implemented | **Added 8/28 (D13/O22-1). The defect's fix.** The prototype auto-selects and passes through whatever the members had. Risk Modeler presents one scheme, defaulted (historical, in the demo pair) and changeable — confirm whether that default is a rule or a coincidence of the pair. |
| The Risk Modeler request's region-peril simulation set is sourced from the API contract, not reverse-engineered. | Not implemented | **Added 8/28 (O22-1b).** Ben derives this required request attribute by inferring how Risk Modeler assembles it from members' model profiles and rate schemes — undocumented, unversioned, now suspect. Source it from the contract, or record the derivation with a test that fails when Risk Modeler changes. The attribute name is transcript-garbled — confirm it against the API contract first. |
| Group currency is selected at group creation. | Not implemented | Demoed 8/28; consistent with note 17 D1/O17-2's submit-time currency model. |
| A group records the run parameters it was created with. | Not implemented | **Added 8/28 (D18/O22-1c).** Event rate scheme, currency, and whatever the simulation-set attribute resolves to have no recorded home; the group metadata screen shows analysis attributes with "some missing … we need to pull it from the right spot." `irp_analysis.submitted_settings` is documented "own analyses only" and holds exactly these fields — decide whether a group row writes it too (DATA_MODEL §6; constitution Art. 8). |
| Invalid groupings show error messaging. | Not implemented | e.g. mixing DLM and HD analyses — **and, added 8/28 (O22-2), the likelier and silent case: members with different event rate schemes when no single scheme is chosen** (the ~$8M defect above). |
| A group shows the list of analyses included in it. | Not implemented | **Added 8/28 (D16). Required.** "Wendy's off on vacation, I have to look at something she did. I don't know inherently from looking at that group what's in it." Structurally free — `group_parent_id` already holds membership; this is a view requirement. |
| Group membership can be edited. | Not implemented | **Added 8/28 (D17) — explicitly a later round.** Groups are cheap to delete and rebuild; the member list above is the prerequisite. An edited group must re-run in Risk Modeler — whether that reuses the row or supersedes it, and what happens to the existing results, is undecided (O22-3). |
| The group builder list sorts and searches by name. | Not implemented | **Added 8/28 (D15).** "Somewhere you might have 50 or 75 analyses to weed through." Does not reverse note 18 O18-5's "I don't need search" — that ruling was about the analyses grid, where filtering does the work; the builder is a flat multi-select pick-list with no filters. |
| Group names are auto-generated and editable. | Not implemented | **Reworded 8/28 (D10)** from "auto-generated from the deal." The demoed default (`CRE` + submission name) drew no objection: "people are going to rename it no matter what you pick." Editability is the requirement; the generation rule is a low bar. |
| "Propagate detailed output" is a group-creation toggle; "create independent groups" is not offered. | Not implemented | **Added 8/28 (D11).** Independent groups "removes all the details underneath … that sounds like it wouldn't be too useful" — hard-coded false. |
| A group's *results* are consumed like any analysis's; the group *object* is not. | Not implemented | **Reworded 8/28 (O22-3)** from "a group is treated like any other analysis," which is wrong as written: viewing a group needs more (the member list) and its metadata comes from a different place. |
| Groups appear in line on the EDM pages their member analyses relate to. | Not implemented | **Added 8/28 (D24).** Groups relate only to submissions today, so a group created from the EDM page vanishes there. Fix: query groups whose member analyses relate to the EDM. A separate group section was floated and dropped — the engine column distinguishes them, group name spanning Portfolio + Template ("I'm fine with that"); the engine column must be added to the EDM screen. |
| Group creation shows live status. | Not implemented | **Deprioritized 8/28 (D12), not dropped.** The prototype's rendering is broken; grouping ran fast in the demo ("maybe it's not needed"), but Ben's prior experience with slow jobs was not grouping — keep it on the list. |
| Nested grouping (groups of groups) is supported. | Not implemented | **Unconfirmed with CIC — ask before building (8/28, O22-2).** Nothing in notes 15–22 confirms CIC wants it, and note 17 D3 rejected the analogous suites-of-suites because AIR's version caused confusion. |

> **Validation method (8/28, D14 — agreed with CIC, goes in the grouping spec).** Run matched examples manually in Risk Modeler and in the workbench and compare the outputs — **and compare the EP curve, not the AAL.** An AAL comparison will not catch a double-listed event; the curve will.
...
| The analyst controls the left-to-right order of analyses and groups on the results view. | Partial | **Added 8/25 (D15).** "I want to see my analyses left to right and my group at the end, or my group at the beginning." A reorder control exists; **drag-and-drop deferred 8/28 (D4)** — "Drag and drop would be better. I agree. It doesn't need to take priority over other things." |
...
> **Rewritten 8/27.** Cheryl's workflow-tool walkthrough (design note 21) replaced the single "ELTs are uploaded" row with the export flow below, and the loss repository schema arrived the same day as screenshots (`dbo.Data`, `dbo.RMSELT`, `dbo.RMS_HistoricalRDS` — column detail in PRD §16.3). The flow: pick analyses → choose a client and enter the export parameters → pick one valid perspective → review with integrity checks → commit.
...
| The workbench exports finalized loss sets to the Loss Repository itself: read from the Risk Modeler API, transform, write SQL. | Not implemented | **Settled 8/27 (D1), re-confirming 8/26 D18 from the other direction.** Reusing CIC's workflow tool was raised and closed: it only sees analyses that live in RiskLink, which would mean round-tripping Risk Modeler results back to RDM and keeping RMS servers alive. DataBridge is not in the path — the 8/26 open question is answered by removal. Do not reopen. |
| One export batch covers one or more selected analyses. | Not implemented | Ben's restated acceptance list, 8/27: "pick multiple analyses … provide a specific perspective that's valid across all of these analyses … search that client list by name … review versus commit." |

**Delivery — client selection**

| Requirement | Implementation | Notes |
|---|---|---|
| The export selects a client from CIC's client table, searched by name. | Not implemented | **Added 8/27 (D2, D3).** The table lives in the loss repository database and CIC maintains it: "I don't think it's anything you have to maintain. I just think it's something you have to be able to access." Still open (note 22 O22-9): where the selected client ID is recorded against the export, and whether the search is scoped or a raw name search across the table. |
| The client table is read-only; the workbench never adds clients. | Implemented | **Reversed 8/28 (note 22 D25), retracting 8/27 D4's INSERT.** Cheryl, after consulting Cheng: "I think we need to view the client table. I don't think we have to add clients … we would have to set up a client ID at that point [during exposure work, in Cheng's workflow tool] … So we can take that off the list." The reasoning is a workflow fact: by the time the workbench reaches the export, the client exists; edge cases go through the workflow tool or SQL. The insert-statement ask is withdrawn. Cheng's add logic, recorded for schema shape only: last ID + 1, IDs above ~8,000 are test entries excluded from the running number; the valid flag is manually maintained off the system (confirms 21 D5 from the other direction — nothing sends it). |
| The client ID is not the cedant name and is not stored on the submission. | Not implemented | **Added 8/27 (D6).** A repository client is roughly cedant × reinsurance type — an external key CIC built by hand; one submission can push under different client IDs across treaty types. The client is chosen per export. |

**Delivery — export parameters**

| Requirement | Implementation | Notes |
|---|---|---|
| Treaty year and inception date auto-populate from the submission and stay editable. | Not implemented | **Added 8/27 (D7).** `submission.treaty_year`, `submission.inception_date`. |
| The analyst enters a data vintage. | Not implemented | **Added 8/27.** `dbo.Data.DataVintage` is a date column; what sources it is unconfirmed (O21-2). |
| A CRM ID is entered on the export and written to the header row. | Not implemented | **Added 8/27.** `dbo.Data.CRMID`. One ID per export — see the reworded exclusion below. |
...
| The data ID is the repository's auto-incrementing counter, read back after the header insert. | Not implemented | **Added 8/27 (D15).** `dbo.Data.DataID`. The workbench never generates it. |

**Delivery — perspective**

| Requirement | Implementation | Notes |
|---|---|---|
| Export offers only the perspectives that actually have data, resolved per analysis from the platform. | Not implemented | **Added 8/27 (D9).** "So should we just provide the list of perspectives that are valid?" — "That would be fabulous." Not a fixed list; WX and QS returned no data for a live analysis. |
| With several analyses selected, the offered list is the intersection across all of them. | Not implemented | **Added 8/27 (D10).** Matches the workflow tool: "all that metadata has to be the same for every analysis that I select." |
| One perspective per export batch. | Not implemented | **Added 8/27 (D11).** Per-analysis and multi-select perspectives were offered and rejected as rare ("Generally, it's a one-to-one"). Two gross + one post-cat = two batches. |
| RP (post-cat) is offered for export. | Not implemented | **Added 8/27.** "The RP perspective is one we definitely need to have." The export vocabulary is wider than the viewing set. |
| WX and QS are never uploaded to the repository. | Not implemented | **Added 8/27 (D14).** A scope rule, not a display filter: "it's going to be across multiple perils. And what we're looking for is for a specific treaty." |
...
**Delivery — review & integrity checks**

| Requirement | Implementation | Notes |
|---|---|---|
| The analyst reviews the export before committing. | Not implemented | **Added 8/27.** Per analysis: analysis ID, name, description, currency. AAL is not shown in the review (D17) — the results view already shows it; if the AAL column above is ever dropped, the review needs its own. |
| Events with loss greater than exposure are caught before commit and the loss is capped at the exposure value. | Not implemented | **Added 8/27 (D18). Must-have.** Such rows hard-error the repository load; the fix is "just set them equal … It's 100% loss" (usually pennies of difference). **8/28 (D29):** the ELT Parquet is pre-fetched, so the check runs on demand with no wait at review — "It's just there." Which `dbo.RMSELT` columns the check compares (`Loss`, `StdDevC`, `ExpValue`) needs written confirmation — the transcript terms were garbled (O21-12). |
| The count of capped rows is shown in the review and recorded on the export. | Not implemented | **Added 8/27.** The cap mutates what CIC receives — never applied silently. |
...
> **These integrity checks are not covered by the exclusions below.** The out-of-scope "formal loss validation against broker/cedant" is a business check against external parties, and the deferred Phase A DataBridge validation is exposure-side reporting. Neither covers D18 — do not reconcile the must-have away. Also pending: the full list of silent corrections Cheng applies in the current tool (negative standard deviations is the one known case); the review step is not fully specified until that list exists (O21-8, O21-13).

**Delivery — commit**

| Requirement | Implementation | Notes |
|---|---|---|
| Commit writes three loss-repository tables, identified by the data ID. | Not implemented | **Added 8/27 (D15), correcting 8/26's two-table reading.** `dbo.Data` (header), `dbo.RMSELT`, `dbo.RMS_HistoricalRDS` (opt-in). Column detail: PRD §16.3. |
| The header row carries the analyst-entered parameters; only the data ID is carried into the loss tables. | Not implemented | **Added 8/27.** "This is sort of the glue that keeps all of our other pieces together." |
| ArchiveFile, AReLossSet, LOB, and Geography are not populated on the header row. | Not implemented | **Added 8/27** (Cheryl's schema note). `AReLossSet` belongs to the out-of-MVP Analyze Re upload. |
| The ELT table receives stochastic events only. | Not implemented | **Added 8/27** (Cheryl's schema note). The stochastic/historical split comes from the reference-data-API enrichment in the callout below (narrowed 8/28); the split is validated against a source, never inferred from business rules (D31). |
| Historical events are written to their own table only when the analyst opts in at commit. | Not implemented | **Added 8/27.** A commit-time checkbox; volume varies by peril (a few for Hawaii earthquake, ~100–250 for hurricane). `dbo.RMS_HistoricalRDS` holds the historical event IDs and metadata, sourced from the RiskLink reference data tables. Whether it was written is recorded on the export. |
| AAL is not written to the repository. | Implemented | **Added 8/27 (D16).** "Nope … That would just be calculated." |
| A repeated export creates a new data set under a fresh data ID; it never updates the prior one. | Not implemented | **Added 8/27.** How the current tool behaves; whether the workbench warns on re-export is open (O21-7). |
