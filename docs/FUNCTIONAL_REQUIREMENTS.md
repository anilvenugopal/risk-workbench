...
| Currency, currency scheme, and vintage are chosen at group-submit time. | Partial | Same picker as analysis submission (§5, spec 009 P-11). **Decided 9/3 (note 26 D7)**: the currency is prefilled with the code every member ran in, else the env default (USD); scheme and vintage keep the env defaults — spec 012 O-10. |
| Group simulation periods are chosen from Risk Modeler's fixed list. | Partial | **Decided 9/3 (note 26 D18)**: 3,125 … 800,000 in a dropdown, 50,000 preselected — spec 012 O-11. **9/4**: the same dropdown on every partition row of a PLT group (`regionPerilSimulationSet[].simulationPeriods`), the group-level one labelled Group simulation periods, no member-length hint. Whether the group-level value overrides the per-partition values (note 26 D19) is still open. |
| Finish on the Members screen inspects and submits in one step when nothing is left to choose. | Partial | **Decided 9/3 (note 26 D8–D10)**: defaults are the members' currency, env scheme and vintage, Propagate ON; **9/4**: a PLT group takes 50,000 simulation periods for the group and every partition. Stops on the Inspection screen with a notice when a scheme or simulation-set choice is needed, currencies differ, or the inspection blocks; treaty mismatches do not stop it. Success shows a confirmation pane — spec 012 O-14. |
...
| Mixing DLM and HD analyses in one group is supported. | Partial | **Reversed 8/27** — previously listed as an invalid grouping. irp-integration reconciles ELT and PLT members in one `regionPerilSimulationSet` and derives `simulateToPLT`; the analyst chooses the simulation periods. |
...
| Requirement | Implementation | Notes |
|---|---|---|
| Own and broker results can be viewed together. | Implemented | Multiple analyses in one view. **Designed 8/25 (D17):** viewing is N-up (~10 soft guideline — never a hard block; scroll/pagination past it). **Built and accepted 8/26** (note 20 D1, spec 011): the merged table and the dedicated results page; the build is uncapped and scrolls sideways. |
| Any two finished analyses can be compared side-by-side, regardless of origin. | Implemented | Ben has a prior comparison engine to build on. **Designed 8/25 (D17/D18), extended 8/26 (note 20 D16):** Compare is a separate action from View; a modal collects strictly pairwise selections into a cart, ~5 pairs per screen (a layout limit — the number is unconfirmed with CIC, note 19 O19-8). Base-vs-many considered and rejected. Renders on a dedicated comparison page in a new browser tab (decided 8/27, PRD §17.3). **Built 8/28 (spec 013):** the Compare modal collects pairs; `/results/comparison` renders them in a new tab. |
