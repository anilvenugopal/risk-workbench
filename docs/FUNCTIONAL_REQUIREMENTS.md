...
| Treaty year is derived from the inception date. | Implemented | Auto-populated, never free-text entry (reconfirmed 8/4). |
...
| The cedant field is a typeahead over existing cedants. | Partial | The current auto-suggest is "not what you think in terms of an auto populate drop down type of deal" (8/4). |
| Name, cedant, treaty type, and inception date are required at creation. | Implemented | Confirmed 8/4 — "seems fair." |
| Required fields are marked explicitly in the form. | Partial | **Added 8/4.** An empty submit returns a bare "cedant is required": "that's not clear… we need to make that very explicit." |
...
| A Submission can link to a related Submission. | Partial | UI label is **"Links to"** (8/4, superseding "Previous" from 7/14) — the relationship is a link to a related submission ID, not necessarily a renewal; the team also jumps back to compare. Link may come from the treaty system. |
