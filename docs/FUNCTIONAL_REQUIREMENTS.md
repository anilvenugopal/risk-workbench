...
| Both corrections are counted per analysis and shown on the export detail page. | Implemented | **Added 8/27; spec 014 FR-018.** Either correction mutates what CIC receives — never applied silently. The detail page shows rows staged, stochastic rows, historical rows, rows with exposure raised, and rows with standard deviation zeroed. |
