...
| Group simulation periods are chosen from Risk Modeler's fixed list. | Partial | **Decided 9/3 (note 26 D18)**: 3,125 … 800,000 in a dropdown, 50,000 preselected, the largest member PLT length as a hint — spec 012 O-11. Whether the group-level value overrides the per-partition simulation set (note 26 D19) is still open. |
| Finish on the Members screen inspects and submits in one step when nothing is left to choose. | Partial | **Decided 9/3 (note 26 D8–D10)**: defaults are the members' currency, env scheme and vintage, Propagate ON, ELT. Stops on the Inspection screen with a notice when a scheme or simulation-set choice is needed, the output is PLT, currencies differ, or the inspection blocks; treaty mismatches do not stop it. Success shows a confirmation pane — spec 012 O-14. |
