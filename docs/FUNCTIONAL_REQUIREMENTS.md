...
| Any two finished analyses can be compared side-by-side, regardless of origin. | Not implemented | Ben has a prior comparison engine to build on. **Designed 8/25 (D17/D18), extended 8/26 (note 20 D16):** Compare is a separate action from View; a modal collects strictly pairwise selections into a cart, ~5 pairs per screen (a layout limit — the number is unconfirmed with CIC, note 19 O19-8). Base-vs-many considered and rejected. Renders on a dedicated comparison page in a new browser tab (decided 8/27, PRD §17.3). |
| Comparison pairs are drawn from the merged analyses table the analyst is on. | Not implemented | **Decided 8/27.** No cross-submission pairing — the year-over-year comparison stays served by copy-with-headers (Delivery, below). The modal offers only analyses with retrieved results. |
| The side-by-side comparison includes a percent-difference column. | Not implemented | e.g. CIC vs. broker — saves the manual Excel step. **Designed 8/25 (D19/D20):** selection order is the contract — first picked = base = first column, percent change follows; decoupled from list order; no row-header column; percent-change columns need no per-pair labelling. |
| A comparison pair must share a run currency — mixed-currency pairs are blocked, never converted. | Not implemented | **Added 8/26 (note 20 D17).** Wendy: "I would just not let people do it"; Cheryl: "everybody should know that's not a valid comparison." Enforced at pair-add time from the run currency stored in `submitted_settings` (spec 011). Deliberate asymmetry: N-up *viewing* still shows mixed currencies unconverted — viewing makes no arithmetic claim; comparison computes a percent change. |
| Perspective and EP-type selections apply screen-wide, to every comparison rendered. | Not implemented | **Designed 8/25 (D20) + 8/26 (note 20 D11):** one EP type at a time; AAL and standard deviation sit outside the selection. Never per pair. |
| The comparison shows which engine/model version each side ran. | Not implemented | Wendy's broker-vs-own requirement (design note 18 O18-10): "just because they ran it in 23 doesn't mean they used the 23 rates." Spec 011 FR-021 captured engine and model version at retrieval so this view can show them. |
